...
class Agent:
    ...
    def _make_client(self):
        ...
        if not api_key:
            raise AgentError(
                "Ключ API не задан: укажите DEEPSEEK_API_KEY в файле day19/.env "
    ...
    def generate(self, prompt: str) -> dict:
        ...
        pipeline_report = self.apply_pipeline(prompt, payload)
        ...
        # распознанная как пайплайн, одиночным вызовом больше не разбирается: один
        # и тот же запрос не должен запускать инструменты двумя путями.
        mcp_report = {}
        if not pipeline_report["detected"]:
