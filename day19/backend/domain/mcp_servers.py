...
(``day19/mcp_server``) и два сервера официального набора для сравнения. Каталог
...
SERVER_KEY_DAY19 = "day19-pipeline"
...
KNOWN_SERVERS: tuple[MCPServerOption, ...] = (
    MCPServerOption(
        key=SERVER_KEY_DAY19,
        label="День 19: свой MCP-сервер (jsonplaceholder + пайплайн)",
        target=config.MCP_DEFAULT_TARGET,
        description=(
            "Собственный сервер дня по stdio, девять инструментов. Чтение: "
            "get_user, get_post и list_user_posts читают "
            "https://jsonplaceholder.typicode.com. Планировщик: schedule_reminder, "
            "collect_data и generate_summary планируют фоновую работу через API "
            "дня. Композиция: search ищет данные в источнике (лента, файл дня или "
            "таблица SQLite), summarize сводит найденное, save_to_file пишет "
            "результат в файл каталога output/."
        ),
    ),
    MCPServerOption(
