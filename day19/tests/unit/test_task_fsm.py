...
Форма — как в ``tests/unit/test_context_fsm.py``: таблица переходов объявлена
...
EXPECTED_ERRORS: dict[TaskStage, dict[TaskEvent, type[Exception]]] = {
    TaskStage.PLANNING: {
        TaskEvent.ROLLBACK: InvalidTransitionError,
    ...
    TaskStage.DONE: {
        TaskEvent.PAUSE: InvalidTransitionError,
        TaskEvent.ADVANCE: InvalidTransitionError,
        TaskEvent.ROLLBACK: InvalidTransitionError,
        TaskEvent.RESUME: UnknownTaskEvent,
    },
    TaskStage.PAUSED: {
...
def test_pause_keeps_step(stage: TaskStage) -> None:
    ...
    Только рабочие этапы: у done паузы нет вовсе (терминальный этап), у paused
    она уже стоит.
    """
    step = STAGE_SAMPLE_STEP[stage]
...
def test_pause_from_done_and_rollback_from_planning_are_explicit_errors() -> None:
    ...
    Пауза из ``done`` теперь тоже ошибка (день 15): этап терминальный,
    продолжать сданную задачу некуда, а пауза подразумевала бы возврат.
    """
    with pytest.raises(InvalidTransitionError):
        make_state(TaskStage.PLANNING).handle(TaskEvent.ROLLBACK, TaskStep.DEFINE_SCOPE)
    with pytest.raises(InvalidTransitionError):
        DoneState().handle(TaskEvent.PAUSE, TaskStep.FINALIZE)
    with pytest.raises(InvalidTransitionError):
