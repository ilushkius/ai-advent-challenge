...
#: Корень дня (day19/mcp_server/config.py → day19/) и корень репозитория: из
...
#: аргументом ``--backend-url`` у сервера и переменной окружения ``DAY19_BACKEND_URL``.
...
#: Файл .env дня — тот же, что читает бэкенд (day19/.env).
...
def resolve_api_key():
    """Ключ DeepSeek: day19/.env → переменная окружения → None.
