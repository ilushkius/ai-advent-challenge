"""DeepSeek внутри инструмента ``summarize`` (день 19).

Ключ берётся из ``day19/.env`` или переменной окружения (``config.resolve_api_key``),
клиент создаётся ЛЕНИВО и живёт в модуле один на процесс: инструмент вызывается из
пайплайна много раз, а соединение на каждый вызов никто не заводит.

Главное правило модуля: **отсутствие LLM — не ошибка инструмента**. ``available()``
отвечает, можно ли вообще звать модель (режим ``--llm off`` выключает её принудительно),
а ``summarize`` бросает ``LLMUnavailable`` — инструмент ловит его и переходит на
агрегацию. Так демо и тесты работают без ключа и без сети, а с ключом получают
настоящую сводку.
"""
from __future__ import annotations

from mcp_server import config  # первый: он добавляет корень репозитория в sys.path
...
def summarize(prompt: list[dict], *, max_tokens: int = config.LLM_MAX_TOKENS) -> str:
    ...
    if not key:
        raise LLMUnavailable(
            "Ключ DeepSeek не найден: укажите DEEPSEEK_API_KEY в day19/.env "
