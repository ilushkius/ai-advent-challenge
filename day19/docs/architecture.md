...
Свой MCP-сервер дня (`day19/mcp_server/`, десять модулей, транспорт stdio)
...
flowchart LR
    ...
    SRV -->|"summarize (LLM)"| D
    SRV -->|"save_to_file"| OUT[("day19/output/<br/>файлы пайплайна")]
    AG --> DB[("SQLite<br/>day19/agents.db")]
    MM --> DB
    PS --> DB
    TS --> DB
    IM --> DB
...
├── mcp_server/       СВОЙ MCP-сервер (10 модулей, транспорт stdio): config (адрес
...
│   │                 границы пайплайна), dependencies (get_manager, get_mcp_registry,
...
│   │                 memory, pipeline, profile, scheduler, task
│   ├── api/          FastAPI: agents, context, invariants, mcp, memory, pipelines,
│   │                 profiles, scheduler, tasks + main и lifespan (старт планировщика)
│   └── utils/        слой объявлен обязательным, но пуст: общий код — в repo-level shared/
├── tests/            unit/ (33 файла), integration/ (32), e2e/ (11) + conftest.py,
│                     support.py, mcp_fakes.py, scheduler_fakes.py, pipeline_fakes.py,
│                     backend_stub.py, mcp_echo_server.py и stub_api.py
...
`uv sync` не пытается установить сам `day19/` в `.venv`.

## Компоненты

| Файл | Зона ответственности |
| --- | --- |
| `day19/app.py` | Streamlit, точка входа (60 строк): `st.set_page_config` («Пайплайны MCP · День 19», «🔀»), `common.init_state()` → `sidebar.render_sidebar()` → `chat_section.render_main_area()`; переключатель семи разделов основной области — `st.radio` («💬 Чат и память» / «👤 Профиль пользователя» / «🧭 Состояние задачи» / «📏 Инварианты» / «🔌 MCP» / «🗓 Планировщик» / «🔀 Пайплайны») |
...
| `mcp_server/file_writer.py` | Запись результата пайплайна: `configure(output_dir)` (`--output-dir`, по умолчанию `day19/output/`), `sanitize_filename` (путь и `..` запрещены, расширение заменяется на запрошенный формат), `render_content` (`txt`/`md` — текст как есть, `json` — объект с полями), `save` → `SavedFile` (`filepath` абсолютный, `size_bytes` по UTF-8); ошибка — `SaveFileError` |
...
| `backend/domain/mcp_servers.py` | Каталог известных серверов для `GET /mcp/servers`: ключи `day19-pipeline`/`fetch`/`filesystem` (у своего сервера девять инструментов — три читают jsonplaceholder, три планируют фон, три служат композиции), `MCPServerOption` (`matches` — сравнение по нормализованной цели, `to_dict`), `KNOWN_SERVERS`, `server_records(connected_target, tool_count)` |
...
|---|---|---|
| Интерфейс | `app.py` (точка входа, 60 строк), `frontend/` (20 модулей) | Страница собирается вызовами секций: `common.init_state()` → `sidebar.render_sidebar()` → `chat_section.render_main_area()`; модули `frontend/` не вызывают `st.*` на импорте |
| MCP-сервер | `mcp_server/` (`config`, `schemas`, `api_client`, `backend_api`, `search_sources`, `summarize_logic`, `file_writer`, `llm_client`, `pipeline_tools`, `server` + `__init__.py`) | Отдельный процесс (транспорт stdio), который поднимает клиент дня: знает только `mcp` SDK, `httpx` и `shared/` (клиент DeepSeek), о бэкенде дня не знает ничего; инструменты объявляются декоратором `@server.tool()` либо регистрируются вызовом `register_pipeline_tools(server)`, поэтому контракт (`inputSchema`/`outputSchema`) собирает SDK из аннотаций и докстринга |
| API | `backend/api/main.py` (сборка `app`), `backend/api/` (9 роутеров), `backend/core/dependencies.py` | Эндпоинт лежит в файле своего домена; доступ к менеджеру — одна точка (`dependencies.get_manager`), её и подменяют тесты |
| Схемы API | `backend/schemas/` (`agent`, `context`, `invariant`, `mcp`, `memory`, `pipeline`, `profile`, `scheduler`, `task` + `__init__.py`) | Схемы разложены по доменам, а импорт остался `from backend.schemas import ...` |
| Данные | `backend/models/*.py` (ORM-таблицы по доменам), `backend/storage/` (`database` — движок и сессии, `task_store`, `invariant_store`, `memory_rows`, `pipeline_store`/`pipeline_rows`) | Таблицы дня наследуются от `shared.db_base.Base`; движок и фабрика сессий — общие помощники `shared/db_base.py`; ORM разложен по доменам слоя, реэкспорт классов — из `backend.storage.database` |
| Домен | `backend/domain/` (`strategies`, `context_fsm`, `context_policy`, `fact_extractor`, `memory_layers`, `profile_values`, `profiles`, `demo_profiles`, `task_fsm`, `task_state_machine`, `task_proposal`, `task_prompt`, `task_intent`, `invariant_values`, `invariant_rules`, `invariant_prompt`, `demo_invariants`, `mcp_connection_fsm`, `mcp_target`, `mcp_tools`, `mcp_tool_call`, `mcp_intent`, `mcp_prompt`, `mcp_servers`, `pipeline_spec`, `pipeline_mapping`, `pipeline_fsm`, `pipeline_intent`, `pipeline_prompt`), `backend/services/` (`compressor`, `task_state`, `invariant_checker`, `mcp_client`, `mcp_errors`, `mcp_registry`, `mcp_loop`, `mcp_transport`, `mcp_tool_runner`, `pipeline`, `pipeline_service`), `backend/storage/` (`database`, `task_store`, `invariant_store`, `memory_rows`, `pipeline_store`), `backend/agents/` (`agent`, `agent_manager` + `manager_*.py`, `memory`, `profile_store`) | Логика дня; крупный класс `AgentManager` собран из миксинов по доменам, состояние задачи разложено на «автомат шагов (`task_fsm`) / граф и guards (`task_state_machine`) / поведение и хранение», проверка инвариантов — на «правила / оркестрация / хранение», путь вызова MCP-инструмента — на «правила допуска и FSM (`mcp_tool_call`) / распознавание (`mcp_intent`) / текст блока (`mcp_prompt`) / оркестрация (`mcp_tool_runner`)», пайплайн — на «декларация (`pipeline_spec`) / маппинг и guards (`pipeline_mapping`) / FSM прогона (`pipeline_fsm`) / распознавание реплики (`pipeline_intent`) / текст блока (`pipeline_prompt`) / прогон (`pipeline`) / запуск и история (`pipeline_service`)», а чистые правила (`domain/`) не зависят ни от БД, ни от HTTP |
| Общий код | `shared/` | Код, не меняющийся между днями: клиент DeepSeek, база SQLAlchemy, токены, логи |
| Тесты | `tests/unit/` (33 файла, в том числе `test_mcp_tool_call.py`, `test_mcp_intent.py`, `test_mcp_prompt.py`, `test_mcp_servers.py`, `test_pipeline_fsm.py`, `test_pipeline_mapping.py`, `test_pipeline_spec.py`, `test_pipeline_intent.py`, `test_pipeline_prompt.py`, `test_search_sources.py`, `test_summarize_logic.py`, `test_file_writer.py`), `tests/integration/` (32, в том числе `test_mcp_server_stdio.py`, `test_mcp_tool_runner.py`, `test_mcp_agent.py`, `test_pipeline_store.py`, `test_pipeline_execution.py`, `test_pipeline_service.py`, `test_pipeline_agent.py`), `tests/e2e/` (11, в том числе `test_mcp_call_api.py`, `test_pipelines_api.py`) + `conftest.py`, `support.py`, `mcp_fakes.py`, `pipeline_fakes.py`, `stub_api.py` | Раскладка по фикстурам: `unit/` — без БД и агента, `integration/` — временная SQLite и `Agent`, `e2e/` — `TestClient`; общие фикстуры и фейки остаются в корне `tests/` (для MCP это `mcp_fakes.py`, для пайплайна — `pipeline_fakes.py`, для внешнего API сервера — стенд `stub_api.py`) |
| Скрипты | `scripts/` (`mcp_tool_demo.py` + `mcp_tool_report.py` — доказательство дня 17, `scheduler_demo.py` + `scheduler_scenarios.py` + `scheduler_report.py` — сценарии планировщика, `pipeline_demo.py` + `pipeline_scenarios.py` + `pipeline_report.py` — сценарии пайплайна, `mcp_demo.py`, `controlled_transitions_demo.py` + `transitions_report.py`, `task_state_demo.py`, `invariants_demo.py`, `seed_invariants.py`, `personalization_comparison.py`) | Прогоны вне pytest на своих БД: пишут отчёты в `docs/reports/` (или в корень дня), ключ и сеть не нужны; сквозной прогон дня 17 требует лишь своего MCP-сервера, а пайплайна — того же сервера с `--llm off` |
...
ORM-класс `database.UserProfile`, таблица `user_profiles` в `day19/agents.db`:
...
(`day19/mcp_server/`), у которого сегодня девять инструментов: три читают
...
`DAY19_BACKEND_URL`), поэтому тот же сервер без правок кода можно направить на
...
`DAY19_BACKEND_URL` (по умолчанию `http://127.0.0.1:8000`), таймаут —
...
отвечает `disconnected`. Пайплайн — другое дело: о MCP-соединении он не хранит
ничего, но **свои прогоны журналирует** в `pipeline_runs`/`pipeline_steps`,
поэтому история запусков и шаги переживают перезапуск.
...
`DAY19_BACKEND_URL` (по умолчанию `http://127.0.0.1:8000`), таймаут 90 с
...
- **Запись только в `day19/output/`.** `save_to_file` чистит имя (путь и `..`
...
- **Один процесс бэкенда и один файл `day19/agents.db`.** Несколько процессов
