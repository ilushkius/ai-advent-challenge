# API дня 19 — агенты DeepSeek с MCP-инструментами, декларативным пайплайном, планировщиком фоновых задач, контролируемыми переходами, инвариантами, состоянием задачи, памятью и профилем

Бэкенд — FastAPI-приложение `day19/backend/api/main.py`. Заголовок приложения —
«Агенты DeepSeek + пайплайны MCP-инструментов — День 19», версия схемы — `13.0.0`
...
живёт в `day19/mcp_server/` (stdio, девять инструментов: `get_user`, `get_post`,
...
Схемы ответов описаны в пакете `day19/backend/schemas/` — по доменам: `agent.py`
...
Всего 78 эндпоинтов: 11 в разделе агентов (десять эндпоинтов CRUD, генерации и
...
инвариантов, MCP, планировщика и пайплайна, а также перечень эндпоинтов (77 строк —
...
```json
{
  "name": "Агенты DeepSeek с контролируемыми переходами, MCP, планировщиком и пайплайнами — День 19",
  "docs": "/docs",
...
Свой сервер дня живёт в `day19/mcp_server/` и запускается по stdio: клиент
...
```json
{
  "servers": [
    {"key": "day19-pipeline",
     "label": "День 19: свой MCP-сервер (jsonplaceholder + пайплайн)",
     "target": "uv run python mcp_server/server.py",
     "description": "Собственный сервер дня по stdio, девять инструментов. Чтение: get_user, get_post и list_user_posts читают https://jsonplaceholder.typicode.com. Планировщик: schedule_reminder, collect_data и generate_summary планируют фоновую работу через API дня. Композиция: search ищет данные в источнике (лента, файл дня или таблица SQLite), summarize сводит найденное, save_to_file пишет результат в файл каталога output/.",
     "connected": true,
     "tool_count": 9},
...
`day19/output/`: имя чистится (путь и `..` запрещены), а расширение заменяется на
запрошенный формат.
    ...
    {"id": 10, "run_id": 4, "step_index": 2, "tool_name": "save_to_file",
     ...
     "output_result": {"reason_code": null, "is_error": false, "duration_ms": 7,
                       "structured": {"filename": "api-run.md",
                                      "filepath": "…/day19/output/api-run.md",
...
| `422` | невалидное тело запроса (Pydantic/FastAPI): невалидные поля профиля, `initial_stage` вне `planning`/`execution`/`validation`, неизвестный этап/шаг, слишком длинные `expected_action`/`reason`, пустой `task_id`, тело `PATCH /tasks/{task_id}/context` без единого флага, пустая цель или неизвестный транспорт у `POST /mcp/connect`, пустое или длиннее 100 символов имя инструмента у `POST /mcp/call`, `schedule_type` вне `date`/`interval`/`cron`, пустой или длиннее 64 символов `tool`, пустое или длиннее 100 символов `name` у `POST /scheduler/tasks`, не объект `pipeline`/`initial_args`, не булево `background` у `POST /pipelines/run`, нечисловой `limit`/`run_id` в `/pipelines/...` | объект с `detail` — списком ошибок |
...
- **Всё состояние переживает рестарт.** Реплики (`short_term_messages`), рабочая
  ...
  лежат в SQLite (`day19/agents.db`), а активные
...
- **Файлы пайплайна пишутся только в `day19/output/`.** `save_to_file` чистит имя
