...
Карта модулей дня 18: что где лежит и за что отвечает. Правила структуры — в
...
День 18 — копия дня 17 плюс **планировщик фоновых задач** (APScheduler). Три
MCP-инструмента получили отложенное и периодическое выполнение:
...
сводка по накопленным данным). Метаданные задач лежат в SQLite
(`scheduled_tasks`), APScheduler поднимается в `lifespan` FastAPI и сверяется с
таблицей, поэтому задачи переживают перезапуск процесса. Добавились роутер
`/scheduler` (14 эндпоинтов), раздел «🗓 Планировщик» в Streamlit (задачи с
паузой, возобновлением, запуском вне расписания и удалением, напоминания, сводки
с метриками, история запусков) и непрочитанные уведомления планировщика в
основной области чата (фрагмент с автообновлением раз в 5 с). Нового процесса
день не заводит: фон живёт в процессе бэкенда; отдельным процессом по-прежнему
запускается только собственный MCP-сервер — по stdio, на время вызова
инструментов.

Унаследовано полностью: свой MCP-сервер дня 17 с тремя инструментами чтения,
вызов инструмента агентом и правила допуска (`mcp_*`), контролируемые переходы
(день 15) и инварианты (день 14), состояние задачи как конечный автомат
(день 13), персонализация профилем (день 12), три слоя памяти и четыре стратегии
контекста (день 11). MCP-подключение, как и раньше, таблиц в БД не имеет:
соединение живёт в памяти процесса (`MCPRegistry`), а вызовы читающих
инструментов идут **наружу** (HTTP-запрос к jsonplaceholder).

Планировщик — первая подсистема дня, которая добавляет таблицы: **шесть новых**
(`scheduled_tasks`, `task_runs`, `reminders`, `notifications`, `collected_data`,
`periodic_summaries`). Источник правды о задачах — `scheduled_tasks`: APScheduler
держит задачи в памяти и сверяется с таблицей раз в 5 с
(`SCHEDULER_SYNC_SECONDS`), поэтому задача, созданная другим процессом
(MCP-сервером через `POST /scheduler/tasks`), подхватывается сама.

## Раскладка

```
day18/
├── app.py                    # точка входа Streamlit (60 строк): set_page_config + вызовы секций
├── mcp_server/               # СОБСТВЕННЫЙ MCP-сервер дня (транспорт stdio, 6 модулей)
│   ├── __init__.py           # докстринг пакета: как запускается (`uv run python mcp_server/server.py`) и чем говорит
│   ├── config.py             # адрес jsonplaceholder, адрес бэкенда дня (DAY18_BACKEND_URL), границы user_id/limit, имя, версия и инструкция сервера
│   ├── schemas.py            # TypedDict-ответы шести инструментов (UserInfo, PostInfo, PostSummary, UserPosts, ReminderScheduled, CollectionStarted, SummaryReady) → outputSchema
...
│   └── server.py             # MCPServer, шесть инструментов (@server.tool), _run/_scheduled, parse_args (--api-base, --timeout, --backend-url), main
├── frontend/                 # Streamlit UI по секциям (18 модулей + __init__.py)
├── backend/                  # FastAPI-бэкенд, домен и доступ к данным
│   ├── __init__.py           # описание пакета + корень репозитория в sys.path (для shared/)
│   ├── api/                  # FastAPI: 8 роутеров по доменам + сборка app (main.py) и lifespan.py, 73 эндпоинта
│   ├── core/                 # config, dependencies — настройки и доступ к менеджеру, MCP-реестру и планировщику
│   ├── domain/               # чистые правила и данные: 31 модуль — FSM задачи/сжатия/MCP/планировщика, графы переходов, расписания и их арифметика, агрегация, распознавание реплик, стратегии, профиль, инварианты, тексты промпта
│   ├── services/             # прикладные сервисы: compressor, task_state, invariant_checker, mcp_loop, mcp_transport, mcp_client, mcp_errors, mcp_registry, mcp_tool_runner, scheduler, schedule_service, scheduled_jobs, apscheduler_bridge, source_fetch
│   ├── storage/              # доступ к БД: database, task_store, invariant_store, memory_rows, scheduler_store, scheduler_data_store, scheduler_rows
│   ├── agents/               # Agent, MemoryManager, ProfileStore, AgentManager + миксины
│   ├── models/               # ORM-таблицы SQLAlchemy по доменам (включая scheduler.py — шесть таблиц планировщика)
│   ├── schemas/              # Pydantic-схемы API по доменам (включая mcp.py и scheduler.py)
...
Отчёты прогонов лежат в `docs/reports/`: `scheduler_demo.md` — отчёт дня 18 (его
...
|---|---|---|
| `core/` | `config.py` (225), `dependencies.py` (89), `__init__.py` (22) | Настройки дня (разделы «MCP (день 17)» и «Планировщик фоновых задач (день 18)») и зависимости роутов: `get_manager`, `get_mcp_registry`, `get_scheduler`, `get_schedule_service`, `agent_or_404`, `task_or_404`, `invariant_or_404` |
| `domain/` | 31 модуль + `__init__.py` (397) | Чистые правила и данные: FSM задачи, сжатия, MCP-подключения, вызова инструмента и планировщика (задача и напоминание), графы допуска и guards переходов, правила допуска вызова и его код причины, распознавание запроса и реплик планировщика, расписания и их арифметика, агрегация накопленных записей, блоки данных для промпта, каталог серверов, стратегии, факты, слои памяти, профиль, инварианты, тексты промпта, разбор цели MCP и структура инструмента. Знают только stdlib, `core.config` и соседей по слою |
| `storage/` | `database.py` (56), `task_store.py` (278), `invariant_store.py` (255), `memory_rows.py` (47), `scheduler_store.py` (196), `scheduler_data_store.py` (245), `scheduler_rows.py` (177), `__init__.py` (128) | Движок и сессии, ORM-строки → словари, `TaskStateStore` (включая журнал отклонённых попыток), `InvariantManager`, `SchedulerStore` (задачи и журнал запусков), `SchedulerDataStore` (напоминания, накопленные записи, сводки, уведомления) |
| `services/` | `compressor.py` (329), `task_state.py` (392), `invariant_checker.py` (296), `mcp_client.py` (392), `mcp_transport.py` (65), `mcp_errors.py` (95), `mcp_loop.py` (96), `mcp_registry.py` (138), `mcp_tool_runner.py` (144), `scheduler.py` (369), `schedule_service.py` (305), `scheduled_jobs.py` (176), `apscheduler_bridge.py` (161), `source_fetch.py` (61), `__init__.py` (133) | `ContextCompressor`, `TaskStateMachine` (граф, guards, единая точка отказа), `InvariantChecker`, `MCPClient` (соединение и вызов инструмента), `MCPEventLoop` (цикл событий в потоке), `MCPRegistry` (одно подключение на процесс), `MCPToolRunner` (допуск → вызов → исход), `TaskScheduler` (сверка БД ↔ APScheduler, тики), `ScheduleService` (операции инструментов), `scheduled_jobs` (что делает тик), `apscheduler_bridge` (триггеры и классы библиотеки), `source_fetch` (единственный выход фона в сеть): оркестрация домена, хранилища и внешних процессов |
| `agents/` | `agent.py` (1925 ⚠️), `memory.py` (291), `profile_store.py` (292), `agent_manager.py` (92), `manager_*.py` (7 файлов), `__init__.py` (57) | `Agent` (включая шаги MCP и планировщика в `apply_mcp_tool`), `MemoryManager`, `ProfileStore`, `AgentManager` из миксинов; менеджер передаёт реестр MCP созданным и восстановленным агентам |
| `models/` | `agent.py` (93), `message.py` (41), `context.py` (157), `memory.py` (81), `user_profile.py` (57), `task_state.py` (119), `invariant.py` (51), `scheduler.py` (211), `__init__.py` (65) | ORM-таблицы SQLAlchemy: `agents`, `short_term_messages`, `summaries`/`token_usage`/`facts`/`checkpoints`, `working_memory`/`long_term_memory`, `user_profiles`, `task_states`/`task_transitions`, `invariants` и шесть таблиц планировщика — `scheduled_tasks`/`task_runs`/`reminders`/`notifications`/`collected_data`/`periodic_summaries` |
| `schemas/` | `agent.py` (342), `context.py` (218), `memory.py` (173), `profile.py` (170), `task.py` (170), `invariant.py` (149), `mcp.py` (193), `scheduler.py` (313), `__init__.py` (227) | Pydantic-схемы API по доменам, реэкспорт из `schemas/__init__.py` |
| `api/` | `agents.py` (292), `context.py` (140), `memory.py` (180), `profiles.py` (126), `tasks.py` (246), `invariants.py` (139), `mcp.py` (180), `scheduler.py` (355), `lifespan.py` (49), `main.py` (69), `__init__.py` (37) | 73 эндпоинта по доменам, старт и остановка фоновых служб (`lifespan.py`) и сборка `app` |
...
У `domain/__init__.py` список имён остался прежним (день 17): семь модулей
планировщика и их шесть десятков публичных имён импортируются **как модули**
(`from . import aggregation, schedule_spec, …`), иначе общий список имён слоя
вышел бы за лимит 400 строк. Код дня и так берёт имена из своего модуля
(`from ..domain.schedule_spec import tool_specs`), поэтому точки входа
`backend.domain.scheduler_fsm` и соседей достаточно. По той же причине в
...
| `frontend/chat_section.py` | 321 | Шестой раздел «🗓 Планировщик», уведомления в основной области, строка «🗓 Планировщик: …» в сводке хода |
...
| Модуль | Строк | Назначение |
|---|---|---|
| `frontend/__init__.py` | 30 | Описание пакета (включая `scheduler_api`, `scheduler_section`, `notifications`); модули не выполняют `st.*` на импорте |
| `frontend/api_client.py` | 388 | HTTP-транспорт к бэкенду (`requests`), `BACKEND_URL` / `DAY18_BACKEND_URL`, `BackendError`, `request_json`, функции дней 13–17 |
...
| `frontend/mcp_section.py` | 276 | Раздел «🔌 MCP»: цель (по умолчанию — свой сервер дня с шестью инструментами), транспорт, кнопки подключения/отключения, статус, таблица инструментов, разбор `input_schema` и `output_schema`, вызовы блоков вызова инструмента и «Спросить агента» |
...
| `backend/api/main.py` | 69 | — | Сборка `app`: CORS, `include_router` (lifespan — в `lifespan.py`) |

Всего 73 эндпоинта (45 унаследованных дней 1–13 + 6 инвариантов дня 14 +
...
`GET /mcp/tools`, `POST /mcp/call`, `GET /mcp/servers` + 14 планировщика дня 18:
задачи (`GET`/`POST /scheduler/tasks`, `DELETE`, `pause`, `resume`, `run`,
`history`), `GET /scheduler/tools`, `GET /scheduler/status`,
`GET /scheduler/reminders`, `GET /scheduler/collected`, `GET /scheduler/summaries`,
...
`backend/api/main.py`. Доступ к менеджеру, реестру MCP и планировщику, а также
404/409 — `backend/core/dependencies.py` (`get_manager`, `get_mcp_registry`,
`get_scheduler`, `get_schedule_service`, `agent_or_404`, `task_or_404`,
`invariant_or_404`).
...
таблиц планировщика — **18 таблиц** на день; `scheduled_tasks` связана каскадом с
`task_runs`, а `reminders`, `notifications` и `periodic_summaries` ссылаются на
неё по `ondelete="SET NULL"` (удаление задачи не уносит историю).

## Тесты

`tests/` — 1374 теста `pytest`, офлайн (временная SQLite + фейк клиента
DeepSeek; MCP — свой stdio-сервер, фейк или локальный HTTP-стенд; планировщик —
фейковый источник данных и планировщик без таймеров), разложены по
...
`tests/support.py`, `tests/mcp_fakes.py`, `tests/scheduler_fakes.py` и
`tests/backend_stub.py` остаются в корне `tests/`: на них
...
|---|---|---|---|
| `backend/agents/agent.py` | 1925 | 400 | Домен `Agent` (память, стратегии, токены, профиль, состояние задачи и переходы, инварианты, шаги MCP и планировщика, генерация) не разложен на миксины — расхождение унаследовано от дней 11–17 (в день 14 было 1727, в день 15 — 1825, в день 16 — 1825, в день 17 +73 дал шаг MCP, в день 18 +27 дал шаг планировщика); рефакторинг `Agent` в эти дни не входил, и это единственное превышение лимита 400 в дне |
...
| `backend/domain/__init__.py` без имён планировщика | 397 | 400 | Семь модулей планировщика импортируются как модули (`from . import aggregation, schedule_spec, …`), а не реэкспортируются по именам: иначе список `__all__` слоя вышел бы за лимит 400 строк. Публичный контракт доступен как `backend.domain.scheduler_fsm.ScheduledTaskFSM` и т. п.; код дня и так берёт имена из своего модуля |

Лимиты `app.py` (60 ≤ 100) и `backend/api/main.py` (69 ≤ 80) соблюдены;
`frontend/common.py` — ровно 400 строк, тоже в пределах лимита. Единственное
превышение 400 строк в дне — унаследованный `backend/agents/agent.py` (1925).

## Проверка лимита строк

Из папки `day18` (исключены `.venv` и вендорный `.agents/`):
...
Ожидаемый вывод сегодня: `[('backend\\agents\\agent.py', 1925)]`.
