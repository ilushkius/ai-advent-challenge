"""Раздел «💬 Чат и память» дня 18: карточка агента, панели, диалог и ввод.
...
задачи, инварианты, MCP, планировщик), карточка активного агента, панели стратегии и
слоёв памяти, диалог, сравнение режимов, блок предупреждения/отказа по инвариантам
последнего хода, форма ввода и очистка истории.

Сводка хода показывает и шаг планировщика: если реплика поставила фоновую задачу
(``record["schedule"]``), рядом с токенами появляется строка «🗓 Планировщик: …» —
иначе заметить, что напоминание уже стоит, было бы негде.
"""
import streamlit as st

from . import (
    api_client, common, context_panels, invariant_panel, mcp_section,
    memory_panels, notifications, profile_section, scheduler_section, task_panel,
)
...
def render_main_area() -> None:
    ...
    if not st.session_state["backend_ok"]:
        st.warning("🔌 Бэкенд недоступен. Запустите его из папки day18/:")
