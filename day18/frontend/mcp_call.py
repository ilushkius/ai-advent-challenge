"""Прямой вызов MCP-инструмента и каталог серверов (день 17).
...
делает агент сам, но вручную и с видимым результатом. Здесь же каталог известных
серверов (``GET /mcp/servers``) с кнопкой подключения к любому из них.
...
def render_servers(payload: dict) -> None:
    """Каталог известных MCP-серверов и кнопка подключения к выбранному.

    Список открытых соединений не показывается: у процесса оно одно, поэтому
    каталог отвечает «что ещё можно подключить», а подключённый сервер в таблице
    просто отмечен.
    """
    servers = payload.get("servers") or []
    if not servers:
        return
    st.subheader("🗂 Каталог MCP-серверов")
    st.caption(
        "Известные цели подключения: свой сервер дня и серверы официального "
        "набора. Число инструментов показывается у подключённого — у остальных "
        "оно станет известно после подключения."
    )
    st.dataframe(
        [{
            "сервер": item.get("label", ""),
    ...
    others = [item for item in servers if not item.get("connected")]
    if not others:
        return
    targets = {item.get("label", ""): item.get("target", "") for item in others}
    chosen = st.selectbox("Подключиться к серверу из каталога", list(targets),
