"""Общие помощники интерфейса дня 17 (наследовано из дня 12): подписи, форматтеры и состояние страницы.
...
# MCP (день 16): состояния подключения FSM → подпись и значок для UI. Копии
# значений домена: frontend не импортирует backend, только HTTP.
MCP_STATE_LABELS = {
...
# Транспорт MCP → подпись для селектора подключения.
MCP_TRANSPORT_LABELS = {
    "auto": "auto — по виду цели",
    "stdio": "stdio — дочерний процесс",
    "sse": "SSE — HTTP (sse://…)",
    "http": "Streamable HTTP (http://…)",
}

# Состояния вызова MCP-инструмента (день 17) → подпись для UI. Копия значений
# домена (backend/domain/mcp_tool_call.MCPToolCallState): frontend не импортирует
# backend, только HTTP.
MCP_CALL_STATE_LABELS = {
    "idle": "инструмент не вызывался",
    "planned": "инструмент выбран",
...
# Причины отказа/сбоя вызова (коды контракта API) → подпись для UI.
MCP_REASON_LABELS = {
    "not_connected": "нет соединения",
    "unknown_tool": "инструмент не найден",
    "bad_arguments": "аргументы не подходят",
    "transport": "сбой связи",
    "tool_error": "инструмент вернул ошибку",
}


# Планировщик (день 18): состояния задачи, напоминания, запуска и уведомлений →
# подписи для UI. Копии значений домена (backend/domain/scheduler_values.py):
# frontend не импортирует backend, только HTTP. Словарей типа расписания и
# причины отказа здесь нет: бэкенд отдаёт их готовым текстом.
...
#: Инструменты планировщика → подписи (в отчётах хода и таблицах).
SCHEDULER_TOOL_LABELS = {
    "schedule_reminder": "⏰ Напоминание",
    "collect_data": "📥 Периодический сбор данных",
    "generate_summary": "📊 Регулярная сводка",
}


def schedule_note(schedule: dict) -> str:
    """Строка «🗓 Планировщик: …» для сводки хода (``""`` — фоновой задачи нет)."""
    if not schedule or not schedule.get("registered"):
        return ""
    tool = SCHEDULER_TOOL_LABELS.get(schedule.get("tool"), schedule.get("tool") or "")
    message = schedule.get("message") or "фоновая задача зарегистрирована"
    return f"🗓 Планировщик: {tool} — {message}"


def schedule_row(task: dict) -> dict:
    """Строка таблицы задач планировщика (подписи интерфейса, а не значения API)."""
    return {
        "№": task.get("id"),
        "имя": task.get("name"),
        "расписание": task.get("schedule_label") or task.get("schedule_type"),
        "состояние": SCHEDULE_STATE_LABELS.get(task.get("status"), task.get("status")),
        "последний запуск": fmt_time(task.get("last_run_at")),
        "следующий запуск": fmt_time(task.get("next_run_at")),
    }
...
def init_state() -> None:
    ...
    # Цель последнего успешного MCP-подключения (подставляется в поле ввода).
    if "mcp_last_target" not in st.session_state:
        st.session_state["mcp_last_target"] = None
    # Результат последнего прямого вызова инструмента (переживает st.rerun).
    if "mcp_last_call" not in st.session_state:
        st.session_state["mcp_last_call"] = None
    # Ответ агента на последний вопрос из раздела «🔌 MCP» (там же — отчёт mcp).
    if "mcp_last_ask" not in st.session_state:
        st.session_state["mcp_last_ask"] = None
