# API дня 18 — агенты DeepSeek с MCP-инструментами, планировщиком фоновых задач, контролируемыми переходами, инвариантами, состоянием задачи, памятью и профилем

Бэкенд — FastAPI-приложение `day18/backend/api/main.py`. Заголовок приложения —
«Агенты DeepSeek + планировщик задач — День 18», версия схемы — `12.0.0`
...
живёт в `day18/mcp_server/` (stdio, шесть инструментов: `get_user`, `get_post`,
`list_user_posts` читают jsonplaceholder.typicode.com, а `schedule_reminder`,
`collect_data` и `generate_summary` ставят фоновые задачи через API дня), а агент
...
Схемы ответов описаны в пакете `day18/backend/schemas/` — по доменам: `agent.py`
...
Всего 73 эндпоинта: 11 в разделе агентов (десять эндпоинтов CRUD, генерации и
...
инвариантов, MCP и планировщика, а также перечень эндпоинтов (72 строки — все,
кроме самого `GET /`).
...
```json
{
  "name": "Агенты DeepSeek с контролируемыми переходами, MCP и планировщиком — День 18",
  "docs": "/docs",
  ...
  "mcp": "/mcp/connect, /mcp/disconnect, /mcp/status, /mcp/tools, /mcp/call, /mcp/servers (6 эндпоинтов)",
...
Свой сервер дня живёт в `day18/mcp_server/` и запускается по stdio: клиент
...
`mcp_server/api_client.py`. Днём 18 у сервера стало **шесть** инструментов: три
прежних читают jsonplaceholder (`get_user`, `get_post`, `list_user_posts`), а три
планировщика (`schedule_reminder`, `collect_data`, `generate_summary`) ставят
фоновые задачи — их тела вызывают `POST /scheduler/tasks` бэкенда дня
(`mcp_server/backend_api.py`), потому что единственный писатель в SQLite — процесс
бэкенда, и фон живёт там же. Каталог инструментов, вызов и правила допуска
...
запроса (для задач планировщика — блок «Данные планировщика»).
...
Каталог дня — шесть инструментов: `get_user`, `get_post`, `list_user_posts`
(чтение jsonplaceholder), `schedule_reminder`, `collect_data`, `generate_summary`
(фоновые задачи). Описания и аргументы трёх последних — в разделе
[«Планировщик»](#планировщик); их вызов возвращает `structuredContent` с полями,
разобранными в [«Формы планировщика»](#формы-планировщика).
...
```json
{ "detail": "Инструмент «nope» не найден в каталоге сервера. Доступны: get_user, get_post, list_user_posts, schedule_reminder, collect_data, generate_summary" }
```
...
{
  "servers": [
    {"key": "day18-jsonplaceholder",
     "label": "День 18: свой MCP-сервер (jsonplaceholder)",
     "target": "uv run python mcp_server/server.py",
     "description": "Собственный сервер дня по stdio: инструменты get_user, get_post и list_user_posts читают https://jsonplaceholder.typicode.com, а schedule_reminder, collect_data и generate_summary планируют фоновую работу через API дня (напоминание, периодический сбор, регулярная сводка).",
     "connected": true,
     "tool_count": 6},
  ...
  "connected_key": "day18-jsonplaceholder"
}
```
...
| `system_prompt` | строка | итоговое system-сообщение запроса (профиль + роль + инварианты + блоки памяти + блок состояния задачи + блок данных MCP + блок данных планировщика; считается до вызова DeepSeek) |
...
| `422` | невалидное тело запроса (Pydantic/FastAPI): невалидные поля профиля, `initial_stage` вне `planning`/`execution`/`validation`, неизвестный этап/шаг, слишком длинные `expected_action`/`reason`, пустой `task_id`, тело `PATCH /tasks/{task_id}/context` без единого флага, пустая цель или неизвестный транспорт у `POST /mcp/connect`, пустое или длиннее 100 символов имя инструмента у `POST /mcp/call`, `schedule_type` вне `date`/`interval`/`cron`, пустой или длиннее 64 символов `tool`, пустое или длиннее 100 символов `name` у `POST /scheduler/tasks` | объект с `detail` — списком ошибок |
...
- **Всё состояние переживает рестарт.** Реплики (`short_term_messages`), рабочая
  ...
  журналом `task_transitions`) и таблицы планировщика (`scheduled_tasks`,
  `task_runs`, `reminders`, `notifications`, `collected_data`,
  `periodic_summaries`) лежат в SQLite (`day18/agents.db`), а активные
  ...
  эндпоинты `/memory/...`, `/tasks/...` и `/scheduler/...` после перезапуска
