"""Движок, сессии и начальная схема БД дня 17 (SQLAlchemy/SQLite).

ORM-таблицы дня описаны в ``backend/models/*.py`` (``agent``, ``message``,
``memory``, ``context``, ``user_profile``, ``task_state``, ``invariant``,
``scheduler``) и реэкспортируются отсюда (``Base``, ``AgentRecord``,
...
# Движок и фабрика сессий приложения (база day18/agents.db, см. config).
