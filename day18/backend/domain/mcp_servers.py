...
(``day18/mcp_server``) и два сервера официального набора для сравнения. Каталог
...
SERVER_KEY_DAY18 = "day18-jsonplaceholder"
...
KNOWN_SERVERS: tuple[MCPServerOption, ...] = (
    MCPServerOption(
        ...
        description=(
            ...
            "(пользователи, посты и посты пользователя), а schedule_reminder, "
            "collect_data и generate_summary планируют фоновую работу через API "
            "дня (напоминание, периодический сбор, регулярная сводка)."
        ),
    ),
    MCPServerOption(
