"""Доменный слой дня 18: чистые правила и данные, без БД, LLM и транспорта.
...
- ``scheduler_values`` — значения планировщика: типы расписаний, состояния задачи
  и напоминания, статусы запуска и коды причин отказа (день 18);
- ``scheduler_fsm`` — стейт-машины задачи планировщика и напоминания (день 18);
- ``schedule_spec`` — инструменты планировщика, их аргументы и расписание
  «по инструменту» (день 18);
...
- ``aggregation`` — агрегация накопленных записей в сводку (числа, уникальные
  значения, текст сводки);
...
# Планировщик дня 18 импортируется МОДУЛЯМИ, а не именами: у него семь
# модулей и шесть десятков публичных имён, и общий список имён слоя вышел бы
# за лимит 400 строк. Код дня и так берёт имена из своего модуля
# (``from ..domain.schedule_spec import tool_specs``), поэтому здесь
# достаточно точки входа: ``backend.domain.scheduler_fsm.ScheduledTaskFSM``.
from . import (
    aggregation,
    schedule_intent,
    schedule_spec,
