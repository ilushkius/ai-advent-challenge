...
1. файл `.env` в папке дня (day18/backend/core/config.py -> day18/.env);
...
# Персистентное хранилище дня: SQLite-файл в папке дня (day18/agents.db).
...
# Путь к .env дня: этот файл лежит в day18/backend/core/, значит .env — в day18/.
...
def resolve_api_key():
    """Возвращает ключ DeepSeek: day18/.env → переменная окружения → None."""
