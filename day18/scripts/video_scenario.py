"""Автопроверка кадров сценария видео дня 17 (``docs/usage.md`` §8).
...
Что НЕ трогается.
    База приложения ``day17/agents.db``: прогон поднимает uvicorn на СВОЕЙ
...
def run() -> None:
    ...
    try:
        ...
        # не нужен вовсе.
        os.environ["DAY18_BACKEND_URL"] = client.base_url
