# День 18 — планировщик фоновых задач: отложенные и периодические MCP-инструменты (+ MCP, память, профиль, задача, инварианты и переходы)

Задание дня — **дать агенту инструменты с отложенным и периодическим
выполнением**. К прежним трём инструментам своего MCP-сервера добавились
`schedule_reminder` (разовое напоминание через N секунд), `collect_data`
(периодический сбор данных из внешнего API) и `generate_summary` (регулярная
сводка по накопленным данным) — всего в каталоге `day18/mcp_server/server.py`
теперь шесть инструментов. Фон живёт в процессе бэкенда: `TaskScheduler` поверх
`apscheduler.schedulers.asyncio.AsyncIOScheduler` запускается в `lifespan`,
останавливается при завершении приложения и обслуживает задачи из таблицы
`scheduled_tasks`. Метаданные задач, журнал запусков и накопленные данные лежат в
SQLite (`scheduled_tasks`, `task_runs`, `reminders`, `notifications`,
`collected_data`, `periodic_summaries`), поэтому задачи переживают перезапуск:
при старте планировщик сверяет БД с таймерами и догоняет пропущенный запуск.
Агент ставит фоновую задачу сам — по реплике («напомни мне через 30 секунд
проверить почту», «собирай данные с …/posts каждые 10 секунд», «покажи сводку за
последний час»), а что именно запланировано, видно в поле `schedule` ответа
генерации. В интерфейсе появились раздел «🗓 Планировщик» и уведомления
планировщика в области чата.

Живой прогон дня —
[docs/reports/scheduler_demo.md](docs/reports/scheduler_demo.md): четыре сценария
задания (напоминание через 30 секунд, сбор каждые 10 секунд, сводка каждые
20 секунд, восстановление после перезапуска), таблица трёх инструментов с
фактическими значениями из БД и схема шести таблиц планировщика.

Структура дня 17 сохранена полностью: MCP-клиент и свой MCP-сервер, инварианты
агента, состояние задачи как конечный автомат с **контролируемыми переходами**,
персонализация профилем пользователя, три слоя памяти, четыре стратегии контекста
и сжатие истории в конспект. Планировщик надстроен над ними и использует общий
код-путь: тела трёх новых инструментов — один HTTP-вызов `POST /scheduler/tasks`,
поэтому и MCP-инструмент, и ручное создание задачи, и форма интерфейса идут через
`ScheduleService.create_task`. Приложение развивает день 17, а `day17/`,
`day16/`, `day15/` и более ранние дни остаются снимками.

Единственный писатель в SQLite — процесс бэкенда: MCP-сервер сам в БД не ходит, он
вызывает API дня. Исключение — таблиц у MCP-подсистемы по-прежнему нет:
соединение живёт в памяти процесса, и после перезапуска бэкенда `/mcp/status`
честно отвечает `disconnected`. Всё остальное состояние (память, профили, задачи,
инварианты, задачи планировщика с их журналами) — в SQLite и переживает рестарт.

Код дней 1–17 не изменялся — их файлы не тронуты; изменения только внутри
`day18/` плюс общий пакет `shared/` в корне репозитория, куда вынесен код, не
...
| Четырнадцать эндпоинтов `/scheduler/...` (всего в API дня 73) и общий код-путь `POST /scheduler/tasks` | `backend/api/scheduler.py`, `backend/schemas/scheduler.py` |
...
(`day18/mcp_server/`, транспорт **stdio**), клиент умеет вызывать его инструменты
(`tools/call`), а агент делает это сам по реплике пользователя. В дне 18 сервер
публикует **шесть** инструментов: три прежних читают jsonplaceholder, а три
новых — `schedule_reminder`, `collect_data` и `generate_summary` — ставят
фоновые задачи через API дня (разбор — в разделе
[«Фоновые задачи (планировщик)»](#фоновые-задачи-планировщик)).

### Свой сервер

| Файл | Что делает |
|---|---|
| `mcp_server/config.py` | адрес внешнего API и бэкенда дня, таймаут, границы `limit` и id, имя/версия сервера, инструкция для модели |
| `mcp_server/schemas.py` | `TypedDict`-структуры ответов (`UserInfo`, `PostInfo`, `PostSummary`, `UserPosts`, `ReminderScheduled`, `CollectionStarted`, `SummaryReady`) — из них SDK собирает `outputSchema` |
...
| `mcp_server/server.py` | регистрация инструментов `@server.tool()`, перевод ошибки внешнего API и бэкенда в `ToolError`, разбор аргументов (`--api-base`, `--backend-url`, `--timeout`), запуск по stdio |
...
* **«🤖 Спросить агента»** — тот же вопрос, но через агента: он сам вызывает
  ...
  10 секунд», «Покажи сводку за последний час»), а рядом со сводкой хода
  появляется строка «🗓 Планировщик: …».
...
curl http://127.0.0.1:8000/mcp/tools          # {"tools": [...], "count": 6, ...} — три читают jsonplaceholder, три ставят фоновые задачи
...
Сценария дня 11 в `day18/` нет — вместе с ним ушли кнопка его запуска, панель
...
В `day18/` диалог ведётся вручную в чате, а доказательства дают
...
Одна строка на задачу в той же базе `day18/agents.db`. `task_id` уникален
...
```bash
cd day18
uv run python scripts/task_state_demo.py --all           # нужен DEEPSEEK_API_KEY в day18/.env
...
Скрипт работает на отдельной базе `day18/task_state_demo.db`, заводит агента
...
```bash
cd day18
uv run python scripts/seed_invariants.py            # 4 демо-правила в day18/agents.db
...
Профиль — одна строка на пользователя в той же базе `day18/agents.db`:
...
```bash
cd day18
uv run python scripts/personalization_comparison.py            # нужен DEEPSEEK_API_KEY в day18/.env
uv run python scripts/personalization_comparison.py --no-api   # офлайн-заглушка, без сети
```

Скрипт пишет `day18/docs/reports/personalization_comparison.md` и работает на отдельной базе
`day18/personalization_demo.db` (пересоздаётся при каждом прогоне).
...
«📏 Инварианты», «🔌 MCP» и «🗓 Планировщик» (ключ `main_section`) — именно radio,
а не `st.tabs`: Streamlit не сохраняет выбор вкладки между перезапусками скрипта,
а после «Сохранить профиль» нужен `st.rerun`.
...
* «💬 Чат и память»: выбор агента, карточка агента (модель, температура,
  ...
  автообновлением раз в 5 секунд и кнопкой «✔ Прочитано» на каждом).
...
* Переменная окружения адреса бэкенда: `DAY18_BACKEND_URL` (по умолчанию
...
* Проверки наследованных возможностей описаны в самом README и в
  [`STRUCTURE.md`](STRUCTURE.md); планировщик проверяется отчётом
  [`docs/reports/scheduler_demo.md`](docs/reports/scheduler_demo.md),
...
Streamlit (порт 8501) ── HTTP (requests) ──► FastAPI (порт 8000) ──► DeepSeek API
   ...
   «🗓 Планировщик»
                                    ...
                                    SQLAlchemy → SQLite (day18/agents.db)
...
| Слой | Папка | Зачем |
|---|---|---|
| Конфигурация и зависимости | `backend/core/` | Настройки дня (лимиты, цены, пути `.env`/`agents.db`, блок настроек планировщика) и доступ роутов к менеджеру агентов, реестру MCP и планировщику |
| Домен | `backend/domain/` | Чистые правила и данные без БД, LLM и HTTP: FSM задачи, сжатия, **MCP-подключения и MCP-вызова**, **планировщика (состояние задачи и напоминания)**, граф допуска и guards переходов, стратегии, факты, слои памяти, профиль, тексты промпта, разбор цели MCP, структура инструмента, распознавание запроса к инструменту и каталог серверов, спецификации трёх инструментов планировщика, формы расписания, агрегация сводок и распознавание реплик о фоновых задачах |
| Доступ к данным | `backend/storage/` | Движок и сессии SQLite, реэкспорт ORM, `TaskStateStore`, `SchedulerStore` и `SchedulerDataStore`, преобразования ORM-строк в словари |
| Прикладные сервисы | `backend/services/` | Оркестрация домена и хранилища: суммаризация (`ContextCompressor`), переходы задачи (`TaskStateMachine`), **MCP** (`MCPClient` с `list_tools`/`call_tool`, `MCPRegistry`, `MCPToolRunner`, `MCPEventLoop`, `mcp_transport`, `mcp_errors`) и **планировщик** (`TaskScheduler`, `ScheduleService`, действия инструментов, HTTP-источник, мост к APScheduler) |
| Агенты | `backend/agents/` | `Agent`, `MemoryManager`, `ProfileStore`, `AgentManager` и его миксины |
| ORM-таблицы | `backend/models/` | SQLAlchemy-модели по доменам (агент, память, контекст, профиль, задача, планировщик) |
| Схемы API | `backend/schemas/` | Pydantic-модели запросов/ответов по доменам (включая `scheduler`) |
| HTTP | `backend/api/` | FastAPI-роутеры по доменам, `lifespan.py` (таблицы → агенты → планировщик) и сборка `app` (`backend.api.main:app`) |
| Утилиты | `backend/utils/` | Слой объявлен обязательным, но в дне 18 пуст: общий код живёт в repo-level `shared/` |
| Интерфейс | `frontend/` | Streamlit по секциям: транспорт, подписи, панели, разделы (включая «🔌 MCP» и «🗓 Планировщик») и уведомления области чата |
...
```
day18/
├── app.py               # Streamlit, точка входа (60 строк): set_page_config →
│                        # common.init_state() → sidebar.render_sidebar() → chat_section.render_main_area()
├── frontend/            # интерфейс по секциям (18 модулей, карта — в STRUCTURE.md)
│   ├── api_client.py    # HTTP-транспорт бэкенда (requests): BACKEND_URL (DAY18_BACKEND_URL), BackendError, request_json
...
│   ├── common.py        # подписи (TASK_STAGE_LABELS, MCP_*, SCHEDULE_*), schedule_note, run_task_action, st.session_state
...
│   ├── chat_section.py  # раздел «💬 Чат и память» и переключатель шести разделов
...
├── mcp_server/          # СВОЙ MCP-сервер дня 18 (транспорт stdio, JSON-RPC по stdin/stdout)
│   ├── config.py        # адрес jsonplaceholder, таймаут, границы limit/id, адрес бэкенда дня, имя, версия и инструкция сервера
│   ├── schemas.py       # TypedDict-структуры ответов (UserInfo, …, ReminderScheduled, CollectionStarted, SummaryReady) → outputSchema
...
│   └── server.py        # MCPServer + шесть инструментов (@server.tool()), ToolError, --api-base/--backend-url/--timeout, server.run("stdio")
├── scripts/             # прогоны демонстраций и сборка отчётов (не пакет, находят корень дня сами)
│   ├── scheduler_demo.py # день 18: оркестрация четырёх сценариев планировщика и сборка отчёта (--report/--tests/--source-url/--keep-db)
...
├── backend/
│   ├── api/             # FastAPI: 8 роутеров по доменам + main.py (сборка app, 69 строк) + lifespan.py
│   ├── core/            # config.py (настройки, пути .env и agents.db, цели MCP, блок планировщика), dependencies.py (get_manager, get_mcp_registry, get_scheduler, get_schedule_service, *_or_404)
...
│   ├── agents/          # agent.py (в т.ч. шаги apply_mcp_tool и планировщика), memory.py, profile_store.py, agent_manager.py, manager_*.py (7 миксинов)
...
│   │                    # notifications, collected_data, periodic_summaries (реэкспорт — через storage/database.py)
│   ├── schemas/         # Pydantic-схемы API по доменам: agent, context, invariant, mcp, memory, profile, scheduler, task
│   └── utils/           # своего кода нет: общий живёт в repo-level shared/
├── tests/               # pytest: 1374 теста в подпапках unit/ (25 файлов), integration/ (28), e2e/ (10)
...
├── docs/
│   ├── architecture.md  # компоненты и модульная структура, схема БД, слои памяти, персонализация, FSM, переходы, инварианты, MCP-сервер и инструменты, планировщик
│   ├── api.md           # 73 эндпоинта с примерами и кодами ошибок
│   ├── usage.md         # инструкция дня 18: установка и запуск, три инструмента планировщика, напоминания, сбор данных, сводки, пауза/возобновление, сценарии проверки
│   └── reports/         # scheduler_demo.md (день 18), mcp_tool_demo.md (день 17), mcp_demo.md (день 16) + унаследованные controlled_transitions_demo.md, task_state_demo.md, personalization_comparison.md
...
├── conftest.py          # добавляет корень day18 в sys.path (импорт `from backend.agents.agent import Agent`)
...
├── agents.db            # SQLite: агенты, слои памяти, профили, задачи, инварианты, задачи планировщика (в .gitignore по *.db)
...
└── .env.example         # шаблон ключа DEEPSEEK_API_KEY и адреса бэкенда DAY18_BACKEND_URL
```
...
В `day18/.env` нужен ключ `DEEPSEEK_API_KEY=sk-...` (для шага агента и генерации);
адрес бэкенда для интерфейса и MCP-сервера задаёт `DAY18_BACKEND_URL` (по
...
```bash
uv run python mcp_server/server.py                       # --help: --api-base, --backend-url и --timeout
...
```bash
uv run pytest -q      # 1374 теста, офлайн (ключа и сети не требуют)
```

Инструкция дня 18 (установка и запуск, три инструмента планировщика, напоминания,
сбор данных, сводки, пауза/возобновление, сценарии проверки) — в
...
В этой рабочей копии так и получилось: `day18/.agents/skills/` содержит
...
Скиллы видны агенту omp, если сессия запущена **из папки `day18`** (провайдер
...
(`skills.customDirectories: day18/.agents/skills`) — там же он ищет только на
...
```bash
cd day18
uv run pytest -q      # 1374 теста
```

1374 теста = унаследованные из дня 17 (MCP-клиент и вызовы инструментов,
контролируемые переходы, инварианты, состояние задачи, персонализация, память,
сжатие) плюс **152** случая на планировщик в новых файлах. Файлов: `unit/` — 25,
`integration/` — 28, `e2e/` — 10.

Новые файлы дня 18 (планировщик фоновых задач):
...
| `tests/integration/test_mcp_scheduler_tools.py` | 8 | настоящий stdio-сервер дня со стендом API: каталог из шести инструментов, `structuredContent` трёх инструментов планировщика, ошибка бэкенда приходит как `is_error` с текстом причины, а не обрывом связи |
...
| `tests/integration/test_mcp_server_stdio.py` | 5 | **свой** `mcp_server/server.py` дочерним процессом: каталог из шести инструментов дня, `input_schema`/`output_schema`, успешный вызов и 404 как `is_error` (внешний API подменён стендом) |
...
`make_mcp_factory` собирают сценарии. Фейки планировщика — в
...
* **Три инструмента с отложенным и периодическим выполнением**:
  ...
  накопленным данным) — в каталоге MCP-сервера дня их шесть вместе с тремя
  прежними.
...
* **Распознавание фоновой задачи по реплике**: «напомни мне через 30 секунд
  ...
  за последний час» — правила дня 18 проверяются раньше правил дня 17, а в ответе
...
* **Стенд для съёмки видео**: `uv run python scripts/video_scenario.py --ui`
  ...
  (своя БД, свой порт, `DAY18_BACKEND_URL`), открывает адрес в браузере и
...
- **Один экземпляр приложения на одну БД.** Планировщик живёт в процессе бэкенда:
  второй экземпляр на той же `day18/agents.db` получит собственную копию таймеров
...
- **MCP-подключение не переживает рестарт бэкенда.** Оно живёт в памяти процесса
  ...
  (память, профили, задачи, инварианты, метаданные планировщика) — в SQLite.
...
- **Наследованные скрипты прогона кадров дня 15** (`scripts/video_scenario*.py`)
  ...
  инструкции, которого в `usage.md` дня 18 нет: файл описывает только MCP и
  планировщик.
- Один процесс бэкенда и один файл `day18/agents.db` (в `.gitignore` по `*.db`).
  Прогон отчёта персонализации работает на отдельной базе
  `day18/personalization_demo.db`, демонстрация состояния задачи — на
  `day18/task_state_demo.db`, демонстрация контролируемых переходов — на
  `day18/controlled_transitions_demo.db`, демонстрация MCP-инструментов — во
  ...
  кадров сценария видео — на `day18/video_scenario.db` (плюс временный
