...
Тесты не ходят в сеть и не пишут в рабочую БД `day18/agents.db`: движок создаётся
...
def create_agent(session_factory, agent_id: str, cfg=None, ensure_record: bool = True,
                 ...
                 mcp_registry=None, **overrides) -> Agent:
    ...
    ``mcp_registry`` (день 17) идёт НЕ в конфигурацию, а в конструктор Agent: это
    реестр MCP-подключения, которым пользуется шаг MCP.
    """
    if cfg is None:
