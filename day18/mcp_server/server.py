"""Собственный MCP-сервер дня 18: шесть инструментов — чтение данных и планировщик.
...
from mcp_server import backend_api  # noqa: E402
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        prog="server.py",
        description=(
            "MCP-сервер дня 18: инструменты get_user, get_post, list_user_posts "
            "поверх jsonplaceholder.typicode.com и инструменты планировщика "
            "schedule_reminder, collect_data, generate_summary через API дня."
        ),
        epilog=(
...
def main(argv: list[str] | None = None) -> int:
    """Настраивает клиентов внешнего API и бэкенда дня и запускает сервер по stdio."""
