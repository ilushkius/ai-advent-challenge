...
В дне 11 стратегия определяет сборку **краткосрочного** слоя: сколько последних
...
Сводка метрик по сценарию «собираем ТЗ» — в
[`../memory_layers_comparison.md`](../memory_layers_comparison.md) (по слоям) и
[`../day10/comparison.md`](../day10/comparison.md) (по стратегиям дня 10).
...
| «🎬 Прогнать сценарий дня 11» | Ставит задачу `tz-portal`, пишет 6 записей рабочей и 4 — долговременной памяти, отправляет 12 реплик, завершает сессию (`POST /memory/session`); итог `24 → 0 / 6 → 6 / 4 → 4` и таблица шагов по слоям в панели «📋 Итог сценария дня 11» |
...
Покадровый сценарий дня 11 (три слоя, A/B по долговременной памяти, новая
сессия, офлайн-доказательство) живёт в одной точке — §8 инструкции:
[`usage.md`](usage.md). Там же обе проверки из задания: §6 — какие данные
попадают в каждый слой, §7 — как слои влияют на ответы. Готовые числа для
сверки — в [`../memory_layers_comparison.md`](../memory_layers_comparison.md).
