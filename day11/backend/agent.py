"""Класс Agent дня 11 — агент с тремя слоями памяти и контролем токенов.
...
class Agent:
    ...
    def _make_client(self):
        ...
        if not api_key:
            raise AgentError(
                "Ключ API не задан: укажите DEEPSEEK_API_KEY в файле day11/.env "
    ...
    def _system_message(self, summary_text: str = "",
        ...
        """Системное сообщение агента: роль + блоки памяти/конспекта/фактов.

        Порядок блоков: системный промпт → рабочая память → долговременная
        память → конспект → факты. Все блоки вкладываются в существующее
