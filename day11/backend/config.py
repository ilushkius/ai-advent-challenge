"""Конфигурация бэкенда дня 11: DeepSeek, дефолты агентов, стратегии, слои памяти.

День 11 развивает день 10: у агента появляются три явных слоя памяти
(краткосрочная, рабочая, долговременная). Здесь — их константы:
``DEFAULT_TASK_ID``, ``LONG_TERM_LIMIT`` (сколько релевантных записей
долговременной памяти идёт в контекст) и лимиты валидации записей.
Стратегии управления контекстом остаются из дня 10 (``DEFAULT_STRATEGY``,
``DEFAULT_WINDOW_SIZE``).

Ключ DEEPSEEK_API_KEY ищется в следующем порядке (конвенция проекта):
1. файл `.env` в папке дня (day11/backend/config.py -> day11/.env);
...
# Персистентное хранилище дня 11: SQLite-файл в папке дня (day11/agents.db).
...
# Путь к .env дня 11: этот файл лежит в day11/backend/, значит .env — в day11/.
...
def resolve_api_key():
    """Возвращает ключ DeepSeek: day11/.env → переменная окружения → None."""
