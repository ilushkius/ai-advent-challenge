# API дня 10 — агенты DeepSeek со стратегиями управления контекстом

Бэкенд — FastAPI-приложение `day10/backend/main.py`. Базовый адрес после запуска
(из папки `day10`):
...
Схемы ответов описаны в `day10/backend/models.py`; сводная таблица полей — в
разделе [«Формы данных»](#формы-данных).

## Эндпоинты

Всего 19 эндпоинтов с префиксом `/agents/{agent_id}` плюс корневой `GET /`.

| Метод | Путь | Назначение | Успех |
|---|---|---|---|
| POST | `/agents` | создать агента (конфигурация, сжатие, стратегия) | 201 AgentInfo |
| GET | `/agents` | список агентов с числом сообщений и стратегией | 200 [AgentSummary] |
...
| POST | `/agents/{agent_id}/generate` | ход диалога по текущей стратегии | 200 GenerateResponse |
...
```json
{
  "name": "Агенты DeepSeek со стратегиями контекста — День 10",
  "docs": "/docs",
  "endpoints": [
    "POST /agents",
    "GET /agents",
...
## Стратегии, ветки и факты (день 10)
...
- **Что появилось в дне 10.** Поля `strategy` и `window_size` в `AgentConfig`/
  `AgentInfo`/`AgentSummary`; поле `strategy` в `ContextInfo`; эндпоинты
  `/strategy`, `/strategies`, `/branches`, `/branches/{branch_id}/switch`,
  `/facts`; схемы `StrategiesOut`, `BranchCreateRequest`/`BranchOut`/
  `BranchListOut`, `FactOut`/`FactsOut`. Таблицы `facts` и `checkpoints`.
...
- **Лимиты контекста 8000/32000.** `deepseek-chat` — 8000 токенов,
  ...
  `trimmed_messages`.
- **Метрики переживают рестарт.** Сообщения, конспекты (`summaries`), факты
  (`facts`), ветки (`checkpoints`) и записи `token_usage` лежат в SQLite
  (`day10/agents.db`), при старте приложения
