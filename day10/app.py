"""День 10: Streamlit-чат агентов DeepSeek со стратегиями управления контекстом.

Приложение общается с FastAPI-бэкендом (порт 8000) по HTTP через `requests`.
Ключ DEEPSEEK_API_KEY фронтенду НЕ нужен — его читает бэкенд (day10/.env или
переменная окружения). Диалог, факты, ветки и метрики хранит бэкенд в SQLite
(day10/agents.db), поэтому всё переживает рестарт.

Что нового по сравнению с днём 9: к сжатию истории (стратегия summary)
добавляются три стратегии сборки контекста и переключатель между ними:

- «🪟 Sliding Window» — только последние N реплик + системный промпт;
...
Интерфейс: в боковой панели — выпадающий список стратегии и слайдер
window_size; в основной области — индикация стратегии и активной ветки, панель
фактов (sticky_facts), дерево веток (branching), кнопка «🎬 Запустить тестовый
сценарий» («собираем ТЗ»).

Запуск из папки day10/:  streamlit run app.py  (бэкенд запускается отдельно:
...
BACKEND_URL = os.environ.get("DAY10_BACKEND_URL", "http://127.0.0.1:8000")
...
def label_agent(agent) -> str:
    """Подпись агента: имя · модель · сообщения · стратегия."""
...
st.set_page_config(page_title="Стратегии контекста · День 10",
...
# ================= UI: боковая панель =================
_load_agents()
st.sidebar.title("🧠 Стратегии контекста")
...
# --- стратегия контекста активного агента (день 10) ---
...
st.caption("Агент помнит диалог (SQLite), а контекст для запроса собирает одна "
           "из четырёх стратегий: скользящее окно, липкие факты, ветвление или "
           "сжатие конспектом. Переключатель — в боковой панели.")
...
if not st.session_state["backend_ok"]:
    st.warning("🔌 Бэкенд недоступен. Запустите его из папки day10/:")
    ...
    if not agents:
        st.info("Агентов пока нет — создайте первого в боковой панели. "
                "Его диалог, факты, ветки и метрики сохранятся в day10/agents.db.")
    else:
