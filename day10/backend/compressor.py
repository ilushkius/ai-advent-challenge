...
3. **куда сохранять** — результат пишется строкой в таблицу ``summaries``
   (append-only, текущий конспект = последняя строка). Полная история в
   ``messages`` НЕ удаляется: конспект лишь заменяет старые реплики в запросе.
...
def render_chunk(rows: Sequence[Message]) -> str:
...
class ContextCompressor:
    ...
    def uncovered_rows(self) -> List[Message]:
    ...
    @staticmethod
    def _compress_slice(uncovered: Sequence[Message], count: int) -> List[Message]:
