...
class AgentRecord(Base):
    ...
    # Стратегия управления контекстом (день 10). Дефолты — константы config,
...
class Checkpoint(Base):
    ...
    хода в режиме branching; переключение заменяет ``messages``-историю агента
...
# Движок и фабрика сессий приложения (база day10/agents.db, см. config).
