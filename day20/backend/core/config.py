...
1. файл `.env` в папке дня (day20/backend/core/config.py -> day20/.env);
...
#: поэтому дочерний процесс запускается из day20/, а не из cwd пользователя.
...
API_TITLE = "Агенты DeepSeek + оркестрация MCP-серверов — День 20"
API_DESCRIPTION = (
    "FastAPI-бэкенд веб-приложения дня 20: три слоя памяти в SQLite, профиль "
    ...
    "ДЕКЛАРАТИВНЫЙ ПАЙПЛАЙН и ОРКЕСТРАЦИЯ ФЛОТА из трёх MCP-серверов "
    "(search_server, data_server, storage_server): план шагов строится по "
    "каталогу инструментов (LLM или эвристикой), каждый шаг логируется в SQLite "
    "с сервером, инструментом, входом, выходом и временем. "
    "Эндпоинты: /mcp/... (5), /mcp/servers/... (3), /scheduler/... (14), "
    "/pipelines/... (5), /orchestration/... (6)."
)
API_VERSION = "14.0.0"

# Персистентное хранилище дня: SQLite-файл в папке дня (day20/agents.db).
...
# Путь к .env дня: этот файл лежит в day20/backend/core/, значит .env — в day20/.
...
def resolve_api_key():
    """Возвращает ключ DeepSeek: day20/.env → переменная окружения → None."""
