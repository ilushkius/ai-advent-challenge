"""Доменный слой дня 19: чистые правила и данные, без БД, LLM и транспорта.
...
- ``mcp_tools`` — инструмент MCP и результат его вызова (``MCPToolResult``);
- ``mcp_tool_call`` — допуск вызова и его FSM (``MCPToolCallFSM``, день 17);
- ``mcp_intent``/``mcp_prompt`` — реплика инструмента и блок данных в промпте;
- ``mcp_server_spec`` — конфигурация ФЛОТА серверов как данные (день 20);
- ``scheduler_values`` — типы расписаний, состояния и коды отказа (день 18);
- ``scheduler_fsm`` — стейт-машины задачи планировщика и напоминания (день 18);
- ``schedule_spec`` — инструменты планировщика и расписание (день 18);
- ``schedule_timing`` — нормализация расписания и момент следующего запуска;
- ``schedule_intent`` — распознавание реплик трёх инструментов планировщика;
- ``aggregation`` — агрегация накопленных записей в сводку;
- ``scheduler_prompt`` — отчёт о шаге планировщика и блок промпта о задаче;
- ``pipeline_*`` — пайплайн дня 19: описание (``pipeline_spec``), маппинг данных
  (``pipeline_mapping``), FSM (``pipeline_fsm``), реплика и блок промпта;
- ``orchestration_*`` — оркестрация дня 20: план шагов (``orchestration_spec``),
  FSM прогона, промпт планировщика, построение плана и реплика-триггер.

Импорт ``AVAILABLE_CATEGORIES`` из ``invariant_values`` переименован на входе слоя
в ``AVAILABLE_INVARIANT_CATEGORIES``: ``AVAILABLE_CATEGORIES`` — уже категории
долговременной памяти (``memory_layers``), и два разных значения под одним
именем в одном пространстве имён — скрытая ошибка, а не удобство. Импорты слоя —
только стандартная библиотека, ``backend.core.config`` и модули этого же слоя.
"""

# Планировщик (день 18) и пайплайн (день 19) импортируются МОДУЛЯМИ, а не именами:
# у них десятки публичных имён, и общий список имён слоя вышел бы за лимит 400
# строк. Код дня берёт имена из своего модуля, поэтому достаточно точки входа:
# ``backend.domain.pipeline_fsm.PipelineFSM``.
from . import (
    aggregation,
    pipeline_fsm,
    pipeline_intent,
