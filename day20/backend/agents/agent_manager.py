...
Четвёртый — ``pipeline_service`` (день 19): служба запусков пайплайна; пятый —
``orchestration_service`` (день 20): служба прогонов по флоту серверов. Обе
получает каждый агент, поэтому прогон из чата и прогон через API пишут историю в
одно место (не заданы — берутся службы процесса).
"""
import threading
...
class AgentManager(AgentPoolMixin, ContextOpsMixin, UsageOpsMixin,
    ...
    def __init__(self, session_factory=None, client_factory=None,
                 mcp_registry=None, pipeline_service=None,
                 orchestration_service=None) -> None:
        ...
        # Служба оркестрации (день 20): передаётся каждому созданному и
        # восстановленному агенту, поэтому флоу по реплике работает с тем же
        # журналом запусков, что и API (None — служба процесса, см. Agent).
