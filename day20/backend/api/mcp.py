"""Роутер API дня 17: MCP-подключение, инструменты и вызов инструмента.

Пять эндпоинтов (каталог флота серверов переехал в ``backend/api/mcp_servers.py``):
...
from ..schemas import (
    MCPCallIn, MCPCallResponse, MCPConnectIn, MCPStatusResponse, MCPToolSchema,
    MCPToolsResponse,
)
