"""Схемы API дня 17: агент, генерация и метрики использования токенов.
...
from .mcp import MCPCallReportOut
from .memory import MemoryInfo
from .orchestration import OrchestrationReportOut
from .pipeline import PipelineReportOut
from .profile import AppliedProfileOut
from .scheduler import ScheduleReportOut
...
class GenerateResponse(BaseModel):
    ...
    # Заполняется ДО вызова DeepSeek, поэтому видно и при 502. Вместе с ним
    # приходят allowed_next и blocked (день 15) — из того же правила допуска,
    # что и у интерфейса.
    task_state: Optional[TaskStateOut] = None
    # Контролируемые переходы (день 15): что сделала реплика пользователя
    # (``applied``/``reason``) и какое предложение модели было отклонено.
    ...
    # MCP-инструмент (день 17): был ли вызван инструмент, какой и с какими
    # аргументами, чем закончилось и ушли ли данные в запрос. Заполняется до
    # вызова DeepSeek, поэтому видно и при 502.
    mcp: Optional[MCPCallReportOut] = None
    # Планировщик фоновых задач (день 18): зарегистрирован ли фоновый процесс по
    # реплике — какой инструмент, какая задача, чем она ответила. None — фоновой
    # задачи не появилось (вызов не распознан, отказ или сбой). Заполняется до
    # вызова DeepSeek, поэтому видно и при 502.
    schedule: Optional[ScheduleReportOut] = None
    # Пайплайн композиции (день 19): что сделал прогон по реплике — статус, шаги,
    # итог и ушёл ли результат в промпт. None — пайплайн не запускался.
    pipeline: Optional[PipelineReportOut] = None
    # Оркестрация MCP-серверов (день 20): что сделал прогон по реплике — план,
    # серверы, шаги с инструментами, итог и ушёл ли результат в промпт. None —
    # оркестрация не запускалась (реплика распознана как пайплайн или как вызов
    # инструмента). Заполняется до вызова DeepSeek, поэтому видно и при 502.
    orchestration: Optional[OrchestrationReportOut] = None
    system_prompt: str = ""
