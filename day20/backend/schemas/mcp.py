...
промпт. Каталог ФЛОТА серверов (``GET /mcp/servers``) описан отдельно —
``backend/schemas/mcp_servers.py``: у процесса по-прежнему одно активное
соединение, а флот серверов дня 20 — это отдельный механизм с отдельным роутом.
