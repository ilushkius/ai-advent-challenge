...
class MCPClient:
    ...
    def __init__(
        ...
        *,
        ...
        # Рабочий каталог дочернего процесса (только stdio): команда флота дня
        # запускается от корня дня, чтобы относительные пути её аргументов
        # разрешались одинаково из API, тестов и скриптов.
        self._cwd = cwd
        ...
        try:
            async with contextlib.AsyncExitStack() as stack:
                read, write = await stack.enter_async_context(
                    transport_context(self._target, self._cwd)
                )
