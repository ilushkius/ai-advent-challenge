...
def transport_context(target: MCPTarget,
                      cwd: str | Path | None = None) -> AsyncContextManager[Any]:
    """Контекст транспорта: stdio (дочерний процесс), SSE или Streamable HTTP.

    ``cwd`` задаёт рабочий каталог дочернего процесса (только для stdio): команда
    флота запускается от корня дня, иначе относительные пути её аргументов
    (``mcp_servers/search_server/server.py``) указывали бы не туда. HTTP-транспорты
    каталог игнорируют: процесс поднимает не клиент.
    """
