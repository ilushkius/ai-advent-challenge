"""Движок, сессии и начальная схема БД дня 19 (SQLAlchemy/SQLite).

ORM-таблицы дня описаны в ``backend/models/*.py`` (``agent``, ``message``,
``memory``, ``context``, ``user_profile``, ``task_state``, ``invariant``,
``scheduler``, ``pipeline``) и реэкспортируются отсюда (``Base``, ``AgentRecord``,
``ShortTermMessage``, ``Summary``, ``TokenUsage``, ``Fact``, ``WorkingMemory``,
``LongTermMemory``, ``Checkpoint``, ``UserProfile``, ``TaskState``,
``TaskTransition``, ``Invariant``, ``ScheduledTask``, ``SchedulerTaskRun``,
``Reminder``, ``SchedulerNotification``, ``CollectedRecord``, ``PeriodicSummary``,
``PipelineRun``, ``PipelineStep``, ``OrchestrationRun``,
``OrchestrationStep``), поэтому остальной код дня по-прежнему импортирует их из
...
# Движок и фабрика сессий приложения (база day20/agents.db, см. config).
