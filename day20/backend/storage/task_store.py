...
from ..domain.task_fsm import TaskStage, TaskStep, rollback_target, stage_from_value
...
if TYPE_CHECKING:  # только для аннотаций (модуль начинается с future-annotations)
    # Импорт отложен намеренно: ``backend.agents.memory`` импортирует этот пакет
    # (``storage``), и жадный импорт на уровне модуля давал бы цикл
    # «storage -> agents -> storage» при старте с ``import backend.storage``.
...
class TaskStateStore:
    ...
    def memory_manager(self) -> MemoryManager:
        ...
        if self._memory is None:
            # Локальный импорт — разрыв цикла storage -> agents -> storage.
            from ..agents.memory import MemoryManager
    ...
    def state_dict(self, row: TaskState) -> dict:
        ...
        Производные поля считаются здесь, а не в слое API: ``allowed_next`` и
        ``blocked`` — это ответ правил допуска (``task_state_machine``),
        ``prompt_block`` — тот же текст, что уходит в системный промпт. Один
        источник — одно место, интерфейс и агент видят одинаковый набор
        доступных переходов.
        """
        context = dict(row.context or {})
        paused_from = row.paused_from_stage
        state = {
            "id": row.id,
            "task_id": row.task_id,
        ...
        guard = guard_context(state)
        allowed = [stage.value for stage in get_allowed_next_stages(row.stage, guard)]
        state["allowed_next"] = allowed
        state["blocked"] = [
            {"stage": stage.value, "reason": reason}
            for stage, reason in get_blocked_stages(row.stage, guard)
        ]
        state["prompt_block"] = render_task_state_block(
            row.stage, row.current_step, row.expected_action, allowed,
    ...
    def apply(self, session, row: TaskState, *, stage: TaskStage, step: TaskStep,
              ...
              paused_from: Optional[tuple[TaskStage, TaskStep]],
              clear_flags: tuple[str, ...] = ()) -> dict:
        """Единственный путь записи состоявшегося перехода: history + журнал + строка.
        ...
        ``clear_flags`` — флаги-согласования, недействительные после движения
        назад (``task_state_machine.cleared_flags``): они удаляются из
        ``context`` здесь, чтобы сброс нельзя было забыть в одном из путей
        перехода.
        """
        ...
        row.paused_from_stage = paused_from[0].value if paused_from else None
        context = dict(row.context or {})
    ...
    def log_rejection(self, session, row: TaskState, *, to_stage: Optional[str],
                      to_step: Optional[str], reason: str) -> None:
        """Записать отклонённую попытку перехода в task_transitions.

        Строку ``task_states`` попытка НЕ меняет: после отказа состояние задачи
        прежнее. Целевые этап и шаг могут быть не названы (``None``) — попытку
        перехода в неизвестный этап иначе не записать.
        """
        now = datetime.now(timezone.utc)
        session.add(TaskTransition(
            task_id=row.task_id, from_stage=row.stage, from_step=row.current_step,
            to_stage=to_stage, to_step=to_step, reason=reason, accepted=False,
            created_at=now,
        ))
        session.commit()
        logger.debug(
            "Отклонённая попытка перехода задачи %s: %s -> %s (%s)",
            row.task_id, row.stage, to_stage, reason,
        )

    def set_flags(self, session, row: TaskState, flags: dict[str, bool]) -> dict:
        """Выставить флаги-согласования в ``context`` строки задачи.

        Изменение флагов — не переход: состояние задачи (этап, шаг, журнал) не
        меняется, поэтому в ``task_transitions`` эта операция не пишется.
        """
        context = dict(row.context or {})
        ...
        session.commit()
        logger.debug("Флаги задачи %s: %s", row.task_id, flags)
        return self.state_dict(row)
