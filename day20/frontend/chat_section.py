"""Раздел «💬 Чат и память» дня 20: карточка агента, панели, диалог и ввод.
...
поставила фоновую задачу (``record["schedule"]``), рядом с токенами появляется
строка «🗓 Планировщик: …», а если запустился прогон (``record["pipeline"]``) —
строка «🔀 Пайплайн: …»: иначе заметить, что задача уже стоит или что результат
прогона ушёл в промпт, было бы негде.
"""
import streamlit as st

from . import (
    api_client, common, context_panels, invariant_panel, mcp_section,
    memory_panels, notifications, orchestration_section, pipeline_section,
    profile_section, scheduler_section, task_panel,
)
...
def render_main_area() -> None:
    ...
    # Уведомления планировщика (день 18): фон работает в бэкенде, и его след
    # (сработавшее напоминание, готовая сводка, упавший тик) виден в основной
    # области — фрагмент сам обновляется каждые 5 секунд.
    if st.session_state.get("backend_ok"):
        notifications.render_notifications()

    if not st.session_state["backend_ok"]:
        st.warning("🔌 Бэкенд недоступен. Запустите его из папки day20/:")
        ...
        if section.startswith("👤"):
            ...
            # MCP-подключение у процесса одно: активный агент разделу не нужен.
            mcp_section.render_mcp_section()
        elif section.startswith("🗓"):
            # Планировщик — тоже служба процесса, а не свойство агента.
            scheduler_section.render_scheduler_section()
        elif section.startswith("🔀"):
            # Пайплайны дня 19: один прогон по одной цепочке инструментов.
            # Пайплайн — тоже служба процесса: он вызывает инструменты сервера дня.
            pipeline_section.render_pipeline_section()
        elif section.startswith("🌐"):
            # Оркестрация дня 20: флот серверов, план шагов и один флот на процесс.
            orchestration_section.render_orchestration_section()
        else:
            if not agents:
                    ...
                    with st.spinner("🤖 Агент думает… это занимает несколько секунд"):
                        try:
                            ...
                            if record.get("status") == "ok":
                                ...
                                # MCP (день 17): вызов инструмента виден и в этом
                                # разделе — ход мог измениться из-за данных сервера.
                                mcp = record.get("mcp") or {}
                                if mcp.get("called"):
                                    notes.append(
                                        "🔧 MCP: " + str(mcp.get("tool")) + " → "
                                        + common.MCP_CALL_STATE_LABELS.get(
                                            mcp.get("state"), mcp.get("state", "")))
                                elif mcp.get("detected") and not mcp.get("accepted"):
                                    notes.append("⚠️ MCP: " + str(mcp.get("error")))
                                # Планировщик (день 18): ход мог поставить фоновую
                                # задачу — это видно и здесь, а не только в разделе
                                # «🗓 Планировщик».
                                schedule_note = common.schedule_note(
                                    record.get("schedule") or {})
                                if schedule_note:
                                    notes.append(schedule_note)
                                # Пайплайн (день 19): ход мог выполнить прогон из
                                # нескольких инструментов — его итог виден и здесь,
                                # а не только в разделе «🔀 Пайплайны».
                                pipeline_note = pipeline_section.pipeline_note(
                                    record.get("pipeline") or {})
                                if pipeline_note:
                                    notes.append(pipeline_note)
                                # Оркестрация (день 20): ход мог выполнить прогон по
                                # нескольким серверам — его итог виден и здесь, а не
                                # только в разделе «🌐 Оркестрация».
                                orch_note = orchestration_section.orchestration_note(
                                    record.get("orchestration") or {})
                                if orch_note:
                                    notes.append(orch_note)
