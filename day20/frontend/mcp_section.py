...
def _load_servers() -> dict | None:
    """Состав флота серверов с бэкенда (None — запрос не удался)."""
    try:
        return mcp_api.api_mcp_servers()
    except api_client.BackendError as exc:
        st.error(f"Состав флота не получен: {exc.message}")
        return None
...
def render_mcp_section() -> None:
    ...
    status = _load_status()
    if status is None:
        st.warning("MCP-раздел работает через бэкенд. Запустите его из папки day20/:")
