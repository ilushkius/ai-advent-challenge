# День 20 — оркестрация MCP-серверов: `search_server` → `data_server` → `storage_server` (+ планировщик, MCP, память, профиль, задача, инварианты и переходы)

Задание дня — **оркестровать флот MCP-серверов**, а не вызвать один инструмент.
Рядом с приложением поднимаются **три независимых MCP-сервера** по stdio, каждый
со своим пакетом и без импортов друг из друга и из `backend/`:
`mcp_servers/search_server` (поиск: `search_web`, `search_local`, `fetch_url`),
`mcp_servers/data_server` (обработка: `summarize`, `extract_keywords`,
`filter_by_date`, `aggregate`) и `mcp_servers/storage_server` (сохранение и
выдача: `save_to_file`, `save_to_db`, `list_saved`, `load_from_file`) — всего
**одиннадцать** инструментов. Состав флота — **данные**: он описан в
`day20/mcp_servers.json` (имя, команда запуска, описание и кэш каталога
`tools_cache`), поэтому новый сервер добавляется правкой файла, **без правки
кода**.

Вызов маршрутизируется по имени инструмента. `MCPRegistry` (день 17 — одно
активное соединение) с днём 20 держит ещё и **флот**: по одному `MCPClient` на
сервер (`connect_all`), кэш каталога каждого (`tools/list` в памяти и в
`tools_cache` файла) и маршрутизацию `find_tool_by_name` → `call_tool`: клиенту
не нужно знать, кто умеет `summarize`. Сервер, не поднявшийся на старте,
подключается **по ходу** (`ensure_connected`), когда до него дошёл шаг плана; при
дублях имён инструментов выигрывает первый сервер в порядке файла с
предупреждением в лог — добавление сервера не меняет поведение работающих
вызовов.

`Orchestrator` строит план шагов и выполняет его: маппинг аргументов
(`$steps.<i>.<путь>` и `{имя}` — тот же механизм, что у пайплайна дня 19) →
условие перехода → маршрутизация по имени инструмента → подключение сервера при
нужде → вызов через `MCPToolRunner(server_name=…)` → строка журнала → событие
FSM. План берётся из трёх источников: **присланный** клиентом (`given`),
**предложенный моделью** DeepSeek (`llm`; промпт прямо требует «определи, какие
инструменты с каких серверов нужно вызвать и в каком порядке») или собранный
**эвристикой** (`heuristic`, когда ключа нет или модель не ответила). Каждый шаг
логируется отдельной строкой `orchestration_steps` с сервером, инструментом,
входными аргументами, результатом, временем и статусом; прогон целиком — строкой
`orchestration_runs` (запрос, план, статус, длительность, список серверов), и
история переживает перезапуск бэкенда.

Из чата оркестрация запускается по реплике — но **узким** правилом
(`backend/domain/orchestration_intent.py`): фразы «в базу», «в бд», «в базу
данных», «в sqlite», «оркестрац…», «несколько/нескольких серверов»,
«разными/разных серверов», «флот/флота серверов», «цепочк…». Реплика дня 19
«найди статьи про RAG, сделай сводку и сохрани в файл» обязана остаться
**пайплайном** (её контракт закреплён тестом), поэтому общее слово «сохрани» её
не включает. Результат прогона уходит в системный промпт блоком «## Результат
оркестрации», а один ход запускает ровно **один** автоматизм: при распознанной
оркестрации пайплайн и одиночный вызов MCP-инструмента пропускаются.

В интерфейсе день 20 добавил **восьмой** раздел «🌐 Оркестрация»: таблица флота и
инструменты выбранного сервера (с кнопкой «🔄 Обновить кэш инструментов»), бóльшая
кнопка **«🚀 Запустить демо-сценарий»** и форма произвольного запроса, живой
прогресс пяти шагов (`@st.fragment(run_every="1s")`), схема флоу (mermaid плюс
текстовая цепочка), таблица шагов «индекс / сервер / инструмент / входные данные
/ выходные данные / время / статус», история запусков с деталями и удалением и
статистика (вызовы по серверам, среднее время шага, топ инструментов). Демо-кнопка
запускает тот же сценарий, что CLI-прогон и тест: пять шагов по трём серверам
(`search_web` → `summarize` → `extract_keywords` → `save_to_file` → `save_to_db`).

Живой прогон дня —
[docs/reports/orchestration_demo.md](docs/reports/orchestration_demo.md): пять
сценариев (демо-кнопка на настоящем флоте, произвольный запрос через агента,
ошибка на шаге, перезапуск приложения, четвёртый сервер без правки кода), таблица
инструментов трёх серверов, трассировка шагов «шаг / сервер / инструмент /
входные данные / выходные данные / время выполнения / статус» со значениями из
журнала, схема двух таблиц и mermaid-диаграмма потока. Артефакт прогона остаётся
в репозитории — `day20/output/demo-scenario.md`.

Структура дня 19 сохранена полностью: планировщик фоновых задач, свой MCP-сервер
дня 17–19, инварианты, состояние задачи с контролируемыми переходами,
персонализация, три слоя памяти, стратегии контекста и декларативный пайплайн.
Единственный писатель в `agents.db` — по-прежнему процесс бэкенда; `storage_server`
пишет **в свой** файл `day20/storage.db` (таблица `saved_records`), а не в базу
дня: инвариант дня 18 «одна база — один писатель» не нарушен. Приложение
развивает день 19, а `day19/`, `day18/`, `day17/` и более ранние дни остаются
снимками (код `day1/`–`day19/` не изменялся).

## Что нового относительно дня 19

| Новое | Где |
|---|---|
| **Три MCP-сервера** (11 инструментов, транспорт stdio, независимые процессы): поиск (`search_web`/`search_local`/`fetch_url`), обработка данных (`summarize`/`extract_keywords`/`filter_by_date`/`aggregate`), сохранение и выдача (`save_to_file`/`save_to_db`/`list_saved`/`load_from_file`) | `mcp_servers/search_server/`, `mcp_servers/data_server/`, `mcp_servers/storage_server/` |
| **Конфигурация флота данными**: состав серверов (`name`/`command`/`args`/`description`/`tools_cache`), загрузка и проверка спецификаций, атомарная запись кэша каталога | `mcp_servers.json`, `backend/domain/mcp_server_spec.py`, `backend/core/mcp_server_config.py` |
| **Многоподключенческий `MCPRegistry`**: активное соединение дня 17 плюс флот, маршрутизация вызова по имени инструмента (`find_tool_by_name`/`call_tool`), подключение сервера по ходу (`ensure_connected`), кэш каталога в памяти и в файле | `backend/services/mcp_registry.py`, `backend/services/mcp_fleet_state.py` |
| **Оркестратор**: план из трёх источников (`given`/`llm`/`heuristic`), порядок шага «маппинг → условие → маршрутизация → подключение → вызов → журнал → FSM», фоновый запуск для API и интерфейса и синхронный для агента, терминальный статус при любом исходе | `backend/services/orchestrator.py`, `backend/services/orchestration_service.py`, `backend/services/orchestration_planner.py` |
| **Домен оркестрации**: план `DEMO_PLAN` и его проверка, FSM прогона (`idle`/`running`/`completed`/`stopped`/`failed`), промпт планировщика и разбор ответа, эвристика реплики, системный блок «## Результат оркестрации» | `backend/domain/orchestration_spec.py`, `orchestration_fsm.py`, `orchestration_plan.py`, `orchestration_intent.py`, `orchestration_prompt.py` |
| **Таблицы**: две новые — `orchestration_runs` (запрос, план, статус, длительность, `servers_used`) и `orchestration_steps` (сервер, инструмент, входные аргументы, результат, время, статус, ошибка) с каскадным удалением | `backend/models/orchestration.py`, `backend/storage/orchestration_store.py`, `backend/storage/orchestration_rows.py` |
| **API**: шесть эндпоинтов `/orchestration` (запуск, демо, история с фильтром и статистикой, отчёт, шаги, удаление) и три `/mcp/servers` (флот, инструменты сервера, обновление кэша) — всего в API дня **85** записей эндпоинтов; версия приложения `14.0.0` | `backend/api/orchestration.py`, `backend/api/mcp_servers.py`, `backend/schemas/orchestration.py`, `backend/schemas/mcp_servers.py` |
| **Интерфейс**: восьмой раздел «🌐 Оркестрация» — флот, кнопка «🚀 Запустить демо-сценарий», живой прогресс шагов, схема флоу (mermaid и текст), таблица шагов, история и статистика; в сводке хода — строка оркестрации | `frontend/orchestration_section.py`, `frontend/orchestration_steps.py`, `frontend/orchestration_api.py`, `frontend/chat_section.py` |
| **Тесты и документы**: домен/сервисы/хранилище/API оркестрации и три stdio-теста реальных серверов, фейки флота `tests/orchestration_fakes.py`; отчёт `docs/reports/orchestration_demo.md` | `tests/unit/test_orchestration_*.py`, `tests/unit/test_mcp_server_*.py`, `tests/integration/test_orchestration_*.py`, `tests/integration/test_mcp_fleet_registry.py`, `tests/integration/test_*_server_stdio.py`, `tests/e2e/test_orchestration_api.py`, `tests/e2e/test_mcp_servers_api.py` |

## Оркестрация

Оркестрация — то, чем день 20 отличается от дня 19. День 19 соединил инструменты
**одного** сервера в пайплайн; день 20 разносит инструменты по **трём** серверам и
даёт реестру решать, кому уходит вызов, а оркестратору — строить из них цепочку.

### Три сервера и их инструменты

| Сервер | Команда запуска (из `day20/`) | Инструменты |
|---|---|---|
| `search_server` | `uv run python mcp_servers/search_server/server.py` | `search_web` (лента jsonplaceholder `posts`/`users` или Википедия), `search_local` (файл внутри папки дня), `fetch_url` (страница по `http(s)://`, текст без тегов) |
| `data_server` | `uv run python mcp_servers/data_server/server.py` | `summarize` (DeepSeek, а без ключа — агрегация), `extract_keywords` (частоты со стоп-словами), `filter_by_date` (границы ISO-8601 или `ДД.ММ.ГГГГ`), `aggregate` (`count`/`sum`/`avg`/`min`/`max` по группам) |
| `storage_server` | `uv run python mcp_servers/storage_server/server.py` | `save_to_file` (файл в `output/`), `save_to_db` (строка в `storage.db`), `list_saved` (файлы и строки), `load_from_file` (текст сохранённого файла) |

Флаги серверов: `search_server` — `--api-base`/`--wiki-base`/`--timeout`/`--file-root`;
`data_server` — `--llm {auto,off}`/`--timeout`; `storage_server` —
`--output-dir`/`--db-path`. По умолчанию все три запускаются из `day20/`, то есть
их относительные пути (`output/`, `storage.db`, файлы дня) указывают на папку дня.

### Демо-сценарий одной кнопкой

Кнопка «🚀 Запустить демо-сценарий» в разделе «🌐 Оркестрация» отправляет
`POST /orchestration/demo` и запускает пять шагов по трём серверам:

| Шаг | Сервер | Инструмент | Что делает |
|---|---|---|---|
| 1 | `search_server` | `search_web` | берёт первые `limit` постов (пустой запрос — «без фильтра», потому что лента английская) |
| 2 | `data_server` | `summarize` | сводит найденные элементы в текст (без ключа — агрегацией) |
| 3 | `data_server` | `extract_keywords` | выделяет ключевые слова сводки |
| 4 | `storage_server` | `save_to_file` | пишет `output/demo-scenario.md` (сводка + ключевые слова) |
| 5 | `storage_server` | `save_to_db` | пишет строку `kind='orchestration'` в `storage.db` со ссылкой на файл и ключевые слова |

Прогон идёт **фоном** (`background: true`), поэтому интерфейс сразу получает
`run_id` и показывает прогресс: сколько шагов пройдено из пяти, каждый шаг со
своим сервером, инструментом, результатом и временем, диаграмму флоу и таблицу
шагов; история подтягивается раз в 5 секунд. Ровно тот же сценарий запускают
CLI-прогон и тесты — он описан в домене (`DEMO_QUERY`/`DEMO_PLAN`), а не собран
в интерфейсе.

Доказательство прогона — [docs/reports/orchestration_demo.md](docs/reports/orchestration_demo.md)
(пять сценариев, таблица инструментов трёх серверов, трассировка шагов, схема
таблиц и диаграмма потока); инструкция — [docs/usage.md](docs/usage.md). Как
добавить четвёртый сервер — там же: правка `mcp_servers.json`, код не меняется.
...
# терминал 1 — бэкенд (при старте поднимает таблицы, планировщик и флот серверов)
uv run uvicorn backend.api.main:app --reload --port 8000

# терминал 2 — интерфейс; раздел «🌐 Оркестрация» → «🚀 Запустить демо-сценарий»
uv run streamlit run app.py

# сквозной прогон пяти сценариев оркестрации офлайн, без ключа и сети
uv run python scripts/orchestration_demo.py --report docs/reports/orchestration_demo.md
uv run python scripts/orchestration_demo.py --llm auto --report docs/reports/orchestration_demo.md  # сводка через DeepSeek
uv run python scripts/orchestration_demo.py --tests --report docs/reports/orchestration_demo.md     # плюс прогон автотестов

uv run pytest -q      # автотесты дня, офлайн
```

Серверы флота приложению поднимать вручную не нужно: цели из `mcp_servers.json`
имеют команду `uv run python mcp_servers/<server>/server.py`, и реестр запускает
их дочерними процессами. Отдельная команда нужна только для ручной проверки
(интерфейс сервера — JSON-RPC на stdin/stdout):

```bash
uv run python mcp_servers/search_server/server.py --api-base http://127.0.0.1:8765  # стенд вместо сети
uv run python mcp_servers/data_server/server.py --llm off                          # сводка агрегацией, без ключа
uv run python mcp_servers/storage_server/server.py --output-dir output --db-path storage.db
```

Ключ `DEEPSEEK_API_KEY` в `day20/.env` нужен агенту, планировщику плана и
инструменту `summarize`; без ключа план строит эвристика, а сводку собирает
движок агрегации — демо и тесты идут офлайн.

Ниже — унаследованные разделы дней 19–16. Они остаются историей: счётчики
эндпоинтов и описания каталога серверов в них относятся к своим дням и помечены
как исторические, а там, где поведение изменилось (`GET /mcp/servers`, число
эндпоинтов), добавлена пометка о дне 20. Актуальные значения дня 20: **85**
записей эндпоинтов, `GET /mcp/servers` отдаёт флот из `mcp_servers.json`, а не
список целей.

## Наследовано из дня 19 — MCP-инструменты композиции и декларативный пайплайн: `search` → `summarize` → `save_to_file` (+ планировщик, MCP, память, профиль, задача, инварианты и переходы)
...
каталоге `day20/mcp_server/server.py` теперь **девять** инструментов, а сервер
представляется как `day20-tools` версии `1.3.0`.
...
`day20/` плюс общий пакет `shared/` в корне репозитория, куда вынесен код, не
...
| **API**: пять эндпоинтов `/pipelines` (запуск, список запусков с фильтром статуса, отчёт запуска, шаги запуска, удаление) — всего к дню 19 в API дня **78** (в дне 20 — 85); версия приложения того дня `13.0.0` | `backend/api/pipelines.py`, `backend/schemas/pipeline.py` |
...
| `save_to_file` | `content: str` (обязательный), `filename: str = "pipeline_result.md"`, `format: str = "md"` (`txt`/`md`/`json`) | пишет файл в каталог `day20/output/`: `filename`, `filepath`, `size_bytes`, `format`, `saved_at` |
...
Результат последнего шага — файл в `day20/output/`; прогон офлайн-демонстрации
...
| Четырнадцать эндпоинтов `/scheduler/...` (всего к дню 18 в API дня 78 — с пятью эндпоинтами пайплайна дня 19; в дне 20 записей 85) и общий код-путь `POST /scheduler/tasks` | `backend/api/scheduler.py`, `backend/schemas/scheduler.py` |
| Жизненный цикл вынесен из `main.py`: таблицы → агенты → планировщик (днём 20 — ещё и флот MCP), на выходе планировщик → реестр MCP | `backend/api/lifespan.py` |
...
| Эндпоинты `POST /mcp/call` и `GET /mcp/servers` — всего шесть MCP-эндпоинтов; днём 20 роут `GET /mcp/servers` переехал в роутер флота (`backend/api/mcp_servers.py`) и отдаёт серверы из `mcp_servers.json`, а у активного соединения осталось **пять** эндпоинтов | `backend/api/mcp.py` |
| Раздел «🔌 MCP»: каталог серверов (днём 20 — флот из `GET /mcp/servers`), форма аргументов по `input_schema`, результат вызова и блок «🤖 Спросить агента» | `frontend/mcp_call.py`, `frontend/mcp_ask.py`, `frontend/mcp_section.py`, `frontend/mcp_api.py` |
...
(`day20/mcp_server/`, транспорт **stdio**), клиент умеет вызывать его инструменты
(`tools/call`), а агент делает это сам по реплике пользователя. В дне 19 сервер
(имя `day20-tools`, версия `1.3.0`) публикует **девять** инструментов: три
...
| Файл | Что делает |
|---|---|
| `mcp_server/config.py` | адрес внешнего API и бэкенда дня, таймаут, границы `limit` и id, имя/версия сервера, инструкция для модели; пути каталога `output/`, корня источников, базы дня и ключ DeepSeek из `day20/.env` |
...
* **каталог серверов** — три цели дня 16 (`GET /mcp/servers`: свой сервер, fetch,
  filesystem), у подключённой видно число инструментов, к остальным можно
  подключиться кнопкой; днём 20 тот же роут отдаёт **флот** из `mcp_servers.json`
  (`search_server`/`data_server`/`storage_server`), и таблица показывает команду
  запуска, описание, число инструментов и состояние каждого сервера;
...
curl http://127.0.0.1:8000/mcp/servers        # каталог серверов дня 16; днём 20 — флот из mcp_servers.json (три сервера)
...
Сценария дня 11 в `day20/` нет — вместе с ним ушли кнопка его запуска, панель
...
В `day20/` диалог ведётся вручную в чате, а доказательства дают
...
Одна строка на задачу в той же базе `day20/agents.db`. `task_id` уникален
...
```bash
cd day20
uv run python scripts/task_state_demo.py --all           # нужен DEEPSEEK_API_KEY в day20/.env
...
Скрипт работает на отдельной базе `day20/task_state_demo.db`, заводит агента
...
```bash
cd day20
uv run python scripts/seed_invariants.py            # 4 демо-правила в day20/agents.db
...
Профиль — одна строка на пользователя в той же базе `day20/agents.db`:
...
```bash
cd day20
uv run python scripts/personalization_comparison.py            # нужен DEEPSEEK_API_KEY в day20/.env
uv run python scripts/personalization_comparison.py --no-api   # офлайн-заглушка, без сети
```

Скрипт пишет `day20/docs/reports/personalization_comparison.md` и работает на отдельной базе
`day20/personalization_demo.db` (пересоздаётся при каждом прогоне).
...
* «🔀 Пайплайны»: состояние подключения к серверу дня и кнопка
  ...
  «🗑 Удалить запуск».
* Переменная окружения адреса бэкенда: `DAY20_BACKEND_URL` (по умолчанию
...
Streamlit (порт 8501) ── HTTP (requests) ──► FastAPI (порт 8000) ──► DeepSeek API
   ...
   «🗓 Планировщик» / «🔀 Пайплайны»
                                    ...
                                    SQLAlchemy → SQLite (day20/agents.db)
...
| Слой | Папка | Зачем |
|---|---|---|
| Конфигурация и зависимости | `backend/core/` | Настройки дня (лимиты, цены, пути `.env`/`agents.db`, блоки настроек планировщика, пайплайна и оркестрации, файл флота `mcp_servers.json`), загрузка флота и запись кэша каталога (`mcp_server_config`) и доступ роутов к менеджеру агентов, реестру MCP (активное соединение и флот), планировщику, службе пайплайнов и службе оркестрации |
...
| Утилиты | `backend/utils/` | Слой объявлен обязательным, но в дне 20 пуст: общий код живёт в repo-level `shared/` |
| Интерфейс | `frontend/` | Streamlit по секциям: транспорт, подписи, панели, разделы (включая «🔌 MCP», «🗓 Планировщик», «🔀 Пайплайны» и «🌐 Оркестрация») и уведомления области чата |
...
День 20 добавил к слоям своё, не меняя правила: в домене — `mcp_server_spec`
(спецификация сервера флота) и `orchestration_*` (план и его проверка, FSM
прогона, промпт планировщика и разбор ответа, эвристика реплики, системный блок);
в сервисах — `mcp_fleet_state` (запись сервера), `orchestrator`,
`orchestration_service`, `orchestration_planner`, а `mcp_registry` к активному
соединению получил флот; в хранилище — `orchestration_store`/`orchestration_rows`;
в моделях — `orchestration_runs`/`orchestration_steps`; в схемах — `mcp_servers` и
`orchestration`; в API — роутеры `mcp_servers.py` (три эндпоинта флота) и
`orchestration.py` (шесть эндпоинтов прогонов), а `lifespan` поднимает флот
(`connect_all()`) и закрывает его на выходе; в интерфейсе — `orchestration_api.py`,
`orchestration_section.py` и `orchestration_steps.py`. Отдельно от `backend/`
лежат три MCP-сервера (`mcp_servers/`) — независимые процессы, знающие только
`mcp`-SDK и `shared/`.
...
├── mcp_servers.json     # состав ФЛОТА дня 20: имя, команда запуска, описание и кэш каталога на каждый сервер
├── mcp_servers/         # три независимых MCP-сервера дня 20 (stdio): search_server (search_web, search_local,
│                        # fetch_url), data_server (summarize, extract_keywords, filter_by_date, aggregate),
│                        # storage_server (save_to_file, save_to_db, list_saved, load_from_file)
├── frontend/            # интерфейс по секциям (24 модуля, карта — в STRUCTURE.md)
│   ├── api_client.py    # HTTP-транспорт бэкенда (requests): BACKEND_URL (DAY20_BACKEND_URL), BackendError, request_json
│   ├── mcp_api.py       # запросы MCP-раздела: /mcp/status, /mcp/connect, /mcp/disconnect, /mcp/tools, /mcp/call, /mcp/servers
│   ├── mcp_call.py      # флот серверов (GET /mcp/servers) и ручной вызов инструмента по форме аргументов
│   ├── mcp_ask.py       # блок «🤖 Спросить агента»: агент сам вызывает инструмент и отвечает по данным
│   ├── orchestration_api.py     # запросы дня 20: /orchestration/run, /demo, /runs, /runs/{id}, /runs/{id}/steps, /mcp/servers/...
│   ├── orchestration_section.py # раздел «🌐 Оркестрация»: флот, кнопка демо-сценария, прогресс, история, статистика
│   ├── orchestration_steps.py   # шаг прогона, mermaid-диаграмма флоу и её текстовая схема, таблица шагов
...
│   ├── chat_section.py  # раздел «💬 Чат и память», переключатель восьми разделов и строки «🔀 Пайплайн: …» и «🌐 Оркестрация: …» в сводке хода
...
├── mcp_server/          # СВОЙ MCP-сервер дня 17–19 (транспорт stdio, JSON-RPC по stdin/stdout)
...
│   ├── orchestration_demo.py # день 20: оркестрация пяти сценариев на трёх серверах флота и сборка отчёта (--report/--output-dir/--db/--servers-file/--llm/--tests/--json)
│   ├── orchestration_scenarios.py # что именно проверяют пять сценариев (демо-кнопка, реплика агента, ошибка шага, перезапуск, четвёртый сервер)
│   ├── orchestration_report.py # сборка markdown-отчёта прогона (разделы 1–12, таблица инструментов, трассировка шагов, схема таблиц и диаграмма)
...
├── backend/
│   ├── api/             # FastAPI: 11 роутеров по доменам (в т.ч. mcp_servers — флот, orchestration — прогоны) + main.py (сборка app, 66 строк) + lifespan.py (таблицы → агенты → планировщик → флот)
│   ├── core/            # config.py (настройки, пути .env и agents.db, цели MCP, блоки планировщика, пайплайна и оркестрации, файл флота), mcp_server_config.py (загрузка флота и запись tools_cache), dependencies.py (get_manager, get_mcp_registry, get_scheduler, get_schedule_service, get_pipeline_service, get_orchestration_service, *_or_404)
...
│   │                    # mcp_prompt, mcp_server_spec, scheduler_values, scheduler_fsm, schedule_spec,
...
│   │                    # orchestration_spec, orchestration_fsm, orchestration_plan, orchestration_intent,
...
│   │                    # daemon-потоке), mcp_errors, mcp_registry (MCPRegistry: активное соединение и флот),
│   │                    # mcp_fleet_state (FleetMember), mcp_tool_runner
...
│   │                    # orchestrator (Orchestrator: план и прогон), orchestration_service (OrchestrationService),
...
│   │                    # orchestration_store.py (запуски и шаги оркестрации, статистика), orchestration_rows.py (их проекции)
│   ├── agents/          # agent.py (в т.ч. шаги apply_pipeline, apply_orchestration, apply_mcp_tool и планировщика), memory.py, profile_store.py, agent_manager.py, manager_*.py (7 миксинов)
...
│   │                    # orchestration_runs, orchestration_steps
│   │                    # (реэкспорт — через storage/database.py)
│   ├── schemas/         # Pydantic-схемы API по доменам: agent, context, invariant, mcp, mcp_servers, memory, orchestration, pipeline, profile, scheduler, task
│   └── utils/           # своего кода нет: общий живёт в repo-level shared/
├── tests/               # pytest: файлы в подпапках unit/ (49 файлов), integration/ (41), e2e/ (13); точный счёт тестов — в STRUCTURE.md
...
│                        # orchestration_fakes.py (каталоги и результаты флота дня 20),
...
├── docs/
│   ├── architecture.md  # компоненты и модульная структура, схема БД, слои памяти, персонализация, FSM, переходы, инварианты, MCP-сервер и инструменты, планировщик, пайплайны, оркестрация MCP-серверов
│   ├── api.md           # 85 записей эндпоинтов с примерами и кодами ошибок
│   ├── usage.md         # инструкция дня 20: установка и запуск, три сервера флота и их инструменты, демо-сценарий одной кнопкой, чтение шагов и истории, добавление сервера
│   └── reports/         # orchestration_demo.md (день 20), pipeline_demo.md (день 19), scheduler_demo.md (день 18), mcp_tool_demo.md (день 17), mcp_demo.md (день 16) + унаследованные controlled_transitions_demo.md, task_state_demo.md, personalization_comparison.md
...
├── conftest.py          # добавляет корень day20 в sys.path (импорт `from backend.agents.agent import Agent`)
...
└── .env.example         # шаблон ключа DEEPSEEK_API_KEY и адреса бэкенда DAY20_BACKEND_URL
```
...
В `day20/.env` нужен ключ `DEEPSEEK_API_KEY=sk-...` (для шага агента и генерации,
а также для сводки `summarize` через LLM);
адрес бэкенда для интерфейса и MCP-сервера задаёт `DAY20_BACKEND_URL` (по
...
В этой рабочей копии так и получилось: `day20/.agents/skills/` содержит
...
Скиллы видны агенту omp, если сессия запущена **из папки `day20`** (провайдер
...
(`skills.customDirectories: day20/.agents/skills`) — там же он ищет только на
...
| `tests/e2e/test_mcp_call_api.py` | 13 | контракт `POST /mcp/call` (409/400/422/502 и 200 с `is_error`), `output_schema` в `/mcp/tools`, поле `mcp` ответа генерации (проверка `GET /mcp/servers` ушла в день 20 — роут переехал в роутер флота) |
...
* **Пять эндпоинтов `/pipelines`**: запуск (синхронно или в фоновом потоке),
  список запусков с фильтром статуса, отчёт запуска, его шаги и удаление — всего к
  дню 19 в API дня **78** эндпоинтов (день 20 добавил девять: шесть
  `/orchestration` и три `/mcp/servers`).
...
* **Свой MCP-сервер в дне**: `day20/mcp_server/` (транспорт stdio, имя
  `day20-tools`, версия `1.3.0`) с **девятью**
...
* **Каталог серверов**: `GET /mcp/servers` — свой сервер дня и два сервера
  ...
  цели, сравнение — по нормализованному виду команды. Днём 20 этот роут отдаёт
  **флот** из `mcp_servers.json` (имя, команда запуска, описание, число
  инструментов, состояние): каталог целей дня 16 удалён вместе с модулем
  `backend/domain/mcp_servers.py`.
...
* **Стенд для съёмки видео**: `uv run python scripts/video_scenario.py --ui`
  ...
  (своя БД, свой порт, `DAY20_BACKEND_URL`), открывает адрес в браузере и
...
- **Один экземпляр приложения на одну БД.** Планировщик живёт в процессе бэкенда:
  второй экземпляр на той же `day20/agents.db` получит собственную копию таймеров
...
- **MCP-подключение не переживает рестарт бэкенда.** Оно живёт в памяти процесса
  ...
  пайплайна и оркестрации) — в SQLite, а кэш каталога флота — в `tools_cache`
  файла `mcp_servers.json`.
- **Одно активное MCP-подключение на процесс.** Подключение к другому серверу в
  разделе «🔌 MCP» закрывает прежнее; каталог `GET /mcp/servers` дня 16 — это был
  список известных **целей**, а не открытых соединений. Днём 20 рядом живёт
  **флот**: по соединению на сервер из `mcp_servers.json`, вызов идёт тому
  серверу, который публикует инструмент (`find_tool_by_name` → `call_tool`).
...
- Один процесс бэкенда и один файл `day20/agents.db` (в `.gitignore` по `*.db`).
  Прогон отчёта персонализации работает на отдельной базе
  `day20/personalization_demo.db`, демонстрация состояния задачи — на
  `day20/task_state_demo.db`, демонстрация контролируемых переходов — на
  `day20/controlled_transitions_demo.db`, демонстрация MCP-инструментов — во
  ...
  удаляется вместе с ним, а файл-артефакт пишется в `day20/output/`), прогон
  планировщика — тоже во временном каталоге (`scheduler_run.db`, остаётся при
  `--keep-db`), автопроверка
  кадров сценария видео — на `day20/video_scenario.db` (плюс временный
