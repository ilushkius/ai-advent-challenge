"""HTTP-клиент фронтенда дня 14: запросы к бэкенду FastAPI.

Все обращения интерфейса к бэкенду идут через ``_request``: он собирает URL из
``BACKEND_URL`` (переопределяется переменной окружения ``DAY14_BACKEND_URL``),
...
задачи (день 13) и инварианты (день 14): CRUD правил проекта и проверка текста
(``api_check_invariants``).
"""
import os
...
BACKEND_URL = os.environ.get("DAY14_BACKEND_URL", "http://127.0.0.1:8000")
...
def _request(method, path, **kwargs):
    ...
    try:
        ...
        raise BackendError(
            f"Бэкенд недоступен ({BACKEND_URL}). Запустите его из папки day14/: "
