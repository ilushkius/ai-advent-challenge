"""Демонстрация дня 14: состояние задачи переживает перезапуск процесса.
...
# Скрипт лежит в day14/scripts/, а пакет backend — в корне дня: добавляем корень
...
def main(argv: list) -> int:
    ...
    if "--all" in argv:
        if not offline and not config.resolve_api_key():
            print(
                "Нет ключа DEEPSEEK_API_KEY (day14/.env или переменная окружения). "
