# API дня 14 — агенты DeepSeek с инвариантами, состоянием задачи, памятью и профилем

Бэкенд — FastAPI-приложение `day14/backend/api/main.py`. Заголовок приложения —
«Агенты DeepSeek с инвариантами — День 14», версия схемы — `8.0.0`
...
Схемы ответов описаны в пакете `day14/backend/schemas/` — по доменам: `agent.py`
...
Всего 51 эндпоинт: 11 в разделе агентов (десять эндпоинтов CRUD, генерации и
...
персонализации, состояния задачи и инвариантов и перечень эндпоинтов (51 строка —
...
```json
{
  "name": "Агенты DeepSeek с инвариантами — День 14",
  "docs": "/docs",
...
- **Всё состояние переживает рестарт.** Реплики (`short_term_messages`), рабочая
  ...
  журналом `task_transitions`) лежат в SQLite (`day14/agents.db`), а активные
