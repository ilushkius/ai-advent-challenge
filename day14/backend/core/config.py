"""Конфигурация бэкенда дня 14: DeepSeek, дефолты агентов, стратегии, память, профиль.
...
1. файл `.env` в папке дня (day14/backend/core/config.py -> day14/.env);
...
# Персистентное хранилище дня: SQLite-файл в папке дня (day14/agents.db).
...
# Путь к .env дня: этот файл лежит в day14/backend/core/, значит .env — в day14/.
...
def resolve_api_key():
    """Возвращает ключ DeepSeek: day14/.env → переменная окружения → None."""
