"""Тесты API состояния задачи (день 13) через TestClient.

Проверяются контракты девяти эндпоинтов ``/agents/{id}/tasks`` и
``/tasks/{id}/...``: коды ответов (201/200/400/404/409/422), полный цикл
жизни задачи, журнал переходов и поле ``task_state`` в ответе генерации.
...
def test_transition_to_done_then_advance_is_rejected(client):
    """Завершение задачи одним вызовом; после done шаг вперёд не описан."""
    agent_id = new_agent(client)
    ...
    assert client.post(f"/tasks/{TASK_ID}/pause").status_code == 200
    assert client.post(f"/tasks/{TASK_ID}/resume").json()["stage"] == "done"
