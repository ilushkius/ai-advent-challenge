...
в дне лежит собственный MCP-сервер (Model Context Protocol) — `day17/mcp_server/`,
пять модулей, транспорт stdio, три инструмента поверх публичного mock API
...
flowchart LR
    ...
    AG --> DB[("SQLite<br/>day17/agents.db")]
    MM --> DB
    PS --> DB
    TS --> DB
    IM --> DB
...
```
day17/
├── app.py            точка входа Streamlit (71 строка): только вызовы секций
├── mcp_server/       СВОЙ MCP-сервер (5 модулей, транспорт stdio): config (адрес
│                     API и границы), schemas (TypedDict-ответы инструментов),
│                     api_client (JsonPlaceholderClient + ExternalAPIError),
│                     server (три инструмента через @server.tool(), stdio),
│                     __init__ (описание пакета)
...
│   │                 mcp_tool_runner (допуск → вызов → исход)
...
├── tests/            unit/ (20 файлов), integration/ (22), e2e/ (9) + conftest.py,
│                     support.py, mcp_fakes.py и stub_api.py
...
`uv sync` не пытается установить сам `day17/` в `.venv`.

## Компоненты

| Файл | Зона ответственности |
| --- | --- |
| `day17/app.py` | Streamlit, точка входа (71 строка): `st.set_page_config` («MCP-инструменты · День 17», «🔧»), `common.init_state()` → `sidebar.render_sidebar()` → `chat_section.render_main_area()`; переключатель пяти разделов основной области — `st.radio` («💬 Чат и память» / «👤 Профиль пользователя» / «🧭 Состояние задачи» / «📏 Инварианты» / «🔌 MCP») |
| `mcp_server/server.py` | **Свой MCP-сервер дня** (день 17): `MCPServer` из MCP SDK 2.x, три инструмента через `@server.tool()` (`get_user`/`get_post`/`list_user_posts` — имя берётся из функции, описание из докстринга, `inputSchema` из аннотаций параметров, `outputSchema` из `TypedDict`-аннотации возврата), `_run` переводит `ExternalAPIError` в ошибку инструмента `ToolError`, `parse_args(--api-base/--timeout)` и `main()` → `server.run(transport="stdio")` |
...
| `mcp_server/config.py`, `mcp_server/__init__.py` | Настройки сервера: `DEFAULT_API_BASE` (jsonplaceholder), `DEFAULT_TIMEOUT`, `MAX_POSTS_LIMIT`/`MAX_USER_ID`, `SERVER_NAME`/`SERVER_VERSION`/`SERVER_INSTRUCTIONS`; докстринг пакета — что это за сервер и как он запускается |
| `mcp_server/schemas.py` | `TypedDict`-структуры ответов инструментов (`UserInfo`, `PostInfo`, `PostSummary`, `UserPosts`): SDK собирает из них `outputSchema`, а `structuredContent` ответа становится самим словарём |
...
| `backend/domain/mcp_servers.py` | Каталог известных серверов для `GET /mcp/servers` (день 17): ключи `day17-jsonplaceholder`/`fetch`/`filesystem`, `MCPServerOption` (`matches` — сравнение по нормализованной цели, `to_dict`), `KNOWN_SERVERS`, `server_records(connected_target, tool_count)` |
...
| `backend/api/` | Эндпоинты по доменам: `agents.py` (11), `context.py` (9), `invariants.py` (6), `mcp.py` (6), `memory.py` (10), `profiles.py` (6), `tasks.py` (11) — всего 59; пути абсолютные (`/agents/...`, `/mcp/...`), префиксов нет |
...
| `frontend/` | Streamlit-интерфейс по секциям (15 модулей): `sidebar`, `chat_section` (переключатель пяти разделов `st.radio` с ключом `main_section`, карточка агента, диалог, сводка после хода со строкой про MCP, блок предупреждения/отказа по инвариантам над вводом), `context_panels`, `memory_panels`, `profile_section`, `profile_comparison`, `task_panel` (раздел «🧭 Состояние задачи»), `task_transitions` (кнопки-этапы, причины недоступности, флаги-согласования, пауза/продолжение), `invariant_panel` (раздел «📏 Инварианты»), `mcp_section` (раздел «🔌 MCP»), `mcp_call` (ручной вызов инструмента и каталог серверов), `mcp_ask` («🤖 Спросить агента»), `common` (состояние сессии, `active_agent`, `TASK_STAGE_LABELS`, `TASK_FLAG_LABELS`, `blocked_reason`, `run_task_action`, `INVARIANT_*_LABELS`, `invariant_notice`, `MCP_CALL_STATE_LABELS`, `MCP_REASON_LABELS`, `fmt_time`, `flash`), `api_client` (`BACKEND_URL` из `DAY17_BACKEND_URL`, `_request` и обёртки эндпоинтов, включая одиннадцать функций состояния задачи, шесть — инвариантов и шесть — MCP), `__init__`; `app.py` — только точка входа (71 строка), модули `frontend/` не вызывают `st.*` на импорте |
...
ORM-класс `database.UserProfile`, таблица `user_profiles` в `day17/agents.db`:
...
(`day17/mcp_server/`, три инструмента поверх `jsonplaceholder.typicode.com`).
...
└── server.py      MCPServer + три инструмента (@server.tool()), _run (перевод
                   ошибки API в ToolError), parse_args, main() → run(transport="stdio")
```
...
адресом. `parse_args` даёт ключи `--api-base` и `--timeout`, поэтому тот же сервер
без правок кода можно направить на локальный стенд (так делают тесты).
...
`DAY17_BACKEND_URL` (по умолчанию `http://127.0.0.1:8000`), таймаут 90 с
...
| Сводка после хода | Одна плашка: время, токены, стоимость, `finish_reason`, слои памяти (короткая/рабочая/долговременная в токенах), применённый профиль («👤 профиль strict_tech: 6 элементов» или «👤 профиль: без персонализации»), строка про MCP-шаг («🔧 MCP: get_user → успешно» или предупреждение с причиной отказа), предупреждения |
...
по инвариантам, а MCP-подсистему проверяют пятнадцать файлов `test_mcp_*`; все
зелёные.
Файлы разложены по подпапкам по фикстурам: без БД и агента — `tests/unit/`
(20 файлов), с временной БД и `Agent` — `tests/integration/` (22), через
`TestClient` — `tests/e2e/` (9); общие фикстуры и фейки (`conftest.py`,
`support.py`, а для MCP — `mcp_fakes.py` и стенд внешнего API `stub_api.py`)
остаются в корне `tests/`, поэтому `pythonpath = . tests` из `pytest.ini` не
меняется.
...
- **Один процесс бэкенда и один файл `day17/agents.db`.** Несколько процессов
