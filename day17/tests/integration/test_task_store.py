...
Поведение переходов проверяется в ``tests/integration/test_task_state.py``; здесь — то, что
...
from backend.storage.task_store import TaskStateStore
from backend.domain.task_fsm import TaskStage, TaskStep
from backend.domain.task_state_machine import FLAG_VALIDATION_PASSED
from backend.domain.task_prompt import TASK_STATE_HEADER
...
def test_history_rows_live_in_transitions_table(
    ...
    assert [bool(row.accepted) for row in rows] == [True, True]
...
def test_log_rejection_writes_journal_row_without_touching_state(
    task_session_factory, task_machine, created
):
    """Отклонённая попытка — строка журнала с accepted=False, состояние прежнее."""
    store = TaskStateStore(session_factory=task_session_factory)
    before = task_machine.get_state(TASK_ID)

    with task_session_factory() as session:
        row = store.state_row(session, TASK_ID)
        store.log_rejection(
            session, row, to_stage=None, to_step=None,
            reason="Неизвестный этап задачи: 'нет-такого'",
        )

    entries = task_machine.get_full_history(TASK_ID)
    assert [entry["accepted"] for entry in entries] == [True, False]
    assert entries[-1]["to_stage"] is None
    assert entries[-1]["to_step"] is None
    assert entries[-1]["from_stage"] == TaskStage.PLANNING.value
    assert entries[-1]["reason"].startswith("Неизвестный этап задачи")

    after = task_machine.get_state(TASK_ID)
    assert (after["stage"], after["current_step"]) == (
...
def test_create_snapshots_working_memory(task_session_factory, task_machine):
    ...
    # Метка паузы живёт в колонке строки, а не в context (день 15): перезапись
    # working_memory очередным переходом не должна её затирать.
    assert "paused_from_stage" not in state["context"]
    assert "paused_from_step" not in state["context"]
...
def test_state_dict_reports_allowed_and_blocked_transitions(task_machine, created):
    """Производные списки переходов приходят вместе с состоянием задачи."""
    assert created["allowed_next"] == [TaskStage.PAUSED.value]
    blocked = {item["stage"]: item["reason"] for item in created["blocked"]}
    assert set(blocked) == {
        TaskStage.EXECUTION.value, TaskStage.VALIDATION.value, TaskStage.DONE.value,
    }
    assert blocked[TaskStage.EXECUTION.value] == (
        "Нельзя перейти в execution: план не утверждён"
    )
    assert "Допустимые следующие этапы: paused" in created["prompt_block"]

    with_flag = task_machine.set_flags(TASK_ID, {"plan_approved": True})
    assert with_flag["allowed_next"] == [
        TaskStage.EXECUTION.value, TaskStage.PAUSED.value,
    ]
    assert TaskStage.EXECUTION.value not in {
        item["stage"] for item in with_flag["blocked"]
    }
    assert "Допустимые следующие этапы: execution, paused" in with_flag["prompt_block"]
...
def test_paused_from_stage_lives_in_the_row(task_session_factory, task_machine):
    """Этап паузы — колонка строки: её видит и новая машина, и API.

    Отдельная колонка вместо ключа в ``context`` (день 15): перезапись контекста
    очередным переходом не должна терять точку возврата.
    """
    task_machine.create(AGENT_ID, TASK_ID, TaskStage.EXECUTION.value)
    task_machine.pause(TASK_ID)
    ...
    assert row.paused_from_stage == TaskStage.EXECUTION.value

    restarted = TaskStateMachine(session_factory=task_session_factory)
    state = restarted.get_state(TASK_ID)
    assert state["paused_from_stage"] == TaskStage.EXECUTION.value
    assert restarted.resume(TASK_ID)["stage"] == TaskStage.EXECUTION.value
