"""Интеграционный тест своего MCP-сервера на настоящем stdio (день 17).

Проверяется весь путь целиком: клиент поднимает ``day17/mcp_server/server.py``
...
#: Имена инструментов, которые обязан публиковать сервер дня.
...
def test_catalog_has_three_tools_with_schemas(client):
    """Каталог — три инструмента дня, у каждого схемы аргументов и результата."""
    client.connect()
