"""Тесты каталога MCP-серверов (день 17): что предлагает ``GET /mcp/servers``.
...
def test_catalog_lists_known_servers_in_order():
    ...
    assert [item["key"] for item in records] == [
        SERVER_KEY_DAY17, SERVER_KEY_FETCH, SERVER_KEY_FILESYSTEM,
    ]
...
def test_connected_server_is_the_only_one_marked():
    ...
    assert connected[0]["key"] == SERVER_KEY_DAY17
