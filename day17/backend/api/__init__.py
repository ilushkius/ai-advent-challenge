"""FastAPI-слой дня 17: роутеры по доменам и точка сборки приложения.

- ``agents`` / ``context`` / ``invariants`` / ``mcp`` / ``memory`` / ``profiles`` /
...
__all__ = ["agents", "context", "invariants", "mcp", "memory", "profiles", "tasks"]
