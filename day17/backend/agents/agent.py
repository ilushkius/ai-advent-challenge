"""Класс Agent дня 17 — агент с памятью, задачей, инвариантами, переходами и MCP.
...
class Agent:
    ...
    # --- MCP-инструменты (день 17) ---
    def apply_mcp_tool(self, prompt: str, payload: List[dict]) -> dict:
        """Шаг MCP (день 17): по реплике вызывает инструмент и кладёт данные в промпт.

        Возвращает отчёт для ответа API: был ли инструмент вызван, какой именно, с
        какими аргументами, чем закончилось и сколько токенов добавил блок. Данные
        ...
        и в этом дне он выполняется только по явным ключевым словам реплики.
        Неудача вызова ход не роняет: отказ или сбой видны в отчёте, ответ агента
        строится без внешних данных.
        """
        ...
        return {**outcome.to_dict(), "used_in_prompt": bool(block), "added_tokens": added}
    ...
    def _make_client(self):
        ...
        if not api_key:
            raise AgentError(
                "Ключ API не задан: укажите DEEPSEEK_API_KEY в файле day17/.env "
    ...
    def generate(self, prompt: str) -> dict:
        ...
        промпт (``used_in_prompt``, ``added_tokens``).
        """
        ...
        # этого же запроса. Шаг стоит ПОСЛЕ проверки запроса инвариантами (она
        # бесплатна и локальна, а вызов инструмента — HTTP-запрос наружу: отказ по
        # правилам не должен приводить к обращению к внешнему серверу) и ДО
