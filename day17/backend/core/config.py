...
1. файл `.env` в папке дня (day17/backend/core/config.py -> day17/.env);
...
# Персистентное хранилище дня: SQLite-файл в папке дня (day17/agents.db).
...
# Путь к .env дня: этот файл лежит в day17/backend/core/, значит .env — в day17/.
...
def resolve_api_key():
    """Возвращает ключ DeepSeek: day17/.env → переменная окружения → None."""
