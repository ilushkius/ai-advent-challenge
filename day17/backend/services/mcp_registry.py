"""Реестр MCP-подключения дня 17: одно активное соединение на процесс.
...
прежнее и открывает новое, отдаёт статус, список инструментов и вызывает
инструмент (``tools/call``). Так у процесса ровно один MCP-сервер, а
сервер-stdio не плодит дочерний процесс на каждый запрос списка: ``tools/list``
берётся из открытой сессии (с кэшем до ``refresh=True``).
...
from ..domain.mcp_tools import MCPToolInfo, MCPToolResult
...
class MCPRegistry:
    ...
    def call_tool(self, tool_name: str,
        ...
        """Вызывает инструмент подключённого сервера (без соединения — исключение).

        Реестр не проверяет аргументы: правила допуска — в домене
        (``backend/domain/mcp_tool_call``), а оркеструет их ``MCPToolRunner``.
        Здесь только транспорт: единственное соединение процесса решает, кто
        именно получит вызов.
        """
        if self._client is None:
            raise MCPNotConnectedError(
                "Подключение к MCP-серверу не установлено: сначала POST /mcp/connect"
