"""Раздел «🔌 MCP» дня 17: подключение, каталог серверов и вызов инструментов.
...
#: Цель по умолчанию — СОБСТВЕННЫЙ MCP-сервер дня 17: та же строка лежит в
...
def render_mcp_section() -> None:
    ...
    st.caption(
        ...
        "инструменты: файловую систему, веб-запросы, базы данных. День 17 "
        ...
        "по форме и автоматически из реплики агента. Свой сервер дня читает "
        "jsonplaceholder.typicode.com: инструменты `get_user`, `get_post`, "
        "`list_user_posts`."
    )
    ...
    status = _load_status()
    if status is None:
        st.warning("MCP-раздел работает через бэкенд. Запустите его из папки day17/:")
