...
``BACKEND_URL`` (переопределяется переменной окружения ``DAY17_BACKEND_URL``),
...
BACKEND_URL = os.environ.get("DAY17_BACKEND_URL", "http://127.0.0.1:8000")
...
def request_json(method, path, **kwargs):
    ...
    try:
        ...
        raise BackendError(
            f"Бэкенд недоступен ({BACKEND_URL}). Запустите его из папки day17/: "
