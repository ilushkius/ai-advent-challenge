"""Конфигурация бэкенда дня 9: DeepSeek, дефолты агентов, параметры сжатия.

День 9 развивает день 8: к лимитам контекста и ценам добавляются настройки
сжатия истории (таблица ``summaries`` и конспект в запросе вместо старых реплик).

Ключ DEEPSEEK_API_KEY ищется в следующем порядке (конвенция проекта):
1. файл `.env` в папке дня (day9/backend/config.py -> day9/.env);
...
# Персистентное хранилище дня 9: SQLite-файл в папке дня (day9/agents.db).
...
# Путь к .env дня 9: этот файл лежит в day9/backend/, значит .env — в day9/.
