"""HTTP-клиент фронтенда дня 15: запросы к бэкенду FastAPI.

Все обращения интерфейса к бэкенду идут через ``_request``: он собирает URL из
``BACKEND_URL`` (переопределяется переменной окружения ``DAY15_BACKEND_URL``),
...
проверка текста (``api_check_invariants``).
"""
import os
...
BACKEND_URL = os.environ.get("DAY15_BACKEND_URL", "http://127.0.0.1:8000")
...
def _request(method, path, **kwargs):
    ...
    try:
        ...
        raise BackendError(
            f"Бэкенд недоступен ({BACKEND_URL}). Запустите его из папки day15/: "
...
def api_task_allowed_next(task_id):
    ...
    """
    return _request("GET", f"/tasks/{task_id}/allowed-next")
...
def api_set_task_flags(task_id, flags):
    ...
    """
    return _request("PATCH", f"/tasks/{task_id}/context", json=dict(flags))
