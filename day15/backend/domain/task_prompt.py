...
Что это.
    ...
    Формат — ровно две строки: заголовок и одна строка состояния. Пример:
        ...
        Текущий этап задачи: execution. Допустимые следующие этапы: planning,
        paused. Ожидаемое действие: ожидается реализация модуля. Не пытайся
        перейти в недопустимый этап — сначала заверши текущий. Текущий шаг:
        implement. Предыдущие шаги: planning (gather_requirements, define_scope,
...
def build_prompt_block(
    ...
    step: str | TaskStep,
    expected_action: str,
    allowed_next: Iterable[str | TaskStage],
    resume_stage: str | TaskStage | None = None,
) -> str:
    """Одна строка состояния задачи для системного промпта.

    Допустимые следующие этапы называются явно, а запрет переходить в
    недопустимый — часть контракта блока: модель видит границу, за которую
    стейт-машина её не пустит, и не предлагает заведомо отклоняемый переход.
    """
    current = _stage(stage)
    allowed = ", ".join(
        item if isinstance(item, str) else item.value for item in allowed_next
    ) or "нет"
    return (
        f"Текущий этап задачи: {current.value}. "
        f"Допустимые следующие этапы: {allowed}. "
...
def render_task_state_block(
    ...
    Именно этот текст подставляет ``Agent._system_message``.
    """
    return (
        TASK_STATE_HEADER
        + "\n"
        + build_prompt_block(stage, step, expected_action, allowed_next, resume_stage)
    )
