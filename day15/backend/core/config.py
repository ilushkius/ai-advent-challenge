"""Конфигурация бэкенда дня 15: DeepSeek, дефолты агентов, стратегии, память, профиль.
...
1. файл `.env` в папке дня (day15/backend/core/config.py -> day15/.env);
...
# Персистентное хранилище дня: SQLite-файл в папке дня (day15/agents.db).
...
# Путь к .env дня: этот файл лежит в day15/backend/core/, значит .env — в day15/.
...
def resolve_api_key():
    """Возвращает ключ DeepSeek: day15/.env → переменная окружения → None."""
