# Архитектура дня 15 — «Контролируемые переходы состояний задачи»

Приложение дня 15 — агент DeepSeek (FastAPI-бэкенд + Streamlit-интерфейс,
SQLite, tiktoken) с тремя слоями памяти, профилем пользователя, системой
инвариантов и состоянием задачи как конечным автоматом; к нему добавлены
**контролируемые переходы**: у задачи есть явный граф допуска, guard-условия на
флаги контекста и отказ с объяснением вместо «нажал — и ничего не произошло».
Источник правды о переходах один — `backend/domain/task_state_machine.py`:
таблица `ALLOWED_TRANSITIONS`, guards `GUARDS` и тексты отказа. Вперёд задача
...
flowchart LR
    ...
    CP -->|OpenAI SDK| D
    AG --> DB[("SQLite<br/>day15/agents.db")]
    MM --> DB
    PS --> DB
    TS --> DB
    IM --> DB
...
│   ├── schemas/      Pydantic-схемы API по доменам: agent, context, invariant, memory,
│   │                 profile, task
│   ├── api/          FastAPI: agents, context, invariants, memory, profiles, tasks + main
│   └── utils/        слой объявлен обязательным, но пуст: общий код — в repo-level shared/
├── tests/            unit/ (13 файлов), integration/ (16), e2e/ (7) + conftest.py и support.py
...
`uv sync` не пытается установить сам `day15/` в `.venv`.

## Компоненты

| Файл | Зона ответственности |
| --- | --- |
| `day15/app.py` | Streamlit, точка входа (58 строк): `st.set_page_config` («Контролируемые переходы · День 15», «🚦»), `common.init_state()` → `sidebar.render_sidebar()` → `chat_section.render_main_area()`; переключатель четырёх разделов основной области — `st.radio` («💬 Чат и память» / «👤 Профиль пользователя» / «🧭 Состояние задачи» / «📏 Инварианты») |
...
| `backend/api/main.py` | Сборка FastAPI-приложения (80 строк, версия `9.0.0`): заголовок «Агенты DeepSeek с контролируемыми переходами — День 15» и описание, CORS, `lifespan` (создание таблиц + восстановление агентов), `include_router` шести роутеров; сами 53 эндпоинта (память, стратегии, ветки, факты и метрики, профили, состояние задачи, корневой `GET /` и инварианты) и обработка 404/409/422/502 — в `backend/api/` |
...
| Скрипты | `scripts/` (`controlled_transitions_demo.py` + `transitions_report.py` — доказательство дня 15, `task_state_demo.py`, `invariants_demo.py`, `seed_invariants.py`, `personalization_comparison.py`) | Прогоны вне pytest на своих БД: пишут отчёты в `docs/reports/` (или в корень дня), ключ и сеть не нужны |
...
ORM-класс `database.UserProfile`, таблица `user_profiles` в `day15/agents.db`:
...
`DAY15_BACKEND_URL` (по умолчанию `http://127.0.0.1:8000`), таймаут 90 с
...
**«🧭 Состояние задачи»** и **«📏 Инварианты»** (ключ `main_section`) — именно radio, а не `st.tabs`:
...
SQLite-БД (фикстуры `session_factory` и `make_agent`). Всего **1013** тестов, из
них **558** — по состоянию задачи (тринадцать файлов `test_task_*`) и **141** —
по инвариантам, зелёные.
...
Демонстрация дня 15 — контролируемые переходы состояний задачи (наследованные
инварианты, персонализация и три слоя памяти показываются как работающая основа).
Покадровый сценарий живёт в одной точке — инструкции [`usage.md`](usage.md) (§8).
Кадры: `ALLOWED_TRANSITIONS` и `GUARDS` из
`backend/domain/task_state_machine.py` на экране; заведение задачи
`tz-transitions` и «🔒 Почему часть переходов недоступна» с тремя причинами
(`execution` — «план не утверждён», `validation` — «пропущен этап execution»,
`done` — «пропущены этапы execution и validation»); «⏭ Следующий шаг» на последнем
шаге `planning` отвечает отказом «Нельзя перейти в execution: план не утверждён» и
оставляет строку в «🚫 Попытки недопустимых переходов»; реплика «подтверждаю» не
подставляет флаг («⚠️ Переход по реплике «advance» не выполнен…»); флаг «📝 План
утверждён» открывает ровно тот же переход; пауза `execution/test_locally`,
**перезапуск бэкенда** на той же `day15/agents.db` и `GET /tasks/{task_id}/state`,
вернувший `paused` с `paused_from_stage = execution`; откат `validation →
execution`, снявший `implementation_complete` и `validation_passed`
(`cleared_flags`); `done` без переходов (даже на паузу); замена предложения модели
отказом (`🚧`). Готовые числа, тексты отказов, pid двух процессов и журнал для
сверки — в [`reports/controlled_transitions_demo.md`](reports/controlled_transitions_demo.md)
(`uv run python scripts/controlled_transitions_demo.py --all`); инварианты —
[`../invariants_demo.md`](../invariants_demo.md). Состояние задачи на пяти фазах —
...
- **Один процесс бэкенда и один файл `day15/agents.db`.** Несколько процессов
