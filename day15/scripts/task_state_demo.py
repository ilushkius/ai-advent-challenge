"""Демонстрация дня 15: состояние задачи переживает перезапуск процесса.
...
Переходы между этапами закрыты guard-условиями (день 15): перед каждой границей
скрипт выставляет флаг согласования (``plan_approved``, ``implementation_complete``,
``validation_passed``) — тем же путём, каким это делает чекбокс в панели задачи.
Откат сбрасывает согласования, поэтому фаза 4 подтверждает реализацию заново.
"""
import os
import subprocess
import sys
    ...
    with factory() as session:
        ...
        if existing is None:
            session.add(AgentRecord(
                agent_id=AGENT_ID, name="Демо-агент дня 15",
...
def phase_2(manager, agent) -> list:
    ...
    # Прямой ход из планирования в выполнение закрыт guard-условием: план должен
    # быть утверждён (день 15). Флаг — то же подтверждение, что и чекбокс в UI.
    manager.set_task_flags(TASK_ID, {FLAG_PLAN_APPROVED: True})
    manager.advance_task_step(TASK_ID)
    lines += [
        f"Флаг «план утверждён», затем `advance_task_step`: "
        f"**{state_line(manager, TASK_ID)}** — задача вернулась на сохранённый шаг "
        "планирования, затем пошла дальше.",
        "",
    ]
    return lines
...
def phase_3(manager, agent) -> list:
    ...
    # Выход из выполнения на валидацию тоже закрыт согласованием этапа.
    manager.set_task_flags(TASK_ID, {FLAG_IMPLEMENTATION_COMPLETE: True})
    done = agent.generate("готово")
    lines += reply_block("Шаг 1 — реплика «готово» (после флага реализации)", "готово", done)
...
def phase_4(manager, agent) -> list:
    ...
    # Откат сбросил согласования этапа-цели и последующих: подтверждаем реализацию
    # заново — иначе выход на валидацию закрыт (день 15).
    manager.set_task_flags(TASK_ID, {FLAG_IMPLEMENTATION_COMPLETE: True})
    for _ in range(4):
        manager.advance_task_step(TASK_ID)
    lines += [
...
def write_header(offline: bool) -> None:
    ...
    REPORT.write_text(
        "\n".join([
            "# Отчёт дня 15 — состояние задачи как FSM",
            "",
...
def main(argv: list) -> int:
    ...
    if "--all" in argv:
        if not offline and not config.resolve_api_key():
            print(
                "Нет ключа DEEPSEEK_API_KEY (day15/.env или переменная окружения). "
