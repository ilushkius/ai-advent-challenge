"""Бэкенд автопроверки кадров сценария видео дня 15 (``video_scenario.py``).
...
class UiServer:
    ...
    фронтенд читает адрес бэкенда из ``DAY15_BACKEND_URL`` (иначе ушёл бы на
    ...
    def start(self) -> str:
        ...
        env = dict(os.environ, DAY15_BACKEND_URL=self.backend_url)
        self.process = subprocess.Popen(
