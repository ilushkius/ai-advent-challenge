"""UI-кадры сценария видео дня 15: реальный рендер Streamlit через AppTest.
...
Границы.
    Бэкенд поднимает оркестратор: адрес прогона лежит в ``DAY15_BACKEND_URL``
...
# Скрипт лежит в day15/scripts/, а приложение — в корне дня: добавляем корень дня
