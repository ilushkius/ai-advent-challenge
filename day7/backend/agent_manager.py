"""AgentManager дня 7 — синглтон, пул агентов с контекстной памятью.
...
- ``remove_agent`` удаляет строку агента (сообщения удаляются каскадом);
...
from .database import AgentRecord, Message
...
class AgentManager:
    ...
    def remove_agent(self, agent_id: str) -> bool:
        ...
        if existed:
            with self._session() as session:
                # messages удаляются первыми: даже без FK-каскада не остаётся
                # осиротевших реплик (агент — источник правды в agents).
