"""FastAPI-приложение дня 7: агенты с контекстной памятью в SQLite.

Запуск из папки day7/:  .venv/Scripts/python -m uvicorn backend.main:app --port 8000
Swagger-документация:  http://127.0.0.1:8000/docs

При старте (lifespan) создаются таблицы SQLite и агенты восстанавливаются из
базы вместе с историей диалогов. День 7 отличается от дня 6:
- история агента — это диалог (реплики user/assistant в messages), а не
  записи-попытки; GET history возвращает сообщения по возрастанию времени;
- POST generate возвращает обновлённую историю в поле messages;
- новый эндпоинт DELETE /agents/{agent_id}/history очищает диалог.
...
app = FastAPI(
    title="Агенты DeepSeek с памятью — День 7",
    description=(
...
def root():
    return {
        "name": "Агенты DeepSeek с памятью — День 7",
        "docs": "/docs",
