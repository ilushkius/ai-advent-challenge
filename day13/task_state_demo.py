...
Запуск из папки дня (обязательно: пути к БД и .env считаются от ``backend/``):
    ...
    python task_state_demo.py --reset          # удалить демо-БД и завести отчёт
    python task_state_demo.py --phase 2        # одна фаза в отдельном процессе
    python task_state_demo.py --all            # все фазы, каждая — новый процесс
...
Каждая фаза дописывает в ``task_state_demo.md`` раздел ``## Фаза N``: таблицу
...
DEMO_DB = HERE / "task_state_demo.db"
REPORT = HERE / "task_state_demo.md"
