"""Класс Agent дня 13 — агент с тремя слоями памяти, контролем токенов и профилем.
...
from .task_fsm import InvalidTaskTransition, TaskStage, UnknownTaskEvent
from .task_intent import TaskIntent, classify_task_intent
...
class Agent:
    ...
    def apply_task_intent(self, prompt: str) -> Optional[str]:
        """Обновляет состояние задачи по реплике пользователя (день 13).

        Возвращает значение ``TaskIntent``, если переход применён, иначе
        ``None`` (намерения нет, задача не заведена, задача завершена или
        переход недопустим для текущего этапа). Недопустимый переход не роняет
        генерацию: он пишется в лог, и диалог продолжается как обычно.
        """
        ...
        try:
            if intent is TaskIntent.PAUSE:
                self.task_state_machine.pause(self.task_id)
            elif intent is TaskIntent.RESUME:
                self.task_state_machine.resume(self.task_id)
            elif intent is TaskIntent.ADVANCE:
                self.task_state_machine.advance_step(self.task_id)
            else:
                target = state["rollback_stage"]
                if target is None:
                    return None
                self.task_state_machine.rollback(
    ...
    def _make_client(self):
        ...
        if not api_key:
            raise AgentError(
                "Ключ API не задан: укажите DEEPSEEK_API_KEY в файле day13/.env "
    ...
    def _system_message(self, summary_text: str = "",
        ...
        """Системное сообщение агента: профиль + роль + блоки памяти/задачи.

        Порядок блоков: ПРОФИЛЬ ПОЛЬЗОВАТЕЛЯ (персонализация дня 12) →
        системный промпт агента → рабочая память → долговременная память →
        конспект → факты → СОСТОЯНИЕ ЗАДАЧИ (день 13). Профиль идёт первым: это
        постоянная инструкция пользователя, одинаковая во всех запросах, и она
        не должна теряться за блоками памяти. Состояние задачи идёт последним:
        это КОНКРЕТНАЯ точка, с которой надо продолжать, и она должна читаться
        сразу после общей рамки. Все блоки вкладываются в существующее
        system-сообщение, а не добавляются отдельными: так поведение не зависит
        от того, как провайдер обрабатывает несколько system-сообщений подряд.
        """
        parts = []
    ...
    def generate(self, prompt: str) -> dict:
        ...
        # запроса уже описывает новое состояние.
        self.apply_task_intent(prompt)
