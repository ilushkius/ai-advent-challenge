"""Слой агентов дня 13: агент, его память, профиль и менеджер пула.

- ``agent`` — класс ``Agent`` (память, токены, контекст, стратегии, факты,
  ветки, профиль, состояние задачи);
...
- ``manager_agents`` / ``manager_context`` / ``manager_memory`` /
  ``manager_profiles`` / ``manager_tasks`` / ``manager_usage`` — миксины
  ``AgentManager`` по доменам.
"""
