...
Что это.
    ...
    Диаграмма переходов (единственный источник правды, см. TRANSITIONS в тестах):
        ...
        PLANNING|EXECUTION|VALIDATION|DONE --PAUSE--> PAUSED (шаг сохраняется)
...
Как запустить.
    ...
    кроме стандартной библиотеки. Проверяется тестами:
        ...
        # из папки day13
        python -m pytest -q tests/test_task_fsm.py
...
class InvalidTaskTransition(Exception):
    """Переход недопустим (например, ADVANCE из этапа done)."""
...
# Таблица допустимых переходов между этапами. ADVANCE по шагам внутри этапа и
# поведение paused разбирают классы ниже; эта таблица отвечает на вопрос
# «можно ли вообще попасть из этапа A в этап B».
STAGE_TRANSITIONS: dict[TaskStage, frozenset[TaskStage]] = {
    ...
    TaskStage.DONE: frozenset({TaskStage.PAUSED}),
...
def first_step(stage: TaskStage) -> TaskStep:
    ...
    steps = STAGE_STEPS[stage]
    if not steps:
        raise InvalidTaskTransition(f"у этапа {stage.value} нет собственных шагов")
    return steps[0]
...
def is_valid_transition(from_stage: str | TaskStage, to_stage: str | TaskStage) -> bool:
    """Допустим ли переход между этапами (аргументы — строки или члены Enum)."""
    source = stage_from_value(from_stage) if isinstance(from_stage, str) else from_stage
    target = stage_from_value(to_stage) if isinstance(to_stage, str) else to_stage
    return target in STAGE_TRANSITIONS[source]
...
class PlanningState(TaskStageBase):
    ...
    def handle(self, event: TaskEvent, step: TaskStep) -> tuple[TaskStage, TaskStep]:
        ...
        if event is TaskEvent.ROLLBACK:
            raise InvalidTaskTransition("у этапа planning нет предыдущего этапа")
...
class DoneState(TaskStageBase):
    ...
    Этап терминальный: вперёд идти некуда, откат не описан осознанно (задача
    сдана). Единственный переход наружу — пауза, из которой возврат ведёт в
    ``done`` с тем же шагом ``finalize``.
    """
    ...
    def handle(self, event: TaskEvent, step: TaskStep) -> tuple[TaskStage, TaskStep]:
        if event is TaskEvent.PAUSE:
            return (TaskStage.PAUSED, step)
        if event is TaskEvent.ADVANCE or event is TaskEvent.ROLLBACK:
            raise InvalidTaskTransition("задача завершена: этап done терминальный")
        return super().handle(event, step)
