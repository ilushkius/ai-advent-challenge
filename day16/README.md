# День 16 — MCP-клиент: инструменты внешнего сервера (+ память, профиль, задача, инварианты и переходы)

Задание дня — **подключить MCP** (Model Context Protocol) и получить список
инструментов от внешнего MCP-сервера. Приложение подключается к общедоступному
серверу официального набора — fetch (`uvx mcp-server-fetch`) или filesystem
(`npx -y @modelcontextprotocol/server-filesystem .`) — по stdio, либо к
HTTP-серверу (Streamable HTTP, `sse://`), и показывает каталог его инструментов:
`name`, `description`, `input_schema` и количество. Подключение одно на процесс,
его состояние описано стейт-машиной, а ошибки соединения превращаются в понятную
строку вместо traceback. Вызова инструментов в этом дне **нет** — задача была
установить соединение и прочитать список (`get_tools`).

Живой прогон дня — [docs/reports/mcp_demo.md](docs/reports/mcp_demo.md): какой
сервер, какой командой, что вернул `tools/list`, какие проблемы встретились.

Структура дня 15 сохранена полностью: инварианты агента, состояние задачи как
конечный автомат с **контролируемыми переходами**, персонализация профилем
пользователя, три слоя памяти, четыре стратегии контекста и сжатие истории в
конспект. Новая функциональность надстроена над ними: приложение развивает день
15, а `day15/`, `day14/`, `day13/` и более ранние дни остаются снимками.
...
Код дней 1–15 не изменялся — их файлы не тронуты; изменения только внутри
`day16/` плюс общий пакет `shared/` в корне репозитория, куда вынесен код, не
...
| Новое | Где |
|---|---|
| Зависимость `mcp` (MCP Python SDK 2.x) — единственная новая прямая зависимость дня | `pyproject.toml`, `uv.lock` |
| `MCPClient`: соединение (stdio / Streamable HTTP / SSE), `list_tools`, `disconnect`, свой цикл событий и долгоживущая задача сессии | `backend/services/mcp_client.py` |
| Понятные тексты ошибок подключения и разворачивание `ExceptionGroup` транспорта | `backend/services/mcp_errors.py` |
| Реестр одного MCP-подключения на процесс (смена соединения, статус, закрытие) | `backend/services/mcp_registry.py` |
| Стейт-машина подключения: `MCPConnectionState`/`MCPConnectionEvent`, граф переходов, `UnknownMCPConnectionEvent` | `backend/domain/mcp_connection_fsm.py` |
| Разбор цели: выбор транспорта по виду строки, команда запуска с кавычками и Windows-путями | `backend/domain/mcp_target.py` |
| Структура инструмента контракта (`name`, `description`, `input_schema`) и её нормализация | `backend/domain/mcp_tools.py` |
| Схемы API `MCPConnectIn`, `MCPStatusResponse`, `MCPToolSchema`, `MCPToolsResponse` | `backend/schemas/mcp.py` |
| Эндпоинты `GET /mcp/status`, `POST /mcp/connect`, `POST /mcp/disconnect`, `GET /mcp/tools` | `backend/api/mcp.py` |
| Раздел «🔌 MCP»: цель, транспорт, кнопки подключения и отключения, статус, таблица инструментов, обновление списка | `frontend/mcp_section.py`, `frontend/mcp_api.py` |
| Минимальный консольный скрипт `uv run python scripts/mcp_demo.py` (+ `--json`, `--target`, `--transport`, `--timeout`) | `scripts/mcp_demo.py` |
| Тестовый MCP-сервер для офлайн-проверки настоящего stdio-соединения | `tests/mcp_echo_server.py` |
| Настройки MCP: цель по умолчанию, альтернатива, таймаут, предел страниц, длина цели | `backend/core/config.py` |
| Отчёт дня | `docs/reports/mcp_demo.md` |

Наследовано из дня 15 (описано ниже как есть): контролируемые переходы состояния
задачи с журналом отклонённых попыток; из дня 14 — инварианты проекта; из дня 13 —
состояние задачи как конечный автомат; из дней 11–12 — три слоя памяти, четыре
стратегии, сжатие в конспект, ветки, факты, метрики токенов и персонализация
...
MCP-клиент — то, чем день 16 отличается от дня 15. Подключиться можно из
интерфейса, по HTTP или из кода.

### Из интерфейса

Раздел «🔌 MCP» (переключатель вверху страницы): поле «URL или команда запуска
MCP-сервера» (по умолчанию `uvx mcp-server-fetch`), селектор транспорта
...
строка деталей (цель, транспорт, допустимые события) и таблица инструментов
`name` / `description` / `input_schema`; кнопка «🔄 Обновить список инструментов»
просит сервер заново (`refresh=true`), а раскладка «🧾 Полная input_schema
инструмента» показывает схему аргументов целиком. Пока соединения нет, раздел
показывает подсказку и не рисует пустой список: «не подключено» и «список пуст» —
разные состояния.
...
curl http://127.0.0.1:8000/mcp/tools          # {"tools": [...], "count": 1, ...}
curl http://127.0.0.1:8000/mcp/status         # состояние подключения и сервер
...
client = MCPClient("uvx mcp-server-fetch")   # для HTTP — "http://host:port/mcp"
client.connect()                             # initialize + согласование протокола
for tool in client.list_tools():             # tools/list: name, description, input_schema
    print(tool.name)
...
|---|---|---|
| stdio | `uvx mcp-server-fetch`, `npx -y @modelcontextprotocol/server-filesystem .` | сервер — дочерний процесс, обмен JSON-RPC по stdin/stdout |
...
Подробности — [docs/architecture.md](docs/architecture.md) (раздел
«MCP-интеграция»), эндпоинты — [docs/api.md](docs/api.md), доказательства —
[docs/reports/mcp_demo.md](docs/reports/mcp_demo.md), сценарии проверки —
[docs/usage.md](docs/usage.md).
...
Сценария дня 11 в `day16/` нет — вместе с ним ушли кнопка его запуска, панель
...
В `day16/` диалог ведётся вручную в чате, а доказательства дают
...
Одна строка на задачу в той же базе `day16/agents.db`. `task_id` уникален
...
Флаги в фазах 2–4 выставляются явно (`set_task_flags`): с дня 16 выход из этапа
...
Скрипт работает на отдельной базе `day16/task_state_demo.db`, заводит агента
`demo15` («Демо-агент дня 16») и задачу `tz-portal` и переиспользует
...
```bash
cd day16
uv run python scripts/seed_invariants.py            # 4 демо-правила в day16/agents.db
...
Профиль — одна строка на пользователя в той же базе `day16/agents.db`:
...
```bash
cd day16
uv run python scripts/personalization_comparison.py            # нужен DEEPSEEK_API_KEY в day16/.env
uv run python scripts/personalization_comparison.py --no-api   # офлайн-заглушка, без сети
```

Скрипт пишет `day16/docs/reports/personalization_comparison.md` и работает на отдельной базе
`day16/personalization_demo.db` (пересоздаётся при каждом прогоне).
...
* Переменная окружения адреса бэкенда: `DAY16_BACKEND_URL` (по умолчанию
...
Streamlit (порт 8501) ── HTTP (requests) ──► FastAPI (порт 8000) ──► DeepSeek API
   ...
   «📏 Инварианты» / «🔌 MCP»       └─ Agent × N
                                    ...
                                    MCPRegistry (одно подключение на процесс, в памяти)
                                         └─ MCPClient ──► MCP SDK ──► внешний MCP-сервер
                                              (stdio: uvx/npx-процесс; HTTP: Streamable HTTP / SSE)
                                              │
                                              ▼
                                    SQLAlchemy → SQLite (day16/agents.db)
...
| Домен | `backend/domain/` | Чистые правила и данные без БД, LLM и HTTP: FSM задачи, сжатия и **MCP-подключения**, граф допуска и guards переходов, стратегии, факты, слои памяти, профиль, тексты промпта, разбор цели MCP и структура инструмента |
| Доступ к данным | `backend/storage/` | Движок и сессии SQLite, реэкспорт ORM, `TaskStateStore`, преобразования ORM-строк в словари |
| Прикладные сервисы | `backend/services/` | Оркестрация домена и хранилища: суммаризация (`ContextCompressor`), переходы задачи (`TaskStateMachine`) и **MCP** (`MCPClient`, `MCPRegistry`, `mcp_errors`) |
...
| Утилиты | `backend/utils/` | Слой объявлен обязательным, но в дне 16 пуст: общий код живёт в repo-level `shared/` |
...
```
day16/
├── app.py               # Streamlit, точка входа (62 строки): set_page_config →
│                        # common.init_state() → sidebar.render_sidebar() → chat_section.render_main_area()
├── frontend/            # интерфейс по секциям (13 модулей, карта — в STRUCTURE.md)
│   ├── api_client.py    # HTTP-транспорт бэкенда (requests): BACKEND_URL, BackendError, request_json
│   ├── mcp_api.py       # запросы MCP-раздела: /mcp/status, /mcp/connect, /mcp/disconnect, /mcp/tools
│   ├── common.py        # подписи (TASK_STAGE_LABELS, MCP_STATE_LABELS), run_task_action, st.session_state
...
│   ├── mcp_section.py         # раздел «🔌 MCP»: подключение, статус, таблица инструментов (день 16)
...
│   ├── mcp_demo.py      # день 16: подключение к MCP-серверу и печать списка инструментов (--json/--target/--timeout)
...
├── backend/
│   ├── api/             # FastAPI: 7 роутеров по доменам + main.py (сборка app, 78 строк)
│   ├── core/            # config.py (настройки, пути .env и agents.db, цель MCP), dependencies.py (get_manager, get_mcp_registry, *_or_404)
...
│   │                    # demo_invariants, mcp_connection_fsm, mcp_target, mcp_tools
│   ├── services/        # compressor (суммаризация и конспект), task_state (контракт переходов и отказ),
│   │                    # invariant_checker (правила → LLM), mcp_client (MCPClient), mcp_errors,
...
├── tests/               # pytest: 1097 тестов в подпапках unit/ (16 файлов), integration/ (19), e2e/ (8)
...
├── docs/
│   ├── architecture.md  # компоненты и модульная структура, схема БД, слои памяти, персонализация, FSM, переходы, инварианты, MCP-интеграция
│   ├── api.md           # 57 эндпоинтов с примерами и кодами ошибок
│   ├── usage.md         # инструкция дня 16: установка и запуск, подключение к MCP, список инструментов, отключение, сценарии проверки
│   └── reports/         # mcp_demo.md (день 16) + унаследованные controlled_transitions_demo.md, task_state_demo.md, personalization_comparison.md
...
├── conftest.py          # добавляет корень day16 в sys.path (импорт `from backend.agents.agent import Agent`)
...
├── uv.lock                # точные версии всех пакетов (77) — фиксируется в Git
...
Инструкция дня 16 (установка и запуск, подключение к MCP-серверу, список
инструментов, отключение, сценарии проверки) — в [docs/usage.md](docs/usage.md);
эндпоинты с примерами — в [docs/api.md](docs/api.md); как всё устроено внутри — в
[docs/architecture.md](docs/architecture.md); Swagger — на
`http://127.0.0.1:8000/docs`.

Для MCP-серверов нужны внешние команды: `uvx` (идёт с `uv`) — для
`uvx mcp-server-fetch`, `npx` (Node ≥ 18) — для
`npx -y @modelcontextprotocol/server-filesystem .`. Ключ DeepSeek для MCP-раздела
не нужен: серверы запускаются локально, инструменты только читаются. Проверка
одной командой:

```bash
uv run python scripts/mcp_demo.py    # connect → list_tools → disconnect в консоли
```

Доказательства: MCP — [docs/reports/mcp_demo.md](docs/reports/mcp_demo.md)
(живой прогон на двух серверах, включая ошибки и их решения); унаследованные —
контролируемые переходы
[docs/reports/controlled_transitions_demo.md](docs/reports/controlled_transitions_demo.md),
состояние задачи [docs/reports/task_state_demo.md](docs/reports/task_state_demo.md),
...
В этой рабочей копии так и получилось: `day16/.agents/skills/` содержит
...
Скиллы видны агенту omp, если сессия запущена **из папки `day16`** (провайдер
...
(`skills.customDirectories: day16/.agents/skills`) — там же он ищет только на
...
```bash
cd day16
uv run pytest -q      # 1097 тестов
```

1097 тестов = 1013 унаследованных из дня 15 (контролируемые переходы, инварианты,
состояние задачи, персонализация, память, сжатие) плюс **84** случая на MCP.
Файлов: `unit/` — 16, `integration/` — 19, `e2e/` — 8.

Новые файлы дня 16 (MCP):
...
Фейк MCP-клиента (`FakeMCPClient` в `tests/support.py`) повторяет публичный
контракт `MCPClient`, но с настоящей FSM подключения: тесты API не поднимают
внешних серверов, а `tests/integration/test_mcp_stdio.py` проверяет реальное
stdio-соединение на своём сервере из `tests/mcp_echo_server.py` (сеть не нужна).
...
* **Подключение к внешнему MCP-серверу**: цель — URL (`http://` — Streamable
  HTTP, `sse://` — SSE) или команда запуска по stdio (`uvx mcp-server-fetch`,
  `npx -y @modelcontextprotocol/server-filesystem .`); транспорт выбирается по
  виду цели, явный выбор его переопределяет.
* **Список инструментов по контракту дня**: `name`, `description`,
  `input_schema` и количество — в ответе `GET /mcp/tools` и в таблице раздела
  «🔌 MCP»; полная схема аргументов — в раскладке «🧾 Полная input_schema
  инструмента». Инструменты только читаются: вызовов в дне нет.
* **Стейт-машина подключения**: `disconnected → connecting → connected` и
  `error` при сбое; `GET /mcp/status` показывает состояние, сервер, версию
  протокола, число инструментов, текст последней ошибки и допустимые события
  (`allowed_events`), а недопустимый шаг — явная ошибка.
* **Понятные ошибки вместо traceback**: одна строка «что делали → что случилось
  → что проверить» (нет команды в PATH, истёк таймаут, проверьте URL) — её
  показывают красная плашка интерфейса, `detail` ответа API (`400`/`409`/`502`)
  и консольный скрипт.
...
* **Минимальный скрипт для видео**: `uv run python scripts/mcp_demo.py`
  (`--target`, `--transport`, `--timeout`, `--json`) — четыре строки кода
  `connect` → `list_tools` → `disconnect` и читаемый вывод; ошибка подключения
  печатается одной строкой, код выхода 1.
* **Доказательство прогона**: [docs/reports/mcp_demo.md](docs/reports/mcp_demo.md) —
  два общедоступных сервера (fetch: 1 инструмент, filesystem: 14), ходы
  соединения через API и разбор ошибок.
...
* **Стенд для съёмки видео**: `uv run python scripts/video_scenario.py --ui`
  ...
  (своя БД, свой порт, `DAY16_BACKEND_URL`), открывает адрес в браузере и
...
- **Одно MCP-подключение на процесс.** Подключение к другому серверу закрывает
  прежнее — реестра по имени у дня нет.
- **Инструменты не вызываются.** День читает каталог (`tools/list`);
  `tools/call`, превращение инструментов в функции LLM и политика разрешений —
  следующий шаг.
- **Живой проверки HTTP/SSE-транспорта нет.** Обе ветки покрыты тестами разбора
  цели и ошибок, но прогон дня шёл на stdio-серверах (`mcp-fetch`,
  `server-filesystem`): общедоступного MCP-сервера с открытым HTTP-эндпоинтом без
  ключа не брали.
- **stdio-сервер зависит от внешних команд.** `uvx mcp-server-fetch` требует `uv`,
  `npx …server-filesystem` — Node ≥ 18; без команды подключение падает с
...
- **Наследованные скрипты прогона кадров дня 15** (`scripts/video_scenario*.py`)
  ...
  инструкции, которого в `usage.md` дня 16 нет: файл описывает только MCP.
- Один процесс бэкенда и один файл `day16/agents.db` (в `.gitignore` по `*.db`).
  Прогон отчёта персонализации работает на отдельной базе
  `day16/personalization_demo.db`, демонстрация состояния задачи — на
  `day16/task_state_demo.db`, демонстрация контролируемых переходов — на
  `day16/controlled_transitions_demo.db`, автопроверка кадров сценария видео —
  на `day16/video_scenario.db` (плюс временный `video_scenario.db.pid` с pid
