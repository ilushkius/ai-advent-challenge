"""Ошибки MCP-клиента и понятные тексты (день 16).
...
400/409/502) и консольный скрипт ``scripts/mcp_demo.py``. Здесь же лежит
разворачивание ``ExceptionGroup``: транспорты MCP SDK запускают сервер в
task-group anyio, поэтому ``FileNotFoundError`` (команды нет в PATH) и таймаут
приходят завёрнутыми, а показать пользователю нужно причину, а не обёртку.
