"""MCP-клиент дня 16: соединение с MCP-сервером и список его инструментов.
...
Вызова инструментов здесь нет: день 16 только читает каталог (``tools/list``).
"""
...
from ..domain.mcp_tools import MCPToolInfo, make_tool_info
...
#: Запас поверх таймаута SDK: ожидание ответа служебного цикла из чужого потока.
SUBMIT_GRACE = 10.0
...
class MCPClient:
    ...
    def close(self) -> None:
        ...
        if thread is not None:
            thread.join(timeout=self._limit())
    ...
    def _spawn(self) -> None:
        ...
        try:
            self._serving = asyncio.run_coroutine_threadsafe(
                self._serve(self._shutdown, self._ready), loop
            )
    ...
    async def _serve(self, shutdown: asyncio.Event, ready: threading.Event) -> None:
        ...
        try:
            async with contextlib.AsyncExitStack() as stack:
                read, write = await stack.enter_async_context(self._transport_context())
    ...
    def _abort(self, action: str, exc: BaseException | None = None) -> MCPError:
        ...
        return MCPToolsError(message) if action == "tools" else MCPConnectionError(message)
    ...
    def _transport_context(self) -> AsyncContextManager[Any]:
        """Контекст транспорта: stdio (дочерний процесс), SSE или Streamable HTTP."""
        if self._target.transport is MCPTransport.STDIO:
            params = StdioServerParameters(
                command=self._target.command, args=list(self._target.args)
            )
            return stdio_client(params)
        if self._target.transport is MCPTransport.SSE:
            return sse_client(self._target.url)
        return streamable_http_client(self._target.url)

    def _record_server(self, init: Any) -> None:
        """Запоминает имя, версию и протокол сервера из ``InitializeResult``."""
    ...
    def _submit(self, coro: Any) -> Any:
        ...
        try:
            return future.result(timeout=self._limit())
    ...
    def _ensure_loop(self) -> asyncio.AbstractEventLoop:
        """Поднимает поток с циклом событий (один на клиент, daemon)."""
        if self._loop is not None and not self._loop.is_closed():
            return self._loop
        loop = asyncio.new_event_loop()
        started = threading.Event()
        thread = threading.Thread(
            target=_run_loop, args=(loop, started),
            name=f"mcp-{self._target.transport.value}", daemon=True,
        )
        thread.start()
