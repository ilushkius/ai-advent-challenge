"""Конфигурация бэкенда дня 16: DeepSeek, дефолты агентов, стратегии, память, профиль.
...
1. файл `.env` в папке дня (day16/backend/core/config.py -> day16/.env);
...
# --- MCP-клиент (день 16) ----------------------------------------------------
# Цель подключения по умолчанию для UI и scripts/mcp_demo.py: fetch-сервер
# официального набора MCP — запускается локально по stdio через uvx (пакет
# ставится из PyPI, Node не нужен).
MCP_DEFAULT_TARGET = "uvx mcp-server-fetch"
# Альтернатива из задания дня — файловый сервер официального набора (нужен Node,
# команда запуска скачивает пакет через npx):
...
# Персистентное хранилище дня: SQLite-файл в папке дня (day16/agents.db).
...
# Путь к .env дня: этот файл лежит в day16/backend/core/, значит .env — в day16/.
...
def resolve_api_key():
    """Возвращает ключ DeepSeek: day16/.env → переменная окружения → None."""
