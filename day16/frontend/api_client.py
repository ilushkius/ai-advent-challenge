"""HTTP-клиент фронтенда дня 16: запросы к бэкенду FastAPI.

Все обращения интерфейса к бэкенду идут через ``_request``: он собирает URL из
``BACKEND_URL`` (переопределяется переменной окружения ``DAY16_BACKEND_URL``),
...
проверка текста (``api_check_invariants``). Запросы MCP-раздела дня 16 живут
...
BACKEND_URL = os.environ.get("DAY16_BACKEND_URL", "http://127.0.0.1:8000")
...
def request_json(method, path, **kwargs):
    ...
    try:
        ...
        raise BackendError(
            f"Бэкенд недоступен ({BACKEND_URL}). Запустите его из папки day16/: "
