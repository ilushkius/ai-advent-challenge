"""Раздел «🔌 MCP» дня 16: подключение к MCP-серверу и его инструменты.
...
- таблицу инструментов (``name``, ``description``, ``input_schema``) и кнопку
...
#: Цель по умолчанию — fetch-сервер официального набора MCP (ставится через uvx,
#: Node не нужен). Значение то же, что в ``backend/core/config.MCP_DEFAULT_TARGET``:
#: frontend не импортирует backend, поэтому строка продублирована осознанно.
DEFAULT_TARGET = "uvx mcp-server-fetch"
...
def _render_schema_viewer(tools: list) -> None:
    """Полная схема аргументов выбранного инструмента (``st.json``)."""
...
def _render_tools(payload: dict) -> None:
    """Таблица инструментов и разбор полной схемы аргументов."""
    st.subheader("🧰 Инструменты MCP-сервера")
    st.caption(
        "Каталог инструментов сервера (`tools/list`): имя, описание и JSON "
        "Schema аргументов. День 16 только читает список — вызовов нет."
    )
...
def _render_connect_form(status: dict) -> None:
    ...
    if not status.get("connected"):
        st.info(
            ...
            "список инструментов появится ниже. Пример: `uvx mcp-server-fetch` "
            "(stdio) или `http://127.0.0.1:9000/mcp` (HTTP)."
        )
        return
...
def render_mcp_section() -> None:
    """Раздел «🔌 MCP»: подключение к серверу, статус и список инструментов."""
    st.title("🔌 MCP: инструменты внешнего сервера")
    st.caption(
        "MCP (Model Context Protocol) — способ показать агенту внешние "
        "инструменты: файловую систему, веб-запросы, базы данных. День 16 "
        "подключается к любому общедоступному MCP-серверу (stdio, SSE или "
        "Streamable HTTP) и показывает его каталог `tools/list`. Инструменты "
        "здесь только читаются — вызовов в работе агента нет."
    )
    ...
    status = _load_status()
    if status is None:
        st.warning("MCP-раздел работает через бэкенд. Запустите его из папки day16/:")
