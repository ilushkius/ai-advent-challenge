...
from backend.domain.task_state_machine import FLAG_PLAN_APPROVED, FLAG_VALIDATION_PASSED
...
def test_create_task_activates_it_on_the_agent(manager):
    ...
    assert "Текущий этап задачи: execution" in agent.task_state_block()
...
def test_advance_pause_resume_rollback_roundtrip(manager):
    ...
    manager.set_task_flags(TASK_ID, {FLAG_PLAN_APPROVED: True})  # выход в execution
    for _ in range(3):
...
def test_set_task_flags_opens_the_next_stage(manager):
    """Флаги guard-условий выставляются через менеджер и открывают переход."""
    manager.create_task(AGENT_ID, TASK_ID)
    assert manager.get_task_state(TASK_ID)["allowed_next"] == [TaskStage.PAUSED.value]

    state = manager.set_task_flags(TASK_ID, {FLAG_PLAN_APPROVED: True})
    assert state["context"][FLAG_PLAN_APPROVED] is True
    assert TaskStage.EXECUTION.value in state["allowed_next"]


def test_refused_transition_is_journaled_by_the_manager(manager):
    """Отклонённая попытка видна в журнале менеджера и состояние не меняет."""
    manager.create_task(AGENT_ID, TASK_ID)
    with pytest.raises(InvalidTransitionError):
        manager.transition_task(TASK_ID, TaskStage.DONE.value)
    ...
    assert manager.get_task_state(TASK_ID)["stage"] == TaskStage.PLANNING.value
...
def test_list_active_tasks_excludes_completed(manager):
    ...
    manager.set_task_flags("tz2", {FLAG_VALIDATION_PASSED: True})
...
def test_history_reasons_follow_operations(manager):
    ...
    assert [entry["reason"] for entry in accepted] == [
        "задача создана",
        "следующий шаг",
        "пауза",
...
def test_transition_task_default_reason_and_completion(manager):
    ...
    manager.set_task_flags(TASK_ID, {"implementation_complete": True})
