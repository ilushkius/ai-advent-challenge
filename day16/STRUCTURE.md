...
Карта модулей дня 16: что где лежит и за что отвечает. Правила структуры — в
...
День 16 — копия дня 15 (`day15/` не изменяется, это снимок) плюс **MCP-клиент**:
подключение к внешнему MCP-серверу (stdio / Streamable HTTP / SSE) и список его
инструментов (`tools/list` → `name`, `description`, `input_schema`). Подключение
одно на процесс, его состояние описано стейт-машиной
(`backend/domain/mcp_connection_fsm.py`), ошибки соединения превращаются в
понятную строку. Вызова инструментов в дне нет. Унаследованное сохранено
полностью: контролируемые переходы задачи и инварианты (день 15), состояние
задачи как конечный автомат (день 13), персонализация профилем (день 12), три
слоя памяти и четыре стратегии контекста (день 11).

MCP-подсистема не имеет таблиц в БД: соединение живёт в памяти процесса
(`MCPRegistry`), поэтому схема дня не меняется.

## Раскладка

```
day16/
├── app.py                    # точка входа Streamlit (62 строки): set_page_config + вызовы секций
├── frontend/                 # Streamlit UI по секциям (13 модулей + __init__.py)
├── backend/                  # FastAPI-бэкенд, домен и доступ к данным
│   ├── __init__.py           # описание пакета + корень репозитория в sys.path (для shared/)
│   ├── api/                  # FastAPI: 7 роутеров по доменам + сборка app (main.py)
│   ├── core/                 # config, dependencies — настройки и доступ к менеджеру и MCP-реестру
│   ├── domain/               # чистые правила и данные: FSM задачи/сжатия/MCP, граф переходов, стратегии, профиль, инварианты, тексты
│   ├── services/             # прикладные сервисы: compressor, task_state, invariant_checker, mcp_client, mcp_errors, mcp_registry
...
├── tests/                    # pytest: 1097 тестов (unit/, integration/, e2e/)
...
├── pyproject.toml, uv.lock   # зависимости (uv): прямые — в pyproject (включая mcp), точные версии — в локе
...
Отчёты прогонов лежат в `docs/reports/`: `mcp_demo.md` — артефакт дня 16,
`controlled_transitions_demo.md` (день 15), `task_state_demo.md` (день 13) и
...
|---|---|---|
| `core/` | `config.py` (188), `dependencies.py` (67), `__init__.py` (22) | Настройки дня и зависимости роутов: `get_manager`, `get_mcp_registry`, `agent_or_404`, `task_or_404`, `invariant_or_404` |
| `domain/` | 20 модулей + `__init__.py` (292) | Чистые правила и данные: FSM задачи, сжатия и MCP-подключения, граф допуска и guards переходов, стратегии, факты, слои памяти, профиль, инварианты, тексты промпта, разбор цели MCP и структура инструмента. Знают только stdlib, `core.config` и соседей по слою |
...
| `services/` | `compressor.py` (329), `task_state.py` (392), `invariant_checker.py` (296), `mcp_client.py` (388), `mcp_errors.py` (84), `mcp_registry.py` (123), `__init__.py` (86) | `ContextCompressor`, `TaskStateMachine` (граф, guards, единая точка отказа), `InvariantChecker`, `MCPClient` (соединение с MCP-сервером) и `MCPRegistry` (одно подключение на процесс): оркестрация домена и хранилища |
...
| `schemas/` | `agent.py` (331), `context.py` (218), `memory.py` (173), `profile.py` (170), `task.py` (170), `invariant.py` (149), `mcp.py` (86), `__init__.py` (174) | Pydantic-схемы API по доменам, реэкспорт из `schemas/__init__.py` |
| `api/` | `agents.py` (266), `context.py` (140), `memory.py` (180), `profiles.py` (126), `tasks.py` (246), `invariants.py` (139), `mcp.py` (117), `main.py` (78), `__init__.py` (15) | 57 эндпоинтов по доменам и сборка `app` |
...
## MCP (день 16) — новые модули

| Модуль | Строк | Назначение |
|---|---|---|
| `backend/domain/mcp_connection_fsm.py` | 170 | Стейт-машина подключения: `MCPConnectionState`/`MCPConnectionEvent` (`Enum`), классы состояний с `handle(event)`, `ALLOWED_TRANSITIONS`, `allowed_events`, `UnknownMCPConnectionEvent`. Не знает ни SDK, ни HTTP |
| `backend/domain/mcp_target.py` | 156 | Разбор цели: `MCPTransport` (`auto`/`stdio`/`sse`/`http`), `MCPTarget`, `detect_transport`, `parse_target`/`parse_transport`, `split_command` (кавычки и обратные слэши Windows-путей), `MCPTargetError` |
| `backend/domain/mcp_tools.py` | 71 | Структура инструмента контракта: `MCPToolInfo` (`name`, `description`, `input_schema`, `to_dict` с глубокой копией), `make_tool_info`/`make_tool_infos`, `MCPToolError` |
| `backend/services/mcp_errors.py` | 84 | Ошибки и тексты MCP: `MCPError`/`MCPConnectionError`/`MCPNotConnectedError`/`MCPToolsError`, `ACTION_LABELS`, `leaf_errors` (разворачивание `ExceptionGroup`), `error_message` |
| `backend/services/mcp_client.py` | 388 | `MCPClient`: свой поток с циклом событий и долгоживущая задача `_serve` (контексты MCP SDK входят и выходят в одной задаче), выбор транспорта, `connect`/`list_tools(refresh)`/`disconnect`/`close`, постраничный `tools/list`, синхронный и потокобезопасный интерфейс |
| `backend/services/mcp_registry.py` | 123 | `MCPRegistry`: одно активное подключение процесса — `connect` (закрывает прежнее), `disconnect`, `tools`, `status`, `close`; синглтон `get_mcp_registry` |
| `backend/schemas/mcp.py` | 86 | `MCPConnectIn`, `MCPStatusResponse`, `MCPToolSchema` (`from_info`), `MCPToolsResponse` |
| `backend/api/mcp.py` | 117 | Роутер `/mcp`: connect / disconnect / status / tools; `400` — цель не разобрана, `409` — нет соединения, `502` — сервер недоступен |
| `frontend/mcp_section.py` | 234 | Раздел «🔌 MCP»: цель, селектор транспорта, кнопки подключения и отключения, плашка состояния, таблица инструментов, обновление списка, разбор полной `input_schema` |
| `frontend/mcp_api.py` | 35 | Запросы раздела поверх общего транспорта `request_json` из `frontend/api_client.py` |
| `scripts/mcp_demo.py` | 119 | Минимальная демонстрация: `connect` → `list_tools` → `disconnect`, читаемый вывод и `--json`; ошибка подключения — одной строкой, код выхода 1 |
| `tests/mcp_echo_server.py` | 40 | Тестовый MCP-сервер по stdio (`mcp.server.mcpserver.MCPServer`, инструменты `echo` и `add`) для офлайн-проверки настоящего соединения |

Почему MCP разложен на три домена и три сервиса. Домен отвечает за то, что можно
проверить без сети: граф подключения, разбор строки цели, нормализация полей
инструмента. Сервисы отвечают за то, что требует процессов и потоков: `MCPClient`
(соединение), `mcp_registry` (жизненный цикл одного подключения в приложении) и
`mcp_errors` (единый текст ошибки для UI, API и консоли). Один файл на всё это дал
бы больше 700 строк и смешал бы чистые правила с anyio-задачами; кроме того,
`frontend/api_client.py` (388) и его MCP-запросы развели по разным модулям, потому
что семь доменов в одном файле уже не укладывались в лимит 400 строк.

Почему `MCPClient` держит собственный цикл событий: транспорты и сессия MCP SDK
живут в `AsyncExitStack` внутри task-group anyio, а такой контекст обязан войти и
выйти **в одной задаче**. FastAPI-роуты и Streamlit синхронные, поэтому клиент
запускает поток с циклом событий и одну долгоживущую задачу `_serve`; публичные
методы синхронные и потокобезопасные, а `list_tools` уходит отдельной задачей того
же цикла. Подробнее — раздел «MCP-интеграция» в
...
| Модуль | Строк | Назначение |
|---|---|---|
| `frontend/__init__.py` | 21 | Описание пакета; модули не выполняют `st.*` на импорте |
| `frontend/api_client.py` | 388 | HTTP-транспорт к бэкенду (`requests`), `BACKEND_URL` / `DAY16_BACKEND_URL`, `BackendError`, `request_json`, функции дней 13–16 |
| `frontend/mcp_api.py` | 35 | Запросы MCP-раздела (`/mcp/status`, `/mcp/connect`, `/mcp/disconnect`, `/mcp/tools`) поверх `request_json` |
| `frontend/common.py` | 321 | Подписи (`TASK_STAGE_LABELS`, `TASK_FLAG_LABELS`, `INVARIANT_*_LABELS`, `MCP_STATE_LABELS`, `MCP_TRANSPORT_LABELS`), форматтеры, `invariant_notice`, `stage_button_label`, `blocked_reason`, единый путь мутаций панели задачи `run_task_action`, `st.session_state`: init, флеш-сообщения, выбор активного агента |
| `frontend/sidebar.py` | 191 | Боковая панель: агенты, создание агента, стратегия, задача и сессия (`render_sidebar()`) |
| `frontend/chat_section.py` | 291 | Раздел «💬 Чат и память», переключатель пяти разделов, блок предупреждения/отказа по инвариантам над вводом, сборка страницы (`render_main_area()`) |
...
| `frontend/mcp_section.py` | 234 | Раздел «🔌 MCP»: цель, транспорт, кнопки подключения/отключения, статус, таблица инструментов, обновление списка, разбор схемы |
...
| `backend/api/mcp.py` | 117 | 4 | MCP: подключение, отключение, статус и список инструментов (`/mcp/...`) |
| `backend/api/main.py` | 78 | — | Сборка `app`: `lifespan`, CORS, `include_router` |

Всего 57 эндпоинтов (45 унаследованных дней 1–13 + 6 инвариантов дня 14 +
2 дня 15: `GET /tasks/{id}/allowed-next`, `PATCH /tasks/{id}/context` +
4 дня 16: `POST /mcp/connect`, `POST /mcp/disconnect`, `GET /mcp/status`,
`GET /mcp/tools`).
...
|---|---|---|
| `schemas/__init__.py` | 174 | Реэкспорт всех схем (импорт — из `backend.schemas`) |
...
| `schemas/mcp.py` | 86 | MCP: подключение (`MCPConnectIn`), статус (`MCPStatusResponse`), инструменты (`MCPToolSchema`, `MCPToolsResponse`) |
...
`tests/` — 1097 тестов `pytest`, офлайн (временная SQLite + фейк клиента
DeepSeek; MCP — свой stdio-сервер или фейк), разложены по трём подпапкам **по
фикстурам**: без БД и агента — `unit/`, с временной БД и `Agent` —
`integration/`, через `TestClient` — `e2e/`. `tests/conftest.py` и
`tests/support.py` остаются в корне `tests/`: на них опирается
`pythonpath = . tests` из `pytest.ini` и импорт `from support import ...`.

| Подпапка | Файлов | Тестов | Что проверяет |
|---|---|---|---|
| `tests/unit/` | 16 | 709 | Чистые модули: FSM сжатия, задачи и MCP-подключения, граф допуска и guards переходов, тексты отказа, распознавание предложения модели, политика сжатия, извлечение фактов, значения профиля, значения/правила/тексты инвариантов, разбор цели MCP и структура инструмента |
| `tests/integration/` | 19 | 268 | Хранилище, сервисы и агент на временной БД: `TaskStateStore`/`TaskStateMachine`, `InvariantManager`/`InvariantChecker`, `MemoryManager`, `ProfileStore`, `ContextCompressor`, `AgentManager`, `MCPRegistry`, `MCPClient` на настоящем stdio-сервере |
| `tests/e2e/` | 8 | 120 | API через `TestClient`: 57 эндпоинтов, коды 200/201/400/404/409/422, полный цикл задачи, поля `task_state`/`task_intent`/`task_proposal` и `invariants` в ответе генерации, четыре эндпоинта `/mcp` |

Наследственные наборы дня 15 (переходы), дня 14 (инварианты) и дни 11–12 (память,
персонализация, сжатие, стратегии) в дне 16 не менялись: при пустой таблице
`invariants` проверка не добавляет вызовов, а задача, не пересекающая границ
этапов, ведёт себя как раньше. MCP-тесты добавлены новыми файлами и ничего
унаследованного не подменяют: `FakeMCPClient` (в `tests/support.py`) повторяет
контракт `MCPClient` с настоящей FSM подключения, а
`tests/integration/test_mcp_stdio.py` проверяет реальный stdio-транспорт на своём
сервере `tests/mcp_echo_server.py`.
...
Новые файлы дня 16 (MCP; подпапка — по фикстурам; тестов в файле):

| Модуль | Строк | Тестов | Что проверяет |
|---|---|---|---|
| `tests/unit/test_mcp_connection_fsm.py` | 113 | 24 | Таблица FSM подключения (10 переходов) и её сверка с графом `ALLOWED_TRANSITIONS`, негативные пары → `UnknownMCPConnectionEvent` без смены состояния, идемпотентное отключение, `can`/`allowed_events`/`reset` |
| `tests/unit/test_mcp_target.py` | 131 | 23 | Выбор транспорта по виду цели, разбор команды (кавычки, обратные слэши Windows-путей), перевод `sse://` в `http://`, приоритет явного транспорта, ошибки разбора |
| `tests/unit/test_mcp_tools.py` | 70 | 8 | Нормализация полей инструмента, ошибка на инструмент без имени, глубокая копия схемы в `to_dict`, порядок в `make_tool_infos` |
| `tests/integration/test_mcp_stdio.py` | 82 | 5 | Настоящий MCP-сервер по stdio (`tests/mcp_echo_server.py`): соединение, непустой список с описанием и схемой, согласие кэша и `refresh`, ошибка без соединения, отключение и повторное подключение, контекстный менеджер |
| `tests/integration/test_mcp_client_errors.py` | 69 | 4 | Команда не найдена, недоступный HTTP-адрес, состояние `ERROR`, повтор подключения после ошибки, разбор цели до соединения |
| `tests/integration/test_mcp_registry.py` | 105 | 7 | Реестр: статус без подключения, число инструментов до и после `tools()`, закрытие прежнего соединения при переподключении, `disconnect`/`close`, видимая ошибка после сбоя |
| `tests/e2e/test_mcp_api.py` | 137 | 7 | Четыре эндпоинта через `TestClient`: `200/400/409/422/502`, форма `/mcp/tools` (`name`/`description`/`input_schema` + `count`), `refresh`, статус `error` после 502, отключение без соединения |

Новые файлы дня 15 (унаследованы без изменений):
...
| `backend/services/mcp_client.py` | `shared.logging_utils.get_logger` | Логи подключения, закрытия и ошибок MCP (запуск, список инструментов, обрыв) |
...
|---|---|---|---|
| `backend/agents/agent.py` | 1825 | 400 | Домен `Agent` (память, стратегии, токены, профиль, состояние задачи и переходы, инварианты, генерация) не разложен на миксины — расхождение унаследовано от дней 11–15 (в день 14 было 1727, в день 15 — 1825: +98 дали контролируемые переходы), рефакторинг `Agent` в дни 15–16 не входил |
| `invariants_demo.md` лежит в корне дня | — | — | Путь задан заданием дня 14; в `docs/reports/` файл не дублируется (там лежат `mcp_demo.md` дня 16 и унаследованные отчёты) |
...
| `scripts/video_scenario*.py` (8 модулей) | 69–386 | 400 | Унаследованы из дня 15 и проверяют кадры **прежнего** сценария (контролируемые переходы, §8 прежней инструкции); в `usage.md` дня 16 этого раздела нет — файл описывает только MCP. Скрипты не удалены как артефакт доказательств дня 15, но в демонстрации дня 16 не участвуют: минимальный сценарий даёт `scripts/mcp_demo.py` |
...
| `backend/services/mcp_client.py` делит цикл событий с вызывающим потоком | 388 | 400 | Клиент запускает свой daemon-поток с циклом событий и долгоживущую задачу сессии: контексты MCP SDK обязаны входить и выходить в одной задаче anyio. Публичные методы синхронные (FastAPI и Streamlit синхронные), потокобезопасные через `RLock` |

Лимиты `app.py` (62 ≤ 100) и `backend/api/main.py` (78 ≤ 80) соблюдены.

## Проверка лимита строк

Из папки `day16` (исключены `.venv` и вендорный `.agents/`):
