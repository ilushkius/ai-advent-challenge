"""Автопроверка кадров сценария видео дня 16 (``docs/usage.md`` §8).
...
Что НЕ трогается.
    База приложения ``day16/agents.db``: прогон поднимает uvicorn на СВОЕЙ
...
def run() -> None:
    ...
    try:
        ...
        # не нужен вовсе.
        os.environ["DAY16_BACKEND_URL"] = client.base_url
