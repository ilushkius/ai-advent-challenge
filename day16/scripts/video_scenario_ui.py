"""UI-кадры сценария видео дня 16: реальный рендер Streamlit через AppTest.
...
Границы.
    Бэкенд поднимает оркестратор: адрес прогона лежит в ``DAY16_BACKEND_URL``
...
# Скрипт лежит в day16/scripts/, а приложение — в корне дня: добавляем корень дня
