# Архитектура дня 16 — «MCP-клиент: подключение к внешнему серверу и его инструменты»

Приложение дня 16 — агент DeepSeek (FastAPI-бэкенд + Streamlit-интерфейс,
...
переходами. Новое в этом дне — **MCP-клиент**: приложение подключается к внешнему
MCP-серверу (Model Context Protocol) и получает список его инструментов
(`get_tools`). Соединение открывается по stdio (сервер запускается дочерним
процессом: `uvx mcp-server-fetch`, `npx -y @modelcontextprotocol/server-filesystem .`)
или по HTTP (Streamable HTTP, SSE), состояние подключения описано стейт-машиной,
а список инструментов отдаётся полями `name`, `description`, `input_schema`.
Вызова инструментов в дне нет — только чтение каталога.
...
- клиент соединения — `backend/services/mcp_client.py` (`MCPClient`): транспорты
  ...
  `tools/list`, понятные тексты ошибок вместо traceback;
...
Подробно — в разделе [«MCP-интеграция»](#mcp-интеграция).
...
flowchart LR
    ...
    CP -->|OpenAI SDK| D
    AG --> DB[("SQLite<br/>day16/agents.db")]
    MM --> DB
    PS --> DB
    TS --> DB
    IM --> DB
...
```
day16/
├── app.py            точка входа Streamlit (58 строк): только вызовы секций
...
│   │                 dependencies (get_manager, get_mcp_registry, *_or_404)
...
│   │                 user_profile, task_state, invariant
│   ├── schemas/      Pydantic-схемы API по доменам: agent, context, invariant, mcp,
│   │                 memory, profile, task
│   ├── api/          FastAPI: agents, context, invariants, mcp, memory, profiles, tasks + main
│   └── utils/        слой объявлен обязательным, но пуст: общий код — в repo-level shared/
├── tests/            unit/ (16 файлов), integration/ (19), e2e/ (8) + conftest.py и support.py
...
| Файл | Что в нём |
|---|---|
| `pyproject.toml` | одиннадцать **прямых** зависимостей дня (`fastapi`, `uvicorn[standard]`, `streamlit`, `openai`, `requests`, `sqlalchemy`, `tiktoken`, `httpx`, `pytest`, `pandas`, `mcp`) и нижняя граница версии Python (`requires-python = ">=3.14"`) |
...
`uv sync` не пытается установить сам `day16/` в `.venv`.

## Компоненты

| Файл | Зона ответственности |
| --- | --- |
| `day16/app.py` | Streamlit, точка входа (58 строк): `st.set_page_config` («Контролируемые переходы · День 16», «🚦»), `common.init_state()` → `sidebar.render_sidebar()` → `chat_section.render_main_area()`; переключатель четырёх разделов основной области — `st.radio` («💬 Чат и память» / «👤 Профиль пользователя» / «🧭 Состояние задачи» / «📏 Инварианты») |
...
| `backend/services/mcp_client.py` | `MCPClient` (день 16) — соединение с одним MCP-сервером: свой поток с циклом событий и долгоживущая задача `_serve` (контексты MCP SDK входят и выходят в одной задаче), выбор транспорта `stdio`/`sse`/`streamable_http`, `connect`/`list_tools(refresh)`/`disconnect`/`close`, постраничный `tools/list` (предел `MCP_MAX_TOOL_PAGES`), синхронный интерфейс для FastAPI и Streamlit |
| `backend/services/mcp_errors.py` | Ошибки и тексты MCP: `MCPError`/`MCPConnectionError`/`MCPNotConnectedError`/`MCPToolsError`, `ACTION_LABELS` (connect/tools/serve), `leaf_errors` (разворачивание `ExceptionGroup` транспорта), `error_message` — одна строка «что делали, что случилось, что проверить» |
| `backend/services/mcp_registry.py` | `MCPRegistry` — одно активное MCP-подключение процесса: `connect` (закрывает прежнее), `disconnect`, `tools(refresh)`, `status()` (словарь для схемы API), `close()` в `lifespan`; синглтон `get_mcp_registry` |
...
| `backend/domain/mcp_tools.py` | `MCPToolInfo` (`name`, `description`, `input_schema`, `to_dict` с глубокой копией схемы), `make_tool_info`/`make_tool_infos`, `MCPToolError` (инструмент без имени — ошибка) |
| `backend/api/mcp.py` | Роутер `/mcp`: `POST /mcp/connect`, `POST /mcp/disconnect`, `GET /mcp/status`, `GET /mcp/tools?refresh=`; переводит `MCPTargetError` в 400, ошибки MCP — в 502, отсутствие соединения — в 409 |
| `backend/schemas/mcp.py` | `MCPConnectIn` (цель + транспорт), `MCPStatusResponse`, `MCPToolSchema` (`from_info`), `MCPToolsResponse` (`tools`, `count`, сервер) |
| `frontend/mcp_section.py` | Раздел «🔌 MCP»: поле цели, селектор транспорта, кнопки «🔌 Подключиться» / «⏏ Отключиться», плашка состояния, таблица инструментов, «🔄 Обновить список инструментов», разбор полной `input_schema`, подсказка без соединения |
| `frontend/mcp_api.py` | Запросы MCP-раздела поверх общего транспорта: `api_mcp_status`, `api_mcp_connect`, `api_mcp_disconnect`, `api_mcp_tools(refresh)` |
| `scripts/mcp_demo.py` | Минимальная демонстрация дня: `connect` → `list_tools` → `disconnect` в консоли, читаемый вывод и `--json`; ошибки подключения печатаются одной строкой (код выхода 1) |
| `backend/core/config.py` | URL и модели DeepSeek, дефолты агента, лимиты/тарифы, настройки сжатия, `DEFAULT_STRATEGY`/`DEFAULT_WINDOW_SIZE`, константы слоёв (`DEFAULT_TASK_ID`, `LONG_TERM_LIMIT`, лимиты записей), границы полей состояния задачи (`TASK_STAGE_MAX`, `TASK_STEP_MAX`, `EXPECTED_ACTION_MAX`, `TASK_REASON_MAX`), границы и настройки инвариантов (`INVARIANT_NAME_MAX`, `INVARIANT_DESCRIPTION_MAX`, `INVARIANT_LLM_CHECK`, `INVARIANT_CHECK_MAX_TOKENS`/`TEMPERATURE`), настройки MCP (`MCP_DEFAULT_TARGET`, `MCP_FILESYSTEM_TARGET`, `MCP_TIMEOUT`, `MCP_MAX_TOOL_PAGES`, `MCP_TARGET_MAX`), путь БД и `.env` |
...
| `backend/schemas/` | Пакет Pydantic-схем API по доменам: `agent.py` (конфигурация/патч агента, генерация с полями `task_state`, `task_intent`, `task_proposal` и `invariants`, `TokenMetrics`), `context.py` (сжатие, стратегии, ветки, факты), `invariant.py` (инварианты и результат проверки: `InvariantIn`/`InvariantUpdateIn`/`InvariantOut`/`InvariantCheckIn`/`InvariantCheckOut`/`InvariantViolationOut`), `memory.py` (три слоя памяти), `profile.py` (схемы персонализации `UserProfileIn`/`UserPreferences`/`UserConstraints` с `extra="forbid"`, `UserProfileOut`, `AppliedProfileOut`), `task.py` (состояние задачи: `TaskStateOut` с `allowed_next`/`blocked`, `TaskTransitionOut` с `accepted`, `TaskAllowedNextOut`, `TaskBlockedOut`, `TaskFlagsIn`, тела переходов); `schemas/__init__.py` реэкспортирует все имена, поэтому импорт остался `from backend.schemas import ...` |
| `backend/api/` | Эндпоинты по доменам: `agents.py` (11), `context.py` (9), `invariants.py` (6), `memory.py` (10), `profiles.py` (6), `tasks.py` (11) — всего 53; пути абсолютные (`/agents/...`, `/invariants...`), префиксов нет |
...
| `frontend/` | Streamlit-интерфейс по секциям (12 модулей): `sidebar`, `chat_section` (переключатель четырёх разделов `st.radio` с ключом `main_section`, карточка агента, диалог, сводка после хода, блок предупреждения/отказа по инвариантам над вводом), `context_panels`, `memory_panels`, `profile_section`, `profile_comparison`, `task_panel` (раздел «🧭 Состояние задачи»), `task_transitions` (кнопки-этапы, причины недоступности, флаги-согласования, пауза/продолжение), `invariant_panel` (раздел «📏 Инварианты»), `common` (состояние сессии, `active_agent`, `TASK_STAGE_LABELS`, `TASK_FLAG_LABELS`, `blocked_reason`, `run_task_action`, `INVARIANT_*_LABELS`, `invariant_notice`, `fmt_time`, `flash`), `api_client` (`BACKEND_URL` из `DAY16_BACKEND_URL`, `_request` и обёртки эндпоинтов, включая одиннадцать функций состояния задачи и шесть — инвариантов), `__init__`; `app.py` — только точка входа (58 строк), модули `frontend/` не вызывают `st.*` на импорте |
...
| `backend/api/main.py` | Сборка FastAPI-приложения (80 строк, версия `9.0.0`): заголовок «Агенты DeepSeek с контролируемыми переходами — День 16» и описание, CORS, `lifespan` (создание таблиц + восстановление агентов), `include_router` шести роутеров; сами 53 эндпоинта (память, стратегии, ветки, факты и метрики, профили, состояние задачи, корневой `GET /` и инварианты) и обработка 404/409/422/502 — в `backend/api/` |
...
|---|---|---|
| Интерфейс | `app.py` (точка входа, 58 строк), `frontend/` (12 модулей) | Страница собирается вызовами секций: `common.init_state()` → `sidebar.render_sidebar()` → `chat_section.render_main_area()`; модули `frontend/` не вызывают `st.*` на импорте |
...
| Домен | `backend/domain/` (`strategies`, `context_fsm`, `context_policy`, `fact_extractor`, `memory_layers`, `profile_values`, `profiles`, `demo_profiles`, `task_fsm`, `task_state_machine`, `task_proposal`, `task_prompt`, `task_intent`, `invariant_values`, `invariant_rules`, `invariant_prompt`, `demo_invariants`), `backend/services/` (`compressor`, `task_state`, `invariant_checker`), `backend/storage/` (`database`, `task_store`, `invariant_store`, `memory_rows`), `backend/agents/` (`agent`, `agent_manager` + `manager_*.py`, `memory`, `profile_store`) | Логика дня; крупный класс `AgentManager` собран из миксинов по доменам, состояние задачи разложено на «автомат шагов (`task_fsm`) / граф и guards (`task_state_machine`) / поведение и хранение», проверка инвариантов — на «правила / оркестрация / хранение», а чистые правила (`domain/`) не зависят ни от БД, ни от HTTP |
| Общий код | `shared/` | Код, не меняющийся между днями: клиент DeepSeek, база SQLAlchemy, токены, логи |
| Тесты | `tests/unit/` (13 файлов, в том числе новые `test_task_state_machine.py`, `test_task_transition_texts.py`, `test_task_proposal.py`), `tests/integration/` (16, в том числе новый `test_task_transitions.py`), `tests/e2e/` (7, в том числе новый `test_task_transitions_api.py`) + `conftest.py`, `support.py` | Раскладка по фикстурам: `unit/` — без БД и агента, `integration/` — временная SQLite и `Agent`, `e2e/` — `TestClient`; общие фикстуры и фейки остаются в корне `tests/` |
| Скрипты | `scripts/` (`controlled_transitions_demo.py` + `transitions_report.py` — доказательство дня 16, `task_state_demo.py`, `invariants_demo.py`, `seed_invariants.py`, `personalization_comparison.py`) | Прогоны вне pytest на своих БД: пишут отчёты в `docs/reports/` (или в корень дня), ключ и сеть не нужны |
...
ORM-класс `database.UserProfile`, таблица `user_profiles` в `day16/agents.db`:
...
файловую систему, веб-запросы, базы данных. День 16 **подключается** к
общедоступному MCP-серверу и **читает каталог** его инструментов (`get_tools`);
вызова инструментов (`tools/call`) в дне нет — это следующий шаг, и он не
смешивается с текущим.

Подсистема живёт в трёх слоях: чистая логика (домен), соединение и реестр
(сервисы), транспорт (API) — и не касается таблиц БД: MCP-подключение — это
связь с внешним процессом, а не данные домена.

```
Streamlit «🔌 MCP» ──HTTP──► backend/api/mcp.py ──► MCPRegistry ──► MCPClient
  (frontend/mcp_section.py,      (роутер /mcp/...),   (одно подключение    (поток с
   frontend/mcp_api.py)          схемы schemas/mcp.py  на процесс)         циклом событий
                                                                            + MCP SDK)
                                                                              │
                                                            ┌─────────────────┴──────────────┐
                                                            ▼                                ▼
                                                     stdio: процесс сервера         HTTP: Streamable HTTP / SSE
                                                  (uvx mcp-server-fetch,              (http://… , sse://…)
...
|---|---|---|
|`stdio`|команда запуска (`uvx mcp-server-fetch`, `npx -y @modelcontextprotocol/server-filesystem .`, префикс `stdio:` необязателен)|`stdio_client(StdioServerParameters(...))` — сервер запускается дочерним процессом, обмен JSON-RPC по stdin/stdout|
...
- **обрыв связи** во время работы (`CONNECTED` + `fail`) переводит в `ERROR`:
  следующий `GET /mcp/tools` не пытается «достучаться» до мёртвой сессии, а
  отвечает `502` с причиной;
...
- `MCPClient` держит **свой поток** с циклом событий (`_ensure_loop`) и одну
  долгоживущую задачу `_serve`: она входит в контексты транспорта и сессии, делает
  `initialize`, сообщает о готовности через `threading.Event` и ждёт события
  `shutdown`, после которого закрывает контексты там же, где открыла;
- публичные методы синхронные и потокобезопасные (`RLock`): `connect()`,
  `list_tools(refresh=False)`, `disconnect()`, `close()`; запрос `tools/list`
  выполняется отдельной задачей того же цикла — он не привязан к задаче-владельцу
  контекстов;
...
(`connect`/`tools`/`serve`), развёрнутый `ExceptionGroup` транспорта (`leaf_errors`)
и подсказки по конкретной причине (нет команды в PATH, истёк таймаут, HTTP-транспорт
— проверьте URL). Отказ никогда не оставляет молчаливого состояния: клиент
переходит в `ERROR`, а `GET /mcp/status` показывает текст последней ошибки.
...
4. Раздел читает `GET /mcp/tools` → `MCPRegistry.tools` → `MCPClient.list_tools`,
   рисует таблицу `name`/`description`/`input_schema` и «Инструментов: N»;
   число инструментов подставляется и в плашку статуса.
5. «⏏ Отключиться» → `POST /mcp/disconnect` → `MCPClient.disconnect`: событие
   `shutdown`, ожидание завершения задачи, выход из контекстов (процесс
   stdio-сервера завершается), FSM → `DISCONNECTED`.

## Схема БД (`day16/agents.db`)
...
клиента. Поэтому схема БД днём 16 не меняется, а после перезапуска бэкенда
`/mcp/status` отвечает `disconnected`.
...
`DAY16_BACKEND_URL` (по умолчанию `http://127.0.0.1:8000`), таймаут 90 с
...
**«🧭 Состояние задачи»**, **«📏 Инварианты»** и **«🔌 MCP»** (ключ `main_section`) — именно radio, а не `st.tabs`:
...
| Раздел «🔌 MCP» | Раздел MCP-подключения (`frontend/mcp_section.py`): поле «URL или команда запуска MCP-сервера», селектор транспорта (`auto` / `stdio` / `sse` / `http`), кнопки «🔌 Подключиться» и «⏏ Отключиться», плашка состояния («⚪ не подключено» / «🟢 подключено — сервер, версия, протокол, инструментов: N» / «🔴 ошибка подключения» с текстом причины), строка деталей (цель, транспорт, допустимые события), таблица инструментов `name`/`description`/`input_schema`, кнопка «🔄 Обновить список инструментов» (`refresh=true`), раскладка «🧾 Полная input_schema инструмента» и подсказка, пока соединения нет |
...
SQLite-БД (фикстуры `session_factory` и `make_agent`). Всего **1097** тестов, из
них **558** — по состоянию задачи (тринадцать файлов `test_task_*`), **141** —
по инвариантам и **84** — по MCP (`test_mcp_*`), зелёные.
...
| `tests/unit/test_mcp_tools.py` | Нормализация полей инструмента (`None` → `""`/`{}`, срез пробелов), ошибка на инструмент без имени, глубокая копия схемы в `to_dict`, порядок в `make_tool_infos` |
...
| `tests/e2e/test_mcp_api.py` | Четыре эндпоинта через `TestClient`: `200/400/409/422/502`, форма ответа `/mcp/tools` (`name`/`description`/`input_schema` + `count`), `refresh`, статус `error` после 502, отключение без соединения |
...
Демонстрация дня 16 — **MCP-клиент**: подключение к внешнему серверу и его
инструменты. Пошаговый сценарий — в инструкции [`usage.md`](usage.md) (§3–§7):
минимальный код из `scripts/mcp_demo.py` на экране и его вывод в консоли; раздел
«🔌 MCP» в интерфейсе — цель `uvx mcp-server-fetch`, «🔌 Подключиться», зелёная
плашка с именем сервера и версией протокола, таблица с инструментом `fetch` и его
`input_schema`; второй сервер
`npx -y @modelcontextprotocol/server-filesystem .` — 14 инструментов и кнопка
«🔄 Обновить список инструментов»; неверная цель (`http://127.0.0.1:9/mcp`) —
красная плашка с текстом ошибки и «⏏ Отключиться» в конце. Числа и строки для
сверки — в отчёте [`reports/mcp_demo.md`](reports/mcp_demo.md)
(`uv run python scripts/mcp_demo.py`, ключ и сеть для MCP не нужны).
...
переходы) и ссылаются на §8 инструкции, которого в `usage.md` дня 16 больше нет —
файл описывает только MCP. Отчёты этих прогонов
...
- **Одно MCP-подключение на процесс.** Реестр держит один клиент: подключение к
  другому серверу закрывает прежнее. Это соответствует задаче дня; одновременная
  работа с несколькими серверами потребовала бы реестра по имени.
- **Инструменты не вызываются.** День читает каталог (`tools/list`); `tools/call`,
  преобразование инструментов в функции LLM и политика разрешений — следующий шаг.
...
- **Один процесс бэкенда и один файл `day16/agents.db`.** Несколько процессов
...
- **Единственное превышение лимита строк — `backend/agents/agent.py`: 1825 строк**
  (в дне 14 — 1727, в дне 13 — 1603): файл растёт вместе с функциями агента
  (память, профиль, состояние задачи, инварианты, контролируемые переходы).
  Остальные файлы укладываются в 400 строк, а `backend/api/main.py` стоит ровно
  на границе — 80 строк при лимите 80.
