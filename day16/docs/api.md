# API дня 16 — агенты DeepSeek с MCP-клиентом, контролируемыми переходами, инвариантами, состоянием задачи, памятью и профилем

Бэкенд — FastAPI-приложение `day16/backend/api/main.py`. Заголовок приложения —
«Агенты DeepSeek + MCP — День 16», версия схемы — `10.0.0`
...
**MCP-клиент** (день 16) — подключение к внешнему MCP-серверу (Model Context
Protocol) и получение списка его инструментов: четыре эндпоинта `/mcp/connect`,
`/mcp/disconnect`, `/mcp/status` и `/mcp/tools`. Транспорт выбирается по виду
цели (команда запуска — stdio, `http://` — Streamable HTTP, `sse://` — SSE),
список инструментов отдаётся полями `name`, `description`, `input_schema` вместе
с `count`. Соединение одно на процесс и **не переживает** перезапуск бэкенда (это
связь с внешним процессом, а не данные домена); при остановке приложения оно
закрывается в `lifespan`. Инструменты только читаются — вызовов (`tools/call`) у
дня нет. Разбор — в разделе [«MCP»](#mcp).

Схемы ответов описаны в пакете `day16/backend/schemas/` — по доменам: `agent.py`
...
и инструменты сервера), `memory.py` (три слоя памяти),
...
Всего 57 эндпоинтов: 11 в разделе агентов (десять эндпоинтов CRUD, генерации и
...
| GET | `/mcp/tools` | список инструментов сервера (`name`, `description`, `input_schema`, `count`) | 200 MCPToolsResponse |
...
```json
{
  "name": "Агенты DeepSeek с контролируемыми переходами — День 16",
  "docs": "/docs",
...
Четыре эндпоинта: `POST /mcp/connect`, `POST /mcp/disconnect`,
`GET /mcp/status`, `GET /mcp/tools`. Контракт ошибок: `400` — цель не разобрана
...
таймаут или ошибка `tools/list` (текст — одна строка с подсказкой).
...
| `MCPToolSchema` | `name`, `description`, `input_schema` (JSON Schema аргументов как отдал сервер) |
...
| `409` | профиль с таким `user_id` уже есть (`POST /users/{user_id}/profile`); задача с таким `task_id` уже заведена (`POST /agents/{agent_id}/tasks`); `GET /mcp/tools` без соединения с MCP-сервером | `HTTPException` с `detail` |
| `422` | невалидное тело запроса (Pydantic/FastAPI): невалидные поля профиля, `initial_stage` вне `planning`/`execution`/`validation`, неизвестный этап/шаг, слишком длинные `expected_action`/`reason`, пустой `task_id`, тело `PATCH /tasks/{task_id}/context` без единого флага, пустая цель или неизвестный транспорт у `POST /mcp/connect` | объект с `detail` — списком ошибок |
| `502` | сбой генерации `POST /generate` (нет ключа, сеть, лимиты); MCP-сервер недоступен, команда запуска не найдена, таймаут `initialize` или ошибка `tools/list` | `GenerateResponse` со `status:"error"`; у MCP — `HTTPException` с одной строкой текста и подсказкой |
...
- **Всё состояние переживает рестарт.** Реплики (`short_term_messages`), рабочая
  ...
  журналом `task_transitions`) лежат в SQLite (`day16/agents.db`), а активные
