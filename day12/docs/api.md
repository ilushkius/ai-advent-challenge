# API дня 12 — агенты DeepSeek с трёхслойной памятью и профилем пользователя

Бэкенд — FastAPI-приложение `day12/backend/main.py`. Заголовок приложения —
«Агенты DeepSeek с профилем пользователя — День 12», версия схемы — `6.0.0`
...
День 12 добавляет к этому персонализацию: у агента есть поле `user_id` —
...
Схемы ответов описаны в `day12/backend/models.py`; сводная таблица полей — в
разделе [«Формы данных»](#формы-данных).

## Эндпоинты

Всего 36 эндпоинтов: 30 в разделе агентов (`/agents...`), пять эндпоинтов
профилей пользователя в `/users` и корневой `GET /`.
...
Корневая точка — подсказка: имя приложения, путь к Swagger, префиксы памяти и
персонализации и перечень эндпоинтов.
...
2. собирает ОДНО системное сообщение: блок профиля пользователя (если профиль
   ...
   память → конспект → факты; блок профиля — часть системного сообщения, поэтому
   он попадает в контекст при любой стратегии и учитывается в оценке токенов;
...
| `system_prompt` | строка | итоговое system-сообщение запроса (профиль + роль + блоки памяти; считается до вызова DeepSeek) |
...
| `404` | неизвестный `agent_id` во всех `/agents/{agent_id}/...` (а для `DELETE /memory/long-term/{id}` — ещё и отсутствующая запись); нет профиля у `GET`/`PUT`/`DELETE /users/{user_id}/profile` | `HTTPException` с `detail` |
| `409` | профиль с таким `user_id` уже есть (`POST /users/{user_id}/profile`) | `HTTPException` с `detail` |
| `422` | невалидное тело запроса (Pydantic/FastAPI), в том числе невалидные поля профиля | объект с `detail` — списком ошибок |
...
- **Всё состояние переживает рестарт.** Реплики (`short_term_messages`), рабочая
  ...
  (`summaries`), факты (`facts`), ветки (`checkpoints`), записи `token_usage` и
  профили пользователей (`user_profiles`) лежат в SQLite (`day12/agents.db`), а
  активные сессия и задача — в строке `agents`
  ...
  (`restore_from_db`), поэтому `/usage`, `/summary`, `/history`, `/users/...` и
  эндпоинты `/memory/...` после перезапуска показывают те же данные.
