...
- ``get_user_profile`` / ``create_user_profile`` / ``update_user_profile`` /
  ``delete_user_profile`` / ``list_user_profiles`` (день 12) — CRUD профилей
  пользователей. Изменение профиля применяется к живым агентам этого
  пользователя сразу (``Agent.apply_profile``), поэтому следующий запрос уже
...
from .profile_store import (
    ProfileData, ProfileExistsError, ProfileNotFoundError, ProfileStore,
    empty_profile,
)
