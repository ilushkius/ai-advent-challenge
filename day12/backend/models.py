"""Pydantic-схемы API дня 12 (единое место моделей — конвенция дня 6).

День 12 добавляет персонализацию: схемы профиля пользователя
(``UserPreferences``/``UserConstraints``/``UserProfileIn``/``UserProfileOut``),
разбивку применённого профиля в ответе генерации (``AppliedProfileOut`` +
``ProfileElementOut``) и поле ``user_id`` у агента. Валидация значений профиля
не дублируется: схемы вызывают ``backend/profiles.py``
(``normalize_preferences``/``normalize_constraints``/``normalize_instructions``),
поэтому неизвестный tone или выход за границы constraints — это 422 с текстом
из одного места. Остальные схемы — из дня 11 (три слоя памяти).
"""
...
# ---------- персонализация: профиль пользователя (день 12) ----------
